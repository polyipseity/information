...
After rewriting {@{the joint probability model}@}, {@{the "naive" assumptions}@} come into play: Assume that {@{all features of $\mathbf{x}$}@} are {@{[mutually independent](independence%20(probability%20theory).md), conditional on the class $C_k$}@}, written mathematically as: {@{$$p(x_i \mid x_{i + 1}, \ldots, x_n, C_k) = p(x_i \mid C_k) \,.$$}@} <!--SR:!2031-10-22,2002,330!2026-10-25,652,330-->
