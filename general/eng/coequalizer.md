...
- {@{The coequalizer in the [category of groups](category%20of%20groups.md)}@} is {@{very similar}@}. Here {@{if _f_, _g_ : _X_ → _Y_ are [group homomorphisms](group%20homomorphism.md)}@}, their coequalizer is {@{the [quotient](quotient%20group.md) of _Y_ by the [normal closure](normal%20closure%20(group%20theory).md) of the set $$S=\{f(x)g(x)^{-1}\mid x\in X\}$$}@} \(annotation: Intuitively, why do we use {@{this set to take the quotient}@}? Consider {@{an element $x \in X$}@}. A coequalizer should {@{send $f(x)$ and $g(x)$, both in $Y$, to the "same" group element}@}. If {@{they are the "same" group element}@}, then {@{$f(x) g(x)^{-1}$ is the identity element}@}. If {@{they are not}@}, the coequalizer should {@{"collapse" $Y$ along $f(x) g(x)^{-1}$ such that it becomes the identity element}@}, hence {@{the quotient}@}. And {@{its normal closure}@} is taken because {@{the quotient requires it}@}.\)
...
- In {@{the [category of topological spaces](category%20of%20topological%20spaces.md)}@}, {@{the circle object _S_<sup>1</sup>}@} can be viewed as {@{the coequalizer of the two inclusion maps from the standard 0-simplex to the standard 1-simplex}@}. (annotation: {@{The standard 0-simplex}@} is {@{a single point}@}, and {@{the standard 1-simplex is a line segment}@}. {@{The two inclusion maps}@} send {@{the single point to the two endpoints of the line segment}@}. {@{The coequalizer}@} {@{identifies these two endpoints}@}, resulting in {@{a circle}@}.)
