...
This can be derived using {@{the triple product expansion}@}: {@{$$\mathbf {c} \times \mathbf {a} =(\mathbf {a} \times \mathbf {b} )\times \mathbf {a} =\left\|\mathbf {a} \right\|^{2}\mathbf {b} -(\mathbf {a} \cdot \mathbf {b} )\mathbf {a}$$}@} {@{Rearrange to solve for __b__}@} to give {@{$$\mathbf {b} ={\frac {\mathbf {c} \times \mathbf {a} }{\left\|\mathbf {a} \right\|^{2} } }+{\frac {\mathbf {a} \cdot \mathbf {b} }{\left\|\mathbf {a} \right\|^{2} } }\mathbf {a}$$}@} {@{The coefficient of the last term}@} can be {@{simplified to just the arbitrary constant _t_}@} to {@{yield the result shown above}@}.
