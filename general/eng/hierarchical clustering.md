...
One way to implement this is, after creating a cluster for each observation, {@{construct a [distance matrix](distance%20matrix.md) of all clusters, where the number in the _i_-th row and _j_-th column is the [distance](distance.md) between the _i_-th and _j_-th clusters}@}. Use the matrix to {@{identify the pair of clusters with the least distance}@}. After merging the pair of clusters, {@{the distance matrix should decrease in dimension by one in both axes. Update the distances to the new merged cluster from other untouched clusters}@}. Repeat this until {@{you have only one cluster left}@}. An optimization is that since distance is symmetric, we can use {@{a lower [triangular matrix](triangular%20matrix.md) to store the distances}@}. <!--SR:!2026-04-10,528,295!2026-07-14,578,315!2030-04-16,1613,315!2026-05-21,568,315!2029-10-28,1546,335-->
