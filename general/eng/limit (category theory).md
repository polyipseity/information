...
Let {@{_I_ be a finite category and _J_ be a small [filtered category](filtered%20category.md)}@}. (annotation: {@{A filtered category}@} generalizes {@{the notion of a directed set}@}.) For {@{any [bifunctor](bifunctor.md#bifunctors%20and%20multifunctors) $$F:I\times J\to \mathbf {Set} ,$$}@} there is {@{a [natural isomorphism](natural%20isomorphism.md#natural%20isomorphism) $$\operatorname {colim} \limits _{J}\lim _{I}F(i,j)\rightarrow \lim _{I}\operatorname {colim} \limits _{J}F(i,j).$$}@} In words, {@{filtered colimits in __Set__ commute with finite limits}@}.<!-- It also holds that small colimits commute with small limits. --><sup>[\[2\]](#^ref-2)</sup>
...
- {@{Every [representable functor](representable%20functor.md) _C_ → __Set__}@} {@{preserves limits \(but not necessarily colimits\)}@}. In particular, for {@{any object _A_ of _C_}@}, this is true of {@{the covariant [Hom functor](hom%20functor.md) Hom\(_A_,–\) : _C_ → __Set__}@}. \(annotation: {@{The covariant functor $\text{Hom}(A, -)$}@} preserves {@{limits such as the product $X \times Y$}@} because {@{the natural bijection $\text{Hom}(A, X \times Y) \cong \text{Hom}(A, X) \times \text{Hom}(A, Y)$ holds for any object $A$}@}. However, it generally fails to preserve {@{colimits like the coproduct $X \sqcup Y$}@} because an element of {@{$\text{Hom}(A, X \sqcup Y)$ is a single morphism that must map $A$ into the union}@}, whereas an element of {@{$\text{Hom}(A, X) \sqcup \text{Hom}(A, Y)$ must map $A$ entirely into $X$ or entirely into $Y$}@}, a condition that {@{fails unless $A$ is connected or "indecomposable"}@}.\)
- {@{The [forgetful functor](forgetful%20functor.md) _U_ : __Grp__ → __Set__}@} {@{creates \(and preserves\) all small limits and [filtered colimits](filtered%20colimit.md)}@}; however, {@{_U_ does not preserve coproducts}@}. This situation is {@{typical of algebraic forgetful functors}@}. \(annotation: {@{The forgetful functor $U$}@} preserves {@{all small limits}@} because {@{the underlying set of a group product $\prod G_i$}@} is precisely {@{the Cartesian product of the underlying sets $\prod U(G_i)$ equipped with pointwise operations}@}. Conversely, $U$ does not {@{preserve coproducts}@} because {@{the underlying set of the free product $G * H$}@} consists of all {@{reduced words formed by alternating elements from both groups}@}, which is significantly {@{larger and more complex than the simple disjoint union of sets $U(G) \sqcup U(H)$}@}.\)
- {@{The [free functor](free%20functor.md#free%20functor) _F_ : __Set__ → __Grp__ \(which assigns to every set _S_ the [free group](free%20group.md) over _S_\)}@} is {@{left adjoint to forgetful functor _U_ and is, therefore, cocontinuous}@}. This explains why {@{the [free product](free%20product.md) of two free groups _G_ and _H_}@} is {@{the free group generated by the [disjoint union](disjoint%20union.md) of the generators of _G_ and _H_}@}. \(annotation: As {@{a left adjoint}@}, {@{the free functor $F$}@} is {@{cocontinuous and thus preserves the coproduct of sets $S \sqcup T$}@} by mapping it to {@{the free group $F(S \sqcup T) \cong F(S) * F(T)$, where $*$ denotes the free product}@}. However, it fails to preserve {@{limits like the Cartesian product $S \times T$}@} because {@{the free group generated by a product of sets $F(S \times T)$}@} does not satisfy {@{the universal property of the group product $F(S) \times F(T)$}@}; for instance, elements in {@{$F(S \times T)$ do not generally commute}@} in {@{the way required by the direct product of groups}@}.\)
- {@{The inclusion functor __Ab__ → __Grp__}@} {@{creates limits but does not preserve coproducts}@} \({@{the coproduct of two abelian groups}@} being {@{the [direct sum](direct%20sum%20of%20abelian%20groups.md#direct%20sum%20of%20abelian%20groups)}@}\). \(annotation: {@{The inclusion functor $i: \text{Ab} \to \text{Grp}$}@} {@{creates and preserves limits such as the product $A \times B$}@} because if {@{$A$ and $B$ are abelian, their group-theoretic product remains abelian}@}. It fails to {@{preserve coproducts}@} because {@{the coproduct in $\text{Ab}$}@} is {@{the direct sum $A \oplus B$ (the quotient of the free product where all elements commute)}@}, while {@{the coproduct in $\text{Grp}$}@} is {@{the non-abelian free product $A * B$}@}, meaning {@{$i(A \oplus B) \ncong i(A) * i(B)$}@}.\)
- {@{The forgetful functor __Top__ → __Set__}@} {@{lifts limits and colimits uniquely but creates neither}@}. \(annotation: {@{The forgetful functor $U: \text{Top} \to \text{Set}$}@} {@{lifts limits and colimits}@}, meaning that given {@{a diagram in $\text{Top}$}@}, one can equip {@{the limit set (like the Cartesian product $\prod X_i$) with a unique "coarsest" topology (the product topology) to satisfy the universal property}@}. It does not {@{create them}@}, however, because {@{the existence of the limit in $\text{Set}$}@} does not {@{automatically force the existence of a unique topological structure in $\text{Top}$}@} unless you specifically {@{choose the one that satisfies the initial or final requirement}@}, e.g., {@{the product topology for limits and the quotient topology for colimits}@}.\)
- Let {@{__Met__<sub>_c_</sub>}@} be {@{the category of [metric spaces](metric%20space.md) with [continuous functions](continuous%20function.md) for morphisms}@}. {@{The forgetful functor __Met__<sub>_c_</sub> → __Set__}@} {@{lifts finite limits but does not lift them uniquely}@}. \(annotation: In the category of {@{metric spaces with continuous maps}@}, {@{the forgetful functor}@} {@{lifts finite limits like $X \times Y$}@} by allowing {@{the underlying set to be equipped with a metric}@} such as {@{$d_{\infty}((x_1, y_1), (x_2, y_2)) = \max(d_X(x_1, x_2), d_Y(y_1, y_2))$}@}. It fails to {@{lift them uniquely because multiple non-equivalent metrics}@}, such as {@{the $L^1$ metric $d_1 = d_X + d_Y$ or the $L^2$ Euclidean metric}@}, yield {@{the same underlying set and topology but constitute distinct objects within the metric category}@}.\)
