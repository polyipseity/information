...
The last condition {@{comes from the inhomogeneous equation}@}. After {@{deriving the $n$-th derivative}@}, keep {@{the extra expression $\sum_{i = 1}^n c_i'(x) y_i^{(n - 1)}(x)$}@}. Substitute {@{the $n - 1$ equations we have just derived above into the inhomogeneous equation}@}, then replace {@{the homogeneous equations that subsequently appear with 0}@} to get the last condition: {@{$$\sum_{i = 1}^n c_i'(x) y_i^{(n - 1)}(x) = \frac{b(x)}{a_n(x)}$$, where $b(x)$ is the constant term and $a_n(x)$ is the leading coefficient}@}. Solve {@{the system of linear equations of $n$ conditions to find $c_i(x)$}@}. <!--SR:!2027-04-18,666,290!2027-04-02,895,330-->
