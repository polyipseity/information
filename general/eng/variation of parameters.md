...
In general, {@{this cannot be done for all $x$ using constant coefficients only}@}. However, by {@{viewing the $n$ equations above as a [system of linear equations](system%20of%20linear%20equations.md)}@}, we find that we can {@{vary the constants $c_i$ such that the right hand side matches $y^{(j)}(x)$ at some $x_0$ and approximate nearby values}@}. <!--SR:!2026-11-03,780,330!2027-04-27,798,290!2025-02-23,4,336-->
