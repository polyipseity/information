...
The last condition {{comes from the inhomogeneous equation. For the $n$-th derivative, keep the extra expression $\sum_{i = 1}^n c_i'(x) y_i^{(n)}(x)$. Substitute the $n + 1$ equations we have just derived above into the inhomogeneous equation, replace the homogeneous equation that subsequently appears with 0 to get the last condition: $$\sum_{i = 1}^n c_i'(x) y_i^{(n)}(x) = \frac{b(x)}{a_n(x)}$$, where $b(x)$ is the constant term and $a_n(x)$ is the leading coefficient}}. Solve {{the system of linear equations of $n$ conditions to find $c_i(x)$}}. <!--SR:!2024-03-08,52,310!2024-01-18,16,290-->
