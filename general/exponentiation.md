...
> - {{$\left(e^x\right)^y = e^{xy} \qquad (x,y \in \mathbb{R})$ <br/>__counterexample__: $x = 1 + 2n\pi i, y = 1 + 2n\pi i \quad \forall{n} \in \mathbb{Z}$ <br/>__fix__: $\left(e^x\right)^y = e^{y \log e^{x}} \qquad (e^* \text{ and } \log(*) \text{ are multi-valued})$}}
