...
{{The _n_-th [derivative](derivative.md) of the _μ_-th power of the [natural logarithm](natural%20logarithm.md)}} involves {{the signed Stirling numbers of the first kind}}: {{$${\operatorname {d} ^{n}\!(\ln x)^{\mu } \over \operatorname {d} \!x^{n} }=x^{-n}\sum _{k=1}^{n}s(n,n-k+1)\mu ^{\underline {k} }(\ln x)^{\mu -k},$$}} where {{$\mu ^{\underline {i} }$ is the [falling factorial](falling%20and%20rising%20factorials.md), and $s(n,n-k+1)$ is the signed Stirling number}}.
