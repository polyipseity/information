...
For each $p$, we {{update $h(p)$ to $h(p) = \sum_{q \in p_{\text{from} } } a(q)$ where $p_{\text{from} }$ are all pages linked from $p$}}. That is, a page's hub value is {{the sum of the authority values of nodes it points to}}. This can also be expressed {{using [matrix multiplication](matrix%20multiplication.md)}}: {{$\mathbf{H}_k \gets \mathbf{M} \mathbf{A}_k$ for async iteration and $\mathbf{H}_k \gets \mathbf{M} \mathbf{A}_{k - 1}$ for sync iteration}}. <!--SR:!2024-08-26,55,314!2025-03-03,195,314!2024-08-29,58,314!2025-03-21,211,334-->
