...
>
> 1. {{Let $f(x, t)$ be a [function](function%20(mathematics).md) such that both $f(x, t)$ and its [partial derivative](partial%20derivative.md) with respect to $x$ $\frac{\partial}{\partial x} f(x, t)$ are [continuous](continuous%20function.md) in $x$ and $t$ over the $xt$-plane, including $x_0 \le x \le x_1$ and $\min(a([x_0, x_1]) \cup b([x_0, x_1])) \le t \le \max(a([x_0, x_1]) \cup b([x_0, x_1]))$.}}
> 2. {{Let $a(x)$ and $b(x)$ be [continuously differentiable functions](differentiable%20function.md) on $x_0 \le x \le x_1$.}}
> 3. {{Then, $$\frac{\mathrm{d} }{\mathrm{d}x} \int_{a(x)}^{b(x)} \! f(x, t) \, \mathrm{d}t = f(x, b(x)) b'(x) - f(x, a(x)) a'(x) + \int_{a(x)}^{b(x)} \! \frac\partial{\partial x} f(x, t) \, \mathrm{d}x$$.}} <!--SR:!2024-01-06,4,241!2024-01-11,11,281!2024-01-08,8,261-->
