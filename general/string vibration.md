...
> - {{[string](string%20(structure).md) properties: $T$ is [tension](tension%20(mechanics).md) and $mu$ is [linear mass density](linear%20density.md), i.e. [mass](mass.md) per unit [length](length.md)}}
> - {{[wave](wave.md) properties: $A$ is [amplitude](amplitude.md), $k$ is [wavenumber](wavenumber.md) or [spatial frequency](spatial%20frequency.md), $\omega$ is [angular frequency](angular%20frequency.md), $\phi$ is phase offset}}
