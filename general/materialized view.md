...
The objective is to {{find the best $S$ that maximizes the gain $G(S, \set{\top})$ over the top view, while satisfying the given constraints}}. The constraints we will consider here are {{the maximum number of materialized views $k$ or the max total cost of materialized views $X$}}. The constraints are considered independently.

The pseudocode is as follows:

1. initialization ::: $S \gets \set{\top}$
