...
> [!example] examples
>
> - empty function $\varnothing \to X$ :: It has no limits at every $x$.
> - function at an [isolated point](isolated%20point.md): $f(x \in \set{0}) = 0$ :: It has no limits at every $x$, including $0$.
...
The formal definitions for the above one-sided limits can be obtained by {{taking any of the above definition and replacing the implication antecedent $0 < \lvert x - p \rvert < \delta$ with respectively $0 < p - x < \delta$ and $0 < x - p < \delta$}}. Restated, the __limit of $f$ as $x$ approaches $p$ from below is $L$<!-- LaTeX separator -->__, if: {{For every real $\epsilon > 0$, there exists real $\delta > 0$ such that for all $x \in (a, b)$, $0 < p - x < \delta$ implies $\lvert f(x) - L \rvert < \epsilon$. $$(\forall \epsilon > 0)(\exists \delta > 0)(\forall x \in (a, b))(0 < p - x < \delta \implies \lvert f(x) - L \rvert < \epsilon)$$}}. The __limit of $f$ as $x$ approaches $p$ from above is $L$<!-- LaTeX separator -->__, if: {{For every real $\epsilon > 0$, there exists real $\delta > 0$ such that for all $x \in (a, b)$, $0 < x - p < \delta$ implies $\lvert f(x) - L \rvert < \epsilon$. $$(\forall \epsilon > 0)(\exists \delta > 0)(\forall x \in (a, b))(0 < x - p < \delta \implies \lvert f(x) - L \rvert < \epsilon)$$}}. For the third definition, {{the above replacement is unnecessary as the sequence $T$ can be selected such that it contains reals from one side only}}.
...
> [!example] examples
>
> - function at an [isolated point](isolated%20point.md): $f(x \in \set{0}) = 0$ :: It has no one-sided limits at every $x$, including $0$.

### examples

#### non-existence of one-sided limit(s)

The {{[Dirichlet function](Dirichlet%20function.md): $$f(x) = \begin{cases} 1 & x\text{ rational} \\ 0 & x\text{ irrational} \end{cases}$$}} has no limit at every $x$.
...
>
> - condition ::: limit on the right side of the equations exist or is in [determinate form](#determinate%20forms) <!--SR:!2024-12-23,291,330-->
> - addition ::: $\lim_{x \to p} (f(x) + g(x)) = \lim_{x \to p} f(x) + \lim_{x \to p} g(x)$ <!--SR:!2024-11-03,252,330-->
> - subtraction ::: $\lim_{x \to p} (f(x) - g(x)) = \lim_{x \to p} f(x) - \lim_{x \to p} g(x)$ <!--SR:!2024-09-26,222,330-->
> - multiplication ::: $\lim_{x \to p} (f(x) \cdot g(x)) = \lim_{x \to p} f(x) \cdot \lim_{x \to p} g(x)$ <!--SR:!2024-10-12,235,330-->
> - division ::: $\lim_{x \to p} (f(x) / g(x)) = \lim_{x \to p} f(x) / \lim_{x \to p} g(x)\qquad(\lim_{x \to p} g(x) \ne 0)$ <!--SR:!2024-05-19,116,290-->
> - exponentiation ::: $\lim_{x \to p} {f(x)}^{g(x)} = \lim_{x \to p} f(x)^{\lim_{x \to p} g(x)}\qquad(\lim_{x \to p} f(x) > 0 \text{ or } (\lim_{x \to p} f(x) = 0, 0 < \lim_{x \to p} g(x) < +\infty))$ <!--SR:!2024-07-07,129,250-->
