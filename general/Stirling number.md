...
- increment by binomial coefficient ::: $$\left[{n+1 \atop k+1}\right]=\sum _{j=k}^{n}\left[{n \atop j}\right]{\binom {j}{k} }$$ <p> vs. <p> $$\left\{ {n+1 \atop k+1}\right\}=\sum _{j=k}^{n}{\binom {n}{j} }\left\{ {j \atop k}\right\}$$
- increment by combinatorial argument ::: $$\left[{\begin{matrix}n+1\\k+1\end{matrix} }\right]=\sum _{j=k}^{n}{\frac {n!}{j!} }\left[{j \atop k}\right]$$ <p> vs. <p> $$\left\{ {n+1 \atop k+1}\right\}=\sum _{j=k}^{n}(k+1)^{n-j}\left\{ {j \atop k}\right\}$$
