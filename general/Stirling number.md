...
- increment with fixed _k_ by binomial coefficients ::: $$\left[{n+1 \atop k+1}\right]=\sum _{j=k}^{n}\left[{n \atop j}\right]{\binom {j}{k} }$$ <p> vs. <p> $$\left\{ {n+1 \atop k+1}\right\}=\sum _{j=k}^{n}{\binom {n}{j} }\left\{ {j \atop k}\right\}$$
- increment with fixed _k_ by combinatorial arguments ::: $$\left[{\begin{matrix}n+1\\k+1\end{matrix} }\right]=\sum _{j=k}^{n}{\frac {n!}{j!} }\left[{j \atop k}\right]$$ <p> vs. <p> $$\left\{ {n+1 \atop k+1}\right\}=\sum _{j=k}^{n}(k+1)^{n-j}\left\{ {j \atop k}\right\}$$
...
- partition then choose partitions ::: $$\left[{n \atop l+m}\right]{\binom {l+m}{l} }=\sum _{k}{\binom {n}{k} }\left[{k \atop l}\right]\left[{n-k \atop m}\right]$$ <p> vs. <p> $$\left\{ {n \atop l+m}\right\}{\binom {l+m}{l} }=\sum _{k}{\binom {n}{k} }\left\{ {k \atop l}\right\}\left\{ {n-k \atop m}\right\}$$
- asymptote for fixed _k_ ::: $$\left[{n+k \atop n}\right]{\underset {n\to \infty }{\sim } }{\frac {n^{2k} }{2^{k}k!} }.$$ <p> vs. <p> $$\left\{ {n+k \atop n}\right\}{\underset {n\to \infty }{\sim } }{\frac {n^{2k} }{2^{k}k!} }$$
