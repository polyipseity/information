...
A __bra__ is of the form {{$\langle f|$}}. Mathematically, it denotes {{a [linear form](linear%20form.md) $f: V \to \mathbb C$, i.e. a [linear map](linear%20map.md) that maps each vector in $V$ to a complex number $\mathbb C$}}. Letting the linear functional {{$\langle f|$ act on a vector $|v \rangle$, $f(\mathbf v)$}}, is written as {{$\langle f | v \rangle$}}. A simple case is {{in the finite-dimensional complex vector space $\mathbb C^n$}}, a bra can be identified with {{a [row vector](row%20and%20column%20vectors.md): $$\langle f| = \mathbf f = \begin{bmatrix} f_1 & \cdots & f_n \end{bmatrix}$$}}. Applying the linear functional $\langle f|$ on a vector $|v \rangle$ is then {{a [matrix multiplication](matrix%20multiplication.md): $$\langle f|v \rangle = \mathbf f \mathbf v = \begin{bmatrix} f_1 & \cdots & f_n \end{bmatrix} \begin{bmatrix} v_1 \\ \vdots \\ v_n \end{bmatrix} = \sum_{k = 1}^n f_k v_k$$}}. <!--SR:!2024-09-26,56,310!2024-08-05,17,290!2024-08-05,17,290!2024-09-26,54,310!2024-09-04,37,290!2024-10-01,59,310!2024-10-05,63,310-->
