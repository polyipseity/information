...
Using data on {@{United Airlines (UA) and Shell (S)}@}, suppose their {@{loadings on GDP ($b_{\text{GDP} }$) and inflation ($b_{\text{INF} }$)}@} are known.  To build {@{a portfolio that loads only on GDP}@}, solve {@{$$0.3\,w_{UA}+0.057\,w_S =1,\qquad-0.79\,w_{UA}-1.22\,w_S=0 .$$}@} The solution is {@{$w_{UA}=3.8$, $w_S=-2.46$}@}; {@{the short position in Shell}@} balances {@{the long position in United Airlines}@}.  {@{The residual weight on a risk‑free asset}@} is {@{$w_F=1-(w_{UA}+w_S)=-0.34$}@}, indicating that an investor would {@{borrow at the risk‑free rate to finance the factor portfolio}@}.
