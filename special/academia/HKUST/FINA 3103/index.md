...
- portfolio
  - portfolio / weight ::@:: A portfolio is specified by the set of assets held and their corresponding weight $w_i$. If _not leveraged_, $$w_i=\frac{Q_iP_i}{\sum_{j=1}^n Q_jP_j}$$ where $P_i$ is the price and $Q_i$ the quantity of asset $i$; weights sum to one. If an asset is omitted, its weight is zero. <p> If leveraged, multiply all weights by the leverage, so that the weights sum to the leverage.
