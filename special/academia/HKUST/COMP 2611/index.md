...
- report
  - cache set \(0\) ::@:: Almost did not know what a "cache set" is... <p> A cache can consists of one to many cache sets. Each cache set has a number of cache line, which is determined by its _associativity_. As a special case, a _fully-associative_ cache has exactly one cache set \(otherwise it is not fully-associative\).
