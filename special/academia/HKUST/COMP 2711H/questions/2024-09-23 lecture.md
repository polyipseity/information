...
What is the {{number of derangement of _n_ elements without using the inclusion–exclusion principle}}? <!--SR:!2024-11-06,16,290-->

- strategy ::: Derive a recurrence relation. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->
- solution ::: Let $d_n$ be the number of derangement of _n_ elements. As we have derived before, $d_n = (n - 1) (d_{n - 1} + d_{n - 2})$. The base cases are $d_0 = 1$ and $d_1 = 0$. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->

## derangement and fixed points (without the inclusion–exclusion principle)

What is the {{number of permutations of _n_ elements with exactly _k_ ≤ _n_ fixed points, i.e. exactly _k_ elements are mapped to their original positions}}? (You are allowed to use the number of derangements of _n_ elements, denoted $d_n$.) <!--SR:!2024-11-06,16,290-->

- strategy ::: Consider the fixed points. Then consider the situation after removing the fixed points. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->
- solution ::: Choose _k_ from _n_ elements to serve as the fixed points. Then the remaining _n_ − _k_ elements is a derangement to ensure there are no additional fixed points. So the answer is $$\binom n k d_{n - k} \,.$$ <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->

## number of onto functions (without the inclusion–exclusion principle)

How many {{onto functions (every element in the co-domain has at least one element in the domain mapped to it) are there from a domain of _n_ elements to a co-domain of _m_ elements}}? (No need to care about base cases.) <!--SR:!2024-11-06,16,290-->

- strategy ::: Without the inclusion–exclusion principle, we would usually try to find recurrence relations. When considering recurrence relations, we try to find how the additional or removal of an element would preserve the property we desire (in this case, onto functions). In this case, we can add/remove an element in the domain, in the co-domain, or reinterpret the problem. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->
- solution
...
How many paths {{do not go through any of the marked segments}}? How many paths {{go through exactly 2 of the marked segments}}? (A recurrence relation is accepted.) <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->

- strategy
  - strategy / no marked segments ::: We can assign the each point with a number. The number represents the number of paths from the starting point to that point. This number can be calculated recursively: the starting point has the number 1; all other points have the number that is the sum of the number of the point to the immediate left (if any) and the number of the point to the immediate bottom (if any). <p> If a marked segment is marked, then treat it as nonexistent when finding the point to the immediate left or bottom. <p> We can represent the number each point is assigned with using functions. For example, $a(i, j)$. We can also represent if a segment is marked using another function. Then use said functions to write the recurrence relation. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->
  - strategy / exactly 2 marked segments ::: Now the function also needs to consider the number of marked segments a path has passed through. That is, $a(i, j)$, which originally only represents the number of paths to $(i, j)$ without passing through any marked segments, needs to become $a(i, j, k)$, which represents the number of paths to $(i, j)$ passing through exactly $k$ marked segments. <p> Then we can use said functions to write the recurrence relation again. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->

## inclusion–exclusion principle: warmup questions

Trivial.

1. divisible by 5 ::: How many natural numbers not greater than 1000 are divisible by 5? <!--SR:!2024-11-06,16,290!2024-12-12,42,290-->
2. divisible by 2 ::: How many natural numbers not greater than 1000 are divisible by 2? <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->
3. divisible by both 2 and 5 ::: How many natural numbers not greater than 1000 are divisible by both 2 and 5? <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->
4. divisible by either 2 or 5 ::: How many natural numbers not greater than 1000 are divisible by either 2 or 5? <!--SR:!2024-12-05,35,290!2024-11-06,16,290-->
5. divisible by either 2, 5, or 7 ::: How many natural numbers not greater than 7000 are divisible by either 2, 5, or 7? <!--SR:!2024-12-05,35,290!2024-11-06,16,290-->

## inclusion–exclusion principle: proof

Prove the inclusion–exclusion principle (using binomial coefficients).

- strategy ::: Consider the membership of a fixed element, i.e. the sets an element belongs to. Show that the fixed element is counted once and once only by the inclusion–exclusion principle. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->

## derangement (with the inclusion–exclusion principle)

What is the {{number of derangement of _n_ elements without using the inclusion–exclusion principle}}? (A closed expression is required.) <!--SR:!2024-11-06,16,290-->

- strategy ::: Find ways to define sets from properties. <p> Let there be _n_ elements. Consider the set of all possible arrangements. Define _n_ subsets, $A_1, \ldots A_n$. An arrangement belongs to $A_i$ if the _i_-th element belongs to the _i_-th place. Then derangements are arrangements that belong to none of the subsets. The inclusion–exclusion principle can be applied to get a closed expression. <!--SR:!2024-11-06,16,290!2024-12-05,35,290-->
- number of derangements ::: This is known as a derangement. The number of derangements of _n_ distinct elements is also known as the subfactorial of _n_, !_n_, which is related to the factorial of _n_, _n_!, by $$!n = \begin{cases}1 \,, & n = 0 \\ \lfloor n! / e \rceil \,, & n > 0 && \lfloor * \rceil\text{ denotes the nearest integer} \,. \end{cases}$$ <p> The inclusion–exclusion principle can give us a closed expression that is a truncation of the Maclaurin series of $e^{-1}$. As _n_ approaches infinity, the expression approaches $e^{-1}$. This is why dividing by _e_ and rounding to the nearest integer works. <!--SR:!2024-11-06,16,290!2024-11-06,16,290-->

## number of onto functions (with the inclusion–exclusion principle)

How many {{onto functions (every element in the co-domain has at least one element in the domain mapped to it) are there from a domain of _n_ elements to a co-domain of _m_ elements}}? (A closed expression is required.) <!--SR:!2024-11-06,16,290-->

- strategy ::: With the inclusion–exclusion principle, we find ways to define sets from properties. <p> Let there be _m_ elements in the co-domain. Consider the set of all possible functions from the domain to the co-domain. Define _m_ subsets, $A_1, \ldots A_m$. A function belongs to $A_i$ if the _i_-th element in the co-domain is missed by the function (and thus the function is not onto). Then onto functions are functions that belong to none of the subsets. The inclusion–exclusion principle can be applied to get a closed expression. <!--SR:!2024-11-06,16,290!2024-12-05,35,290-->
