...
We have a country with $n \ge 1$ cities. There is {@{exactly one one-way road between any two cities}@}. A city $c$ is central if {@{every other city $c′$ can either directly reach $c$ or with 1 stop}@}. Prove that {@{there is alway at least one central city}@}.
