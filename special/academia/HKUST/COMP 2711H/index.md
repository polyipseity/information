...
- theorem: directed simple graph, cycle, and strongly connected component ::: A directed simple graph (no self-loops) has no cycles iff every vertex is a strongly connected component by itself. <p> To prove this and its converse, use contradiction.
- graph theory
...
- theorem: directed acyclic graph and topological ordering ::: A directed graph is a directed acyclic graph (DAG) iff the graph has a topological ordering. <p> To prove this and is converse, use contradiction. Two lemmas below and depth-first search (DFS) are useful here.
  ...
  - lemma: directed graph and minimum in-degree ::: If the in-degree of every vertex of a directed graph is 1, the there is a directed cycle. <p> To prove this, we can keep moving to new nodes if there are no directed cycles.
...
- theorem: tree, leaf, and center ::: No leaf can be a center of a tree with 3 or more vertices. <p> To prove this, suppose a leaf is a center. We can write the leaf's only neighbor's distance to any other vertex in terms of the distance of the leaf to any other vertex. This would produce a contradiction.
- theorem: tree and center ::: A tree either has one center or two neighboring centers. <p> This can be proven using strong induction. Prove the base case for _n_ = 1 and _n_ = 2. Then realize that any tree can be transformed into a smaller one by removing all of its leaves so that we can apply the induction hypothesis. After applying the hypothesis, re-adding the leaves preserves the centers.
...
- graph theory
  ...
  - graph theory / maximal matching ::: A __maximal matching__ of a graph is a matching with the maximum number of edges.
  - graph theory / alternating walk, alternating path ::: An __alternating walk__ of a matching is a walk that alternates between edges in the matching and not in the matching. An __alternating path__ is defined analogously.
...
- theorem: minimal vertex cover and maximal matching ::: In every graph, the minimal vertex cover has a size of at least that of maximal matching. <p> To prove this, consider a maximal matching. For every edge in the maximal matching, one point must be in the vertex cover.
- theorem: minimal vertex cover, maximal matching, and bipartite graph ::: In every bipartite graph, the minimal vertex cover has a size equals that of maximal matching. <p> To prove this, we can prove the minimal vertex cover has a size smaller or equal to that of maximal matching. Then combining with the previous theorem gives us the result.
