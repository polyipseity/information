...
- why the two pseudoinverse formulas are equivalent ::@:: Let $A_u=\{y:F(y)<u\}$, $B_u=\{x:F(x)\ge u\}$, and $s=\sup A_u$. Completeness of $\mathbb{R}$ gives the threshold $s$. If $x<s$, some $y\in A_u$ lies to its right, so monotonicity gives $F(x)<u$ and hence $x\notin B_u$. If $x>s$, then $x\notin B_u$ would force $x\in A_u$, contradicting $x>s=\sup A_u$. At $x=s$, right-continuity rules out $F(s)<u$: otherwise points just to the right of $s$ would still satisfy $F<u$, contradicting that $s$ is the supremum. Therefore $B_u=[s,\infty)$ and $\inf B_u=s=\sup A_u$. <!--SR:!2026-05-07,11,316!2026-05-08,12,316-->
