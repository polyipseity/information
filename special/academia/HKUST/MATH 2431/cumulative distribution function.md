...
- why the two pseudoinverse formulas are equivalent ::@:: To prove $\inf\{x:F(x)\ge u\}=\sup\{y:F(y)<u\}$, let $A_u=\{y:F(y)<u\}$, $B_u=\{x:F(x)\ge u\}$, and $s=\sup A_u$. The goal is to show $B_u=[s,\infty)$. If $x<s$, choose $y\in A_u$ with $x<y\le s$; monotonicity gives $F(x)\le F(y)<u$, so $x\notin B_u$. If $x>s$ and $x\notin B_u$, then $F(x)<u$, so $x\in A_u$, contradicting $x>\sup A_u$. Finally, if $F(s)<u$, right-continuity would force points just to the right of $s$ to still satisfy $F<u$, again contradicting that $s$ is the supremum of $A_u$. Hence $B_u=[s,\infty)$, so $\inf B_u=s=\sup A_u$. <!--SR:!2026-06-13,36,316!2026-06-14,37,316-->
