...
- Lebesgue-Stieltjes theorem (unified statement) ::@:: A function $F\colon\mathbb{R}\to\mathbb{R}$ is the cumulative distribution function of a unique probability measure on $(\mathbb{R},\mathcal{B}(\mathbb{R}))$ if and only if $F$ is nondecreasing, right-continuous, with $\lim_{x\to-\infty}F(x)=0$ and $\lim_{x\to\infty}F(x)=1$. In other words, these four structural properties are not merely necessary; together they completely characterize real-valued probability laws. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
- why uniqueness holds via half-lines ::@:: If two probability measures have the same cumulative distribution function, then they agree on every left half-line $(-\infty,b]$ because both assign that set probability $F(b)$. Those half-lines form a pi-system generating $\mathcal{B}(\mathbb{R})$, so the pi-lambda theorem extends equality from that generator to all Borel sets. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
- why the theorem really inverts the basic CDF properties ::@:: The forward direction says every probability measure produces a function with the four standard CDF properties. The converse says the process can be reversed: starting from any function with exactly those properties, one can reconstruct a unique probability measure. That is why the theorem is the full probability-measure/CDF correspondence rather than just a one-way checklist. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
...
- concrete construction from a uniform input ::@:: Start with $U\sim\mathrm{U}(0,1)$ and define $X=F^{-1}(U)$. The goal is then to prove that this explicitly constructed random variable has cumulative distribution function $F$, while uniqueness is handled separately by the half-line generator argument. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
- core equivalence in the concrete proof ::@:: The heart of the construction is $F^{-1}(u)\le x \iff u\le F(x)$. The left side asks whether the generalized inverse sends the uniform level $u$ to a point at or before $x$, while the right side asks whether the graph of $F$ has already reached height $u$ by the time one gets to $x$. These are the same event viewed from the $x$-axis and the $u$-axis respectively. <!--SR:!2026-04-12,4,316!2026-04-12,4,316--> Rigour comes from the order-completeness of $\mathbb{R}$, monotonicity, and right-continuity at the threshold.
- why the constructed variable has cumulative distribution function $F$ ::@:: If $X=F^{-1}(U)$, then $\{X\le x\}=\{F^{-1}(U)\le x\}=\{U\le F(x)\}$. Because $U$ is uniform on $(0,1)$, the probability of that event is exactly the length of $(0,F(x)]\cap(0,1)$, namely $F(x)$. Therefore $P[X\le x]=F(x)$ for every $x$. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
- why the proof is both constructive and unique ::@:: Existence is constructive because the proof literally builds $X=F^{-1}(U)$. Uniqueness is separate and measure-theoretic: if two probability measures share the same cumulative distribution function, then they agree on all left half-lines, and the pi-lambda theorem extends that agreement to all Borel sets. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
- why the concrete proof is also a simulation algorithm ::@:: The proof does not merely show existence abstractly; it tells you how to sample from the law. If you can generate $U\sim\mathrm{U}(0,1)$, then applying the generalized inverse produces $X=F^{-1}(U)$ with the desired distribution. That is exactly inverse-transform sampling. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
- why the uniform law is the only external input ::@:: Once uniform measure on $(0,1)$ is available, every one-dimensional real-valued probability law can be generated through a suitable generalized inverse. In that sense the uniform law is the basic raw material from which the Lebesgue-Stieltjes construction manufactures all real-line distributions. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
- why $\mathrm{U}(0,1)$ and $\mathrm{U}([0,1])$ are interchangeable here ::@:: The only difference between $(0,1)$ and $[0,1]$ lies at the endpoints, and continuous probability laws assign probability $0$ to singletons. So those endpoints do not affect the inverse-transform construction in any essential probabilistic way. <!--SR:!2026-04-12,4,316!2026-04-12,4,316-->
