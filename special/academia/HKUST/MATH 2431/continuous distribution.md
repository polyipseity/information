...
- why the minibus examples motivate densities ::@:: Both minibus models follow the same two-step idea: first assign probabilities to intervals in a fine partition, then let the mesh size go to $0$. In that limit the discrete weighted sum becomes an integral $\int f(x)\,dx$, and the function $f$ records how those interval weights were distributed. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
...
- definition via a density ::@:: A continuous distribution on $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$ is a probability measure characterized by a density $f$, so probabilities are computed by integrating $f$ over Borel sets. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
