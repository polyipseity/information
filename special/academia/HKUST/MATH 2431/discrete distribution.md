...
- on finite $\Omega$, $p(\omega)=1/|\Omega|$ for every $\omega\in\Omega$ ::@:: This is the discrete uniform distribution on $\Omega$; it models a finite experiment in which every outcome is equally likely. <!--SR:!2026-05-15,16,317!2026-05-14,15,317-->
- discrete uniform interpretation ::@:: The discrete uniform distribution models a finite experiment in which all outcomes are symmetric or equally likely, such as a fair die roll or a uniformly chosen element of a finite set. <!--SR:!2026-05-17,18,330!2026-05-18,17,302-->
- why $p(\omega)=1/|\Omega|$ on finite $\Omega$ ::@:: In a discrete uniform distribution, symmetry makes all point masses equal and normalization forces those $|\Omega|$ equal masses to add up to $1$. <!--SR:!2026-05-17,16,317!2026-05-19,18,330-->
...
- on $\{0,1\}$, $p(1)=p$ and $p(0)=1-p$ ::@:: This is the Bernoulli distribution $\mathrm{Ber}(p)$; it models a single success/failure experiment. <!--SR:!2026-05-19,18,330!2026-05-15,16,317-->
- Bernoulli interpretation ::@:: A Bernoulli random quantity records one yes/no or success/failure outcome, so it is the natural model for a single trial or for the indicator of an event. <!--SR:!2026-05-15,16,317!2026-05-12,17,317-->
- $p(1)=p$ and $p(0)=1-p$ on $\{0,1\}$ ::@:: This PMF has that form because there are only two outcomes: once success is assigned probability $p$, normalization leaves exactly $1-p$ for failure. <!--SR:!2026-05-19,18,330!2026-05-17,18,330-->
...
- on $\{0,1,\ldots,n\}$, $p(k)=\binom{n}{k}p^k(1-p)^{n-k}$ ::@:: This is the binomial distribution $\mathrm{Bin}(n,p)$; it counts how many successes occur in $n$ independent Bernoulli trials with success probability $p$. <!--SR:!2026-05-09,14,290!2026-05-15,16,317-->
- $X\sim \mathrm{Bin}(n,p)$ ::@:: This means $X$ records the total number of successes in $n$ independent Bernoulli trials, so it forgets the order of the trials and remembers only the success count. <!--SR:!2026-05-18,17,317!2026-05-18,17,310-->
- on $\{0,1\}^n$, $Q[(\omega_1,\ldots,\omega_n)]=p^{\sum_j \omega_j}(1-p)^{n-\sum_j \omega_j}$ ::@:: This is the ordered Bernoulli-trial model underlying the binomial law: each string records the full success/failure pattern, and grouping strings by total successes produces $\mathrm{Bin}(n,p)$. <!--SR:!2026-05-15,16,317!2026-05-14,15,317-->
- $E_k=\{(\omega_1,\ldots,\omega_n):\sum_{j=1}^n \omega_j=k\}$ and $|E_k|=\binom{n}{k}$ ::@:: Therefore $Q[E_k]=\binom{n}{k}p^k(1-p)^{n-k}$; there are $\binom{n}{k}$ ordered patterns with exactly $k$ successes. <!--SR:!2026-05-11,16,317!2026-05-16,17,317-->
- $\binom{n}{k}p^k(1-p)^{n-k}$ ::@:: This binomial PMF equals "number of success/failure patterns with exactly $k$ successes" times "probability of one such pattern": there are $\binom{n}{k}$ such patterns, and each has probability $p^k(1-p)^{n-k}$. <!--SR:!2026-05-12,17,317!2026-05-17,18,330-->
- if $X$ is the number of sixes in four throws of a fair die ::@:: Then $X\sim \mathrm{Bin}(4,1/6)$, because each throw is a Bernoulli experiment with success = "a six appears". <!--SR:!2026-05-14,15,317!2026-05-14,15,317-->
- $\{0,1,\ldots,n\}$ versus $\{0,1\}^n$ when only the success count matters ::@:: The smaller state space is more efficient than the full pattern space, and this kind of reduction in complexity motivates the later use of random variables. <!--SR:!2026-05-18,17,310!2026-05-17,18,330-->
...
- $P[X=k]=(1-p)^k p$ for $k\in\mathbb{N}_0$ ::@:: This is the geometric convention supported on $\mathbb{N}_0$; it counts the number of failures before the first success. <!--SR:!2026-05-19,18,330!2026-05-14,15,302-->
- $P[Y=k]=(1-p)^{k-1}p$ for $k\in\mathbb{N}$ ::@:: This is the geometric convention supported on $\mathbb{N}$; it counts the trial number of the first success. <!--SR:!2026-05-15,16,310!2026-05-14,15,302-->
- $X\in\mathbb{N}_0$ counts failures before first success, while $Y\in\mathbb{N}$ counts the first-success trial ::@:: These are the two common geometric interpretations; they describe the same experiment using different counting conventions. <!--SR:!2026-05-16,15,317!2026-05-18,17,317-->
- why $(1-p)^k p$ on $\mathbb{N}_0$ and $(1-p)^{k-1}p$ on $\mathbb{N}$ ::@:: In either geometric convention, the PMF comes from a run of failures followed by the first success: $(1-p)^k p$ corresponds to $k$ failures followed by success, and $(1-p)^{k-1}p$ corresponds to $k-1$ failures followed by success on trial $k$. <!--SR:!2026-05-14,15,317!2026-05-14,15,317-->
- $Y=X+1$ ::@:: If $X$ counts failures before the first success and $Y$ counts the trial number of the first success, then the two geometric conventions differ only by this shift of $1$. <!--SR:!2026-05-13,18,330!2026-05-17,18,330-->
- $\mathrm{Geo}(p)$ with support $\mathbb{N}_0$ versus support $\mathbb{N}$ ::@:: Different books use this notation for different geometric conventions, so one must check whether it counts failures before the first success or the trial number of the first success. <!--SR:!2026-05-13,18,330!2026-05-18,17,317-->
...
- $p(k)=\dfrac{\binom{M}{k}\binom{N-M}{n-k}}{\binom{N}{n}}$ on $\{0,1,\ldots,n\}$ ::@:: This is the hypergeometric distribution $\mathrm{H}(N,M,n)$; it gives the probability of drawing exactly $k$ favorable items when $n$ items are sampled uniformly without replacement from a population of size $N$ containing $M$ favorable ones. <!--SR:!2026-05-17,18,330!2026-05-16,17,317-->
- $X\sim \mathrm{H}(N,M,n)$ ::@:: This means $X$ records how many favorable items appear in a sample of size $n$ drawn uniformly without replacement from a finite population. <!--SR:!2026-05-17,18,330!2026-05-16,15,310-->
- $\dfrac{\binom{M}{k}\binom{N-M}{n-k}}{\binom{N}{n}}$ ::@:: This hypergeometric probability is a counting ratio: the numerator counts samples with exactly $k$ favorable and $n-k$ unfavorable items, and the denominator counts all samples of size $n$. <!--SR:!2026-05-12,17,310!2026-05-16,17,317-->
- why $p(k)=\dfrac{\binom{M}{k}\binom{N-M}{n-k}}{\binom{N}{n}}$ in the hypergeometric model ::@:: The PMF is a counting ratio: choose the favorable and unfavorable items separately to count admissible samples, then divide by the total number of samples of size $n$. <!--SR:!2026-05-19,18,330!2026-05-13,14,290-->
- hypergeometric approximation
  - hypergeometric approximation / statement ::@:: If $n$ is fixed, $M_N/N\to p$, and $X_N\sim \mathrm{H}(N,M_N,n)$, then for each fixed $k\in\{0,\ldots,n\}$ one has $P[X_N=k]\to \binom{n}{k}p^k(1-p)^{n-k}$; a large-population hypergeometric law approaches $\mathrm{Bin}(n,p)$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-18,17,317!2026-05-18,17,310-->
  - hypergeometric approximation / intuition ::@:: (1) Sampling without replacement creates dependence between draws. <br/> (1.1) But if $N$ is huge and $n$ is fixed, the favorable fraction changes only slightly: $M_N/N\approx p$, while after one removal it is still about $(M_N-1)/(N-1)\approx p$. <br/> (1.2) So the successive draw probabilities are all close to $p$ and the count of favorable draws should resemble $\mathrm{Bin}(n,p)$. <br/> (2) Therefore $P[X_N=k]$ should be close to $\binom{n}{k}p^k(1-p)^{n-k}$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-15,16,310!2026-05-15,16,302-->
  - hypergeometric approximation / derivation strategy ::@:: (1) Start from $P[X_N=k]=\dfrac{\binom{M_N}{k}\binom{N-M_N}{n-k}}{\binom{N}{n}}$. <br/> (1.1) Rewrite it as $\binom{n}{k}\dfrac{(M_N)_k(N-M_N)_{n-k}}{(N)_n}$. <br/> (1.2) Then split it into the products $\prod_{j=0}^{k-1}\dfrac{M_N-j}{N-j}$ and $\prod_{j=0}^{n-k-1}\dfrac{N-M_N-j}{N-k-j}$. <br/> (2) Since $n,k$ are fixed and $M_N/N\to p$, these products tend to $p^k$ and $(1-p)^{n-k}$, giving the binomial PMF. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-12,17,317!2026-05-14,15,317-->
  - hypergeometric approximation / exact factorization ::@:: For $X_N\sim \mathrm{H}(N,M_N,n)$, $P[X_N=k]=\binom{n}{k}\dfrac{(M_N)_k(N-M_N)_{n-k}}{(N)_n}=\binom{n}{k}\left(\prod_{j=0}^{k-1}\dfrac{M_N-j}{N-j}\right)\left(\prod_{j=0}^{n-k-1}\dfrac{N-M_N-j}{N-k-j}\right)$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-10,15,310!2026-05-16,17,317-->
  - hypergeometric approximation / factor limits ::@:: If $M_N/N\to p$ and $n,k$ are fixed, then $\prod_{j=0}^{k-1}\dfrac{M_N-j}{N-j}\to p^k$ and $\prod_{j=0}^{n-k-1}\dfrac{N-M_N-j}{N-k-j}\to (1-p)^{n-k}$. The limits may be derived by dividing each numerator and denominator by $N$ enough times. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-14,15,302!2026-05-18,17,317-->
  - hypergeometric approximation / full proof ::@:: Combine the exact factorization of $P[X_N=k]$ with the factor limits to obtain $P[X_N=k]\to \binom{n}{k}p^k(1-p)^{n-k}$, which is the PMF of $\mathrm{Bin}(n,p)$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-17,18,330!2026-05-17,18,330-->
...
Now carry this out exactly and keep track of which limits are being taken. For fixed $k$, $\binom{n}{k}p_n^k(1-p_n)^{n-k}=\frac{n!}{k!(n-k)!}\left(\frac{\lambda}{n}\right)^k\left(1-\frac{\lambda}{n}\right)^{n-k}$. Rewrite the binomial coefficient as $\binom{n}{k}=\frac{n(n-1)\cdots(n-k+1)}{k!}=\frac{n^k}{k!}\prod_{j=0}^{k-1}\left(1-\frac{j}{n}\right)$. Substituting this into the binomial mass yields $\binom{n}{k}p_n^k(1-p_n)^{n-k}=\frac{\lambda^k}{k!}\left(\prod_{j=0}^{k-1}\left(1-\frac{j}{n}\right)\right)\left(1-\frac{\lambda}{n}\right)^n\left(1-\frac{\lambda}{n}\right)^{-k}$. The point of this factorization is that it splits the limit into one constant factor $\lambda^k/k!$ and three separate $n\to\infty$ limits.

For the finite product, note that $k$ is fixed, so there are only finitely many factors. For each fixed $j\in\{0,1,\ldots,k-1\}$, we have $1-j/n\to 1$, hence $\prod_{j=0}^{k-1}(1-j/n)\to 1$. If one wants a slightly more quantitative proof, write $\log\prod_{j=0}^{k-1}(1-j/n)=\sum_{j=0}^{k-1}\log(1-j/n)$; since each $j/n\to 0$ and $\log(1-u)\sim -u$ as $u\to 0$, this sum is asymptotic to $-\sum_{j=0}^{k-1}j/n=O(1/n)\to 0$, so the product tends to $e^0=1$.

For the failure block, use the standard exponential limit $\left(1-\frac{\lambda}{n}\right)^n\to e^{-\lambda}$. One way to justify it is to take logs: $\log\left(\left(1-\frac{\lambda}{n}\right)^n\right)=n\log\left(1-\frac{\lambda}{n}\right)\to -\lambda$, again because $\log(1-u)\sim -u$ as $u\to 0$. Exponentiating gives the desired limit $e^{-\lambda}$. Finally, because $1-\lambda/n\to 1$ and the exponent $-k$ is fixed, we also have $\left(1-\frac{\lambda}{n}\right)^{-k}\to 1$.

Now apply the product rule for limits: each of the three $n$-dependent factors has a finite limit, so the product tends to the product of the limits. Therefore $\lim_{n\to\infty}\binom{n}{k}p_n^k(1-p_n)^{n-k}=\frac{\lambda^k}{k!}\cdot 1\cdot e^{-\lambda}\cdot 1=\frac{\lambda^k}{k!}e^{-\lambda}$. So a binomial model with many trials and very small success probability but fixed mean $np_n=\lambda$ converges pointwise to the Poisson law.
...
- on $\mathbb{N}_0$, $p(k)=\dfrac{\lambda^k}{k!}e^{-\lambda}$ for $\lambda>0$ ::@:: This is the Poisson distribution $\mathrm{Pois}(\lambda)$; it models counts that can in principle be arbitrarily large. <!--SR:!2026-05-15,16,310!2026-05-18,17,317-->
- $X\sim \mathrm{Pois}(\lambda)$ ::@:: This is the natural model for count data such as goals in a football match or raindrops in a region during a time interval, where there is no fixed upper bound on the number of events. <!--SR:!2026-05-10,15,290!2026-05-17,18,330-->
- $\dfrac{\lambda^k}{k!}e^{-\lambda}$ ::@:: In the Poisson PMF, the factor $\lambda^k/k!$ weights the count at value $k$ when the typical count level is $\lambda$, and the factor $e^{-\lambda}$ is the normalization constant that makes the total mass equal to $1$. <!--SR:!2026-05-18,17,317!2026-05-18,17,317-->
- $e^{\lambda}=\sum_{k=0}^{\infty}\lambda^k/k!$ ::@:: This exponential power series explains the factor $e^{-\lambda}$ in the Poisson PMF, because multiplying $\lambda^k/k!$ by $e^{-\lambda}$ makes the total probability sum to $e^{-\lambda}e^{\lambda}=1$. <!--SR:!2026-05-14,15,317!2026-05-15,16,317-->
- Poisson approximation
  - Poisson approximation / statement ::@:: If $p_n=\lambda/n$ and $k$ is fixed, then $\binom{n}{k}p_n^k(1-p_n)^{n-k}\to\dfrac{\lambda^k}{k!}e^{-\lambda}$ as $n\to\infty$; a rare-event binomial law with fixed mean converges to $\mathrm{Pois}(\lambda)$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-19,18,330!2026-05-09,14,290-->
  - Poisson approximation / intuition ::@:: (1) Set $p_n=\lambda/n$, so each trial is rare but the mean stays fixed: $np_n=\lambda$. <br/> (1.1) For fixed $k$, the success part behaves like $\binom{n}{k}(\lambda/n)^k\approx \lambda^k/k!$. <br/> (1.2) The failure block behaves like $(1-\lambda/n)^n\to e^{-\lambda}$. <br/> (2) So $\binom{n}{k}p_n^k(1-p_n)^{n-k}$ should approach $\dfrac{\lambda^k}{k!}e^{-\lambda}$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-18,17,302!2026-05-16,17,302-->
  - Poisson approximation / derivation strategy ::@:: (1) Start from $\binom{n}{k}(\lambda/n)^k(1-\lambda/n)^{n-k}$ with $k$ fixed and $n\to\infty$. <br/> (1.1) Rewrite $\binom{n}{k}$ as $\dfrac{n^k}{k!}\prod_{j=0}^{k-1}(1-j/n)$ so that the whole expression becomes $\dfrac{\lambda^k}{k!}\left(\prod_{j=0}^{k-1}(1-j/n)\right)(1-\lambda/n)^n(1-\lambda/n)^{-k}$. <br/> (1.2) Then take the three limits separately: the finite product tends to $1$, the failure block tends to $e^{-\lambda}$, and the correction factor tends to $1$. <br/> (2) Multiply the limits to recover the Poisson PMF. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-16,17,310!2026-05-13,18,330-->
  - Poisson approximation / exact factorization ::@:: For fixed $k$ and $p_n=\lambda/n$, first rewrite $\binom{n}{k}=\dfrac{n(n-1)\cdots(n-k+1)}{k!}=\dfrac{n^k}{k!}\prod_{j=0}^{k-1}\left(1-\dfrac{j}{n}\right)$. Substituting into the binomial mass gives $\binom{n}{k}p_n^k(1-p_n)^{n-k}=\dfrac{\lambda^k}{k!}\left(\prod_{j=0}^{k-1}\left(1-\dfrac{j}{n}\right)\right)\left(1-\dfrac{\lambda}{n}\right)^n\left(1-\dfrac{\lambda}{n}\right)^{-k}$, which exposes exactly the factors whose limits must be computed. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-17,18,330!2026-05-11,10,277-->
  - Poisson approximation / why the factor limits hold ::@:: In the Poisson approximation proof with fixed $k$, the finite product $\prod_{j=0}^{k-1}(1-j/n)$ tends to $1$ because each factor tends to $1$ and there are only finitely many of them; more quantitatively, $\log\prod_{j=0}^{k-1}(1-j/n)=\sum_{j=0}^{k-1}\log(1-j/n)\sim -\sum_{j=0}^{k-1}j/n\to 0$. Also, $(1-\lambda/n)^n\to e^{-\lambda}$ because $n\log(1-\lambda/n)\to -\lambda$, and $(1-\lambda/n)^{-k}\to 1$ because the base tends to $1$ while the exponent $-k$ is fixed. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-17,16,317!2026-05-14,15,317-->
  - Poisson approximation / full proof ::@:: Rewrite the binomial mass into the factorized form $\dfrac{\lambda^k}{k!}\left(\prod_{j=0}^{k-1}(1-j/n)\right)(1-\lambda/n)^n(1-\lambda/n)^{-k}$, compute the three limits separately, and then apply the product rule for limits. This gives $\binom{n}{k}p_n^k(1-p_n)^{n-k}\to\dfrac{\lambda^k}{k!}e^{-\lambda}$, which is the full proof of the Poisson approximation. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-18,17,317!2026-05-13,18,330-->
  - Poisson process / small-time ODE derivation ::@:: For $P_n(t)=P(N_t=n)$, small-time assumptions give $P_0'(t)=-\lambda P_0(t)$ and, for $n\ge1$, $P_n'(t)=-\lambda P_n(t)+\lambda P_{n-1}(t)$ with $P_0(0)=1$, $P_n(0)=0$; solving yields $P(N_t=n)=e^{-\lambda t}(\lambda t)^n/n!$. <p> The mathematical expressions may be interpreted as a continuous-time Markov chain with transition rates $\lambda$. The initial conditions say that the process starts at $0$, and the ODEs say that the process jumps from $n$ to $n+1$ at rate $\lambda$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-05-16,17,317!2026-05-17,16,302-->
