...
A discrete distribution is a probability measure on a countable sample space. In that setting every event probability is assembled from the masses of single outcomes, so one can pass back and forth between the measure $P$ and its probability mass function $p(\omega) = P[\{\omega\}]$. The named laws in this chapter—uniform, Bernoulli, binomial, geometric, hypergeometric, and Poisson—are all special cases of this countable-space framework.

---

Flashcards for this section are as follows:

- overview ::@:: A discrete distribution is a probability measure on a countable sample space; it is completely described by the point masses $p(\omega)=P[\{\omega\}]$, and the main named examples are the discrete uniform, Bernoulli, binomial, geometric, hypergeometric, and Poisson laws. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
...
- measure data equals pmf data ::@:: On a countable sample space $\Omega$, specifying a discrete distribution $P$ and specifying a function $p\colon\Omega\to[0,1]$ with $\sum_{\omega\in\Omega} p(\omega)=1$ are equivalent, because each one uniquely determines the other. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
