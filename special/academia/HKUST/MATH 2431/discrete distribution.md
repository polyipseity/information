...
- definition on a countable sample space ::@:: A discrete distribution is a probability measure on $(\Omega,\mathcal{P}(\Omega))$ when the sample space $\Omega$ is countable. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-04-12,4,310!2026-04-12,4,310-->
...
- Poisson approximation
  ...
  - Poisson approximation / derivation strategy ::@:: (1) Start from $\binom{n}{k}(\lambda/n)^k(1-\lambda/n)^{n-k}$ with $k$ fixed and $n\to\infty$. <br/> (1.1) Rewrite $\binom{n}{k}$ as $\dfrac{n^k}{k!}\prod_{j=0}^{k-1}(1-j/n)$ so that the whole expression becomes $\dfrac{\lambda^k}{k!}\left(\prod_{j=0}^{k-1}(1-j/n)\right)(1-\lambda/n)^n(1-\lambda/n)^{-k}$. <br/> (1.2) Then take the three limits separately: the finite product tends to $1$, the failure block tends to $e^{-\lambda}$, and the correction factor tends to $1$. <br/> (2) Multiply the limits to recover the Poisson PMF. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-04-12,4,290!2026-04-12,4,310-->
  - Poisson approximation / exact factorization ::@:: For fixed $k$ and $p_n=\lambda/n$, first rewrite $\binom{n}{k}=\dfrac{n(n-1)\cdots(n-k+1)}{k!}=\dfrac{n^k}{k!}\prod_{j=0}^{k-1}\left(1-\dfrac{j}{n}\right)$. Substituting into the binomial mass gives $\binom{n}{k}p_n^k(1-p_n)^{n-k}=\dfrac{\lambda^k}{k!}\left(\prod_{j=0}^{k-1}\left(1-\dfrac{j}{n}\right)\right)\left(1-\dfrac{\lambda}{n}\right)^n\left(1-\dfrac{\lambda}{n}\right)^{-k}$, which exposes exactly the factors whose limits must be computed. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-04-12,4,310!2026-04-11,3,257-->
  - Poisson approximation / why the factor limits hold ::@:: In the Poisson approximation proof with fixed $k$, the finite product $\prod_{j=0}^{k-1}(1-j/n)$ tends to $1$ because each factor tends to $1$ and there are only finitely many of them; more quantitatively, $\log\prod_{j=0}^{k-1}(1-j/n)=\sum_{j=0}^{k-1}\log(1-j/n)\sim -\sum_{j=0}^{k-1}j/n\to 0$. Also, $(1-\lambda/n)^n\to e^{-\lambda}$ because $n\log(1-\lambda/n)\to -\lambda$, and $(1-\lambda/n)^{-k}\to 1$ because the base tends to $1$ while the exponent $-k$ is fixed. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-04-12,4,297!2026-04-12,4,297-->
  - Poisson approximation / full proof ::@:: Rewrite the binomial mass into the factorized form $\dfrac{\lambda^k}{k!}\left(\prod_{j=0}^{k-1}(1-j/n)\right)(1-\lambda/n)^n(1-\lambda/n)^{-k}$, compute the three limits separately, and then apply the product rule for limits. This gives $\binom{n}{k}p_n^k(1-p_n)^{n-k}\to\dfrac{\lambda^k}{k!}e^{-\lambda}$, which is the full proof of the Poisson approximation. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-04-12,4,297!2026-04-12,4,310-->
  - Poisson process / small-time ODE derivation ::@:: For $P_n(t)=P(N_t=n)$, small-time assumptions give $P_0'(t)=-\lambda P_0(t)$ and, for $n\ge1$, $P_n'(t)=-\lambda P_n(t)+\lambda P_{n-1}(t)$ with $P_0(0)=1$, $P_n(0)=0$; solving yields $P(N_t=n)=e^{-\lambda t}(\lambda t)^n/n!$. <p> The mathematical expressions may be interpreted as a continuous-time Markov chain with transition rates $\lambda$. The initial conditions say that the process starts at $0$, and the ODEs say that the process jumps from $n$ to $n+1$ at rate $\lambda$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual --> <!--SR:!2026-04-12,4,297!2026-04-12,4,282-->
