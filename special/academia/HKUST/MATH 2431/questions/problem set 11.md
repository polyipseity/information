...
>
> __$X_n$.__ For $x<0$, {@{$F_{X_n}(x)=0$}@}. For $0\le x<n/\lambda$, integrate using {@{the antiderivative $\frac{d}{dt}[-(1-\lambda t/n)^n]=\lambda(1-\lambda t/n)^{n-1}$}@}: {@{$$F_{X_n}(x)=\int_0^x\lambda\Bigl(1-\frac{\lambda t}{n}\Bigr)^{n-1}\!dt=\Bigl[-\Bigl(1-\frac{\lambda t}{n}\Bigr)^n\Bigr]_{t=0}^{t=x}=1-\Bigl(1-\frac{\lambda x}{n}\Bigr)^n.$$}@} For {@{$x\ge n/\lambda$, $F_{X_n}(x)=1$}@}. As {@{$n\to\infty$, $(1-\lambda x/n)^n\to e^{-\lambda x}$}@}, so {@{$$F_{X_n}(x)\to\bigl(1-e^{-\lambda x}\bigr)\mathbf 1_{[0,\infty)}(x),$$}@} the CDF of {@{an $E(\lambda)$ distribution}@}. Hence {@{$X_n\xrightarrow{d}E(\lambda)$}@}.
>
> __$Y_n$.__ For $x<0$, {@{$F_{Y_n}(x)=0$}@}. For {@{$0\le x<1$, $$F_{Y_n}(x)=\int_0^x\frac{n+1}{n}t^{1/n}\,dt=\Bigl[t^{(n+1)/n}\Bigr]_{t=0}^{t=x}=x^{(n+1)/n}.$$}@} For {@{$x\ge1$, $F_{Y_n}(x)=1$}@}. Write {@{$x^{(n+1)/n}=x\cdot x^{1/n}$}@}. Since {@{$x^{1/n}\to1$ for $x>0$, $$F_{Y_n}(x)\to\begin{cases}0,&x<0,\\ x,&0\le x<1,\\ 1,&x\ge1,\end{cases}$$}@} the CDF of {@{a $U([0,1])$ distribution}@}. Hence {@{$Y_n\xrightarrow{d}U([0,1])$}@}.
