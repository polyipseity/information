...
- independence / definition two events ::@:: Events $A,B$ are (stochastically) independent if $P[A \cap B] = P[A]\,P[B]$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
- independence / pairwise independence ::@:: A family $(A_i)_{i\in I}$ is pairwise independent if every distinct pair $(A_i,A_j)$ with $i\neq j$ satisfies $P[A_i\cap A_j]=P[A_i]P[A_j]$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
- independence / joint independence ::@:: A family $(A_i)_{i\in I}$ is jointly independent if for every finite subset $\{i_1,\ldots,i_m\}$ one has $P[A_{i_1}\cap\cdots\cap A_{i_m}] = \prod_{\ell=1}^m P[A_{i_\ell}]$; this implies pairwise independence but not conversely. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
- independence / not same as disjoint ::@:: Disjoint events with positive probability cannot be independent, because $P[A\cap B]=0$ while $P[A]P[B]>0$; independence is about factorisation of probabilities, not set disjointness. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
- independence / heuristic vs definition ::@:: If $P[A]>0$ and $P[B]>0$, then $A$ and $B$ are independent in the factorisation sense $P[A \cap B]=P[A]P[B]$ **if and only if** $P[A\mid B]=P[A]$ and $P[B\mid A]=P[B]$: from $P[A \cap B]=P[A]P[B]$ we get $P[A\mid B]=P[A\cap B]/P[B]=P[A]$ and $P[B\mid A]=P[A\cap B]/P[A]=P[B]$, and conversely $P[A\mid B]=P[A]$ and $P[B\mid A]=P[B]$ imply $P[A\cap B]=P[B]P[A\mid B]=P[A]P[B]$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
...
- independence / stability under complements (statement) ::@:: If $A_1,\ldots,A_n$ are jointly independent and $B_i$ is either $A_i$ or $A_i^c$ for each $i$, then the family $B_1,\ldots,B_n$ is also jointly independent. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
- independence / complements (step 1: split $A_1$) ::@:: For $n=2$, independence of $A_1$ and $A_2$ gives $P[A_1\cap A_2]=P[A_1]P[A_2]$; writing $A_1=(A_1\cap A_2)\cup(A_1\cap A_2^c)$ as a disjoint union and using additivity yields $P[A_1\cap A_2^c]=P[A_1](1-P[A_2])=P[A_1]P[A_2^c]$.
- independence / complements (step 2: swap roles) ::@:: Swapping the roles of $A_1$ and $A_2$ gives $P[A_1^c\cap A_2]=P[A_1^c]P[A_2]$ by the same splitting argument applied to $A_2$. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
- independence / complements (step 3: both complements) ::@:: Applying the same decomposition with $A_1^c$ in place of $A_1$ and $A_2^c$ in place of $A_2$ shows $P[A_1^c\cap A_2^c]=P[A_1^c]P[A_2^c]$; together with the previous steps this proves stability under complements for $n=2$, and the general $n$ follows by induction. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
...
- independence / hearts and aces ::@:: In a uniform 52-card deck, $A=$"heart", $B=$"ace" satisfy $P[A]=1/4$, $P[B]=1/13$, $P[A\cap B]=1/52=P[A]P[B]$, so they are independent. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
- independence / pairwise vs joint ::@:: For two fair coin tosses, take $A$="heads on first", $B$="heads on second", $C$="exactly one head"; each has $P[A]=P[B]=P[C]=1/2$ and every pair satisfies $P[A\cap B]=P[A]P[B]$, $P[A\cap C]=P[A]P[C]$, $P[B\cap C]=P[B]P[C]$, so the events are pairwise independent, but $P[A\cap B\cap C]=0$ while $P[A]P[B]P[C]=1/8$, so they are not jointly independent. <!-- check: ignore-line[two_sided_calc_warning]: conceptual -->
