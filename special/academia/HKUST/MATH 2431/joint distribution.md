...
- joint CDF rectangle condition: What is the $n$-dimensional analogue of the non-decreasing condition for a joint CDF? ::@:: The probability mass in any rectangle $(a_1,b_1]\times\cdots\times(a_n,b_n]$ is nonnegative. For $n=2$: $F(b_1,b_2)-F(a_1,b_2)-F(b_1,a_2)+F(a_1,a_2)\ge0$. In general: $\sum_{i_1=0}^1\cdots\sum_{i_n=0}^1(-1)^{n-\sum_k i_k}F(x^{(i_1)}_1,\dots,x^{(i_n)}_n)\ge0$ where $x^{(0)}_k=a_k$ and $x^{(1)}_k=b_k$.
