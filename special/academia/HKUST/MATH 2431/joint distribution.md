...
- covariance definition (finite second moments): What is $\operatorname{Cov}(X,Y)$? ::@:: $\operatorname{Cov}(X,Y)=E[(X-E[X])(Y-E[Y])]=E[XY]-E[X]E[Y]$, where $X,Y$ have finite second moments (so $\operatorname{Var}[X],\operatorname{Var}[Y]<\infty$). The computation formula $E[XY]-E[X]E[Y]$ follows by expanding the product.
- correlation coefficient: How is $\rho(X,Y)$ defined? ::@:: $\rho(X,Y)=\operatorname{Cov}(X,Y)/\sqrt{\operatorname{Var}[X]\operatorname{Var}[Y]}$ when both variances are positive, and $0$ otherwise. It is unitless and always lies in $[-1,1]$.
- bilinearity of covariance: Write the bilinearity identity for $\operatorname{Cov}(a+\sum c_iX_i,\;b+\sum d_jY_j)$. ::@:: $\operatorname{Cov}(a+\sum c_iX_i,\;b+\sum d_jY_j)=\sum_i\sum_j c_i d_j\operatorname{Cov}(X_i,Y_j)$. The additive constants $a,b$ vanish; only the coefficients $c_i,d_j$ of the random terms matter.
- covariance under independence: If $X$ and $Y$ are independent, is $\operatorname{Cov}(X,Y)=0$? ::@:: Yes — independence implies $\operatorname{Cov}(X,Y)=0$, but the converse is false: zero covariance does not imply independence.
- Bienaymé formula: What is $\operatorname{Var}(\sum_{i=1}^n X_i)$ in terms of variances and covariances? ::@:: $\operatorname{Var}(\sum_{i=1}^n X_i)=\sum_{i=1}^n\operatorname{Var}[X_i]+\sum_{i\neq j}\operatorname{Cov}(X_i,X_j)$, provided each $X_i$ has finite second moment. For uncorrelated $X_i$, it reduces to $\sum\operatorname{Var}[X_i]$.
- correlation bounds: What bounds does $\rho(X,Y)$ satisfy, and when is equality attained? ::@:: $|\rho(X,Y)|\le 1$, with equality $|\rho|=1$ iff $Y=aX+b$ almost surely for some $a\neq 0$ (perfect linear dependence). The sign of $\rho$ matches $\operatorname{sgn}(a)$.
- covariance matrix: What is the covariance matrix of $\boldsymbol X=(X_1,\dots,X_n)$? ::@:: $\Sigma=(\operatorname{Cov}(X_i,X_j))_{i,j=1}^n$, symmetric and positive semidefinite, with $\operatorname{Var}[u^\top\boldsymbol X]=u^\top\Sigma u$ for any $u\in\mathbb R^n$. Diagonal entries are variances $\Sigma_{ii}=\operatorname{Var}[X_i]$.
- proof of correlation bounds $|\rho|\le1$: How do you show $|\rho(X,Y)|\le1$ using variance? ::@:: Start from $0\le\operatorname{Var}(X/\sqrt{\operatorname{Var}[X]}\pm Y/\sqrt{\operatorname{Var}[Y]})$. Expand using $\operatorname{Var}(aX\pm bY)=a^2\operatorname{Var}[X]+b^2\operatorname{Var}[Y]\pm2ab\operatorname{Cov}(X,Y)$: with $a=1/\sqrt{\operatorname{Var}[X]}$, $b=1/\sqrt{\operatorname{Var}[Y]}$, this becomes $1+1\pm2\rho=2(1\pm\rho)$. Thus $0\le2(1\pm\rho)$. Taking $+$ gives $\rho\ge-1$; taking $-$ gives $\rho\le1$. Hence $|\rho|\le1$.
- $\rho=\pm1$ iff linear relation: proof idea ::@:: ($\Rightarrow$) If $\rho=1$, then $\operatorname{Var}(X/\sqrt{\operatorname{Var}[X]}-Y/\sqrt{\operatorname{Var}[Y]})=2(1-1)=0$. Zero variance $\implies$ the expression is constant a.s., i.e.\ $X/\sqrt{\operatorname{Var}[X]}-Y/\sqrt{\operatorname{Var}[Y]}=c$ for some constant $c$. Solving for $Y$ gives $Y=\sqrt{\operatorname{Var}[Y]/\operatorname{Var}[X]}\,X-c\sqrt{\operatorname{Var}[Y]}$, i.e. $Y=aX+b$ with $a>0$. The case $\rho=-1$ is analogous, giving $a<0$. ($\Leftarrow$) Conversely, if $Y=aX+b$, then $\operatorname{Cov}(X,Y)=a\operatorname{Var}[X]$ and $\operatorname{Var}[Y]=a^2\operatorname{Var}[X]$, so $\rho=a\operatorname{Var}[X]/(\sqrt{\operatorname{Var}[X]}\cdot|a|\sqrt{\operatorname{Var}[X]})=a/|a|=\pm1$.
...
- MVN characteristic function: What is $\varphi_{\boldsymbol X}(\boldsymbol t)$ for $\boldsymbol X\sim N(\boldsymbol\mu,\Sigma)$? ::@:: $\varphi_{\boldsymbol X}(\boldsymbol t)=E[e^{i\boldsymbol t^\top\boldsymbol X}]=\exp(i\boldsymbol t^\top\boldsymbol\mu-\frac12\boldsymbol t^\top\Sigma\boldsymbol t)$ for $\boldsymbol t\in\mathbb R^n$. This definition works for any symmetric nonnegative definite $\Sigma$ (including singular), unlike the density which requires $\Sigma\succ0$. When $\Sigma$ is singular, the distribution concentrates on an affine subspace.
