...
- **Open intervals and $\inf$ with $<$.** The set $(-\infty,x)$ is open. If the infimum is strictly below $x$, some $X_j$ is also strictly below $x$: $$\{\inf_j X_j<x\}=\bigcup_j\{X_j<x\}.$$ A countable union of open preimages — still open, hence measurable.
- **Closed intervals and $\sup$ with $\le$.** The set $(-\infty,x]$ is closed. If every $X_j$ is at most $x$, then so is the supremum: $$\{\sup_j X_j\le x\}=\bigcap_j\{X_j\le x\}.$$ A countable intersection of closed preimages — still closed, hence measurable.
