...
app.post("/merge", // Whenever we POST /merge, ...
  bodyParser.json(),
  (req, res) => {
    const data = req.body
    if ("attributes" in data && "values" in data) {
      ...
      for (let k = 0; k < Math.min(data.attributes.length, data.values.length); ++k) {
        if (!(orig[data.attributes[k]] === undefined) && isObject(orig[data.attributes[k]]) && isObject(data.values[k])) {
          ...
          orig[data.attributes[k]] = orig[data.attributes[k]].concat(data.values[k]) // Could be vulnerable
        } else {
          orig[data.attributes[k]] = data.values[k] // Could be vulnerable
        }
      }
    }
    res.send(secret.cmd === "cat flag.txt" ? "flag{:blobcat_radiation:}" : "Try harder")
  }
)
