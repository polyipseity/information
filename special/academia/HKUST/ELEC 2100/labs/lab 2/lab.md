...
- In the MATLAB fragment `h = conv(h2, h1); y = conv(h, x);`, why is it useful to compute the intermediate impulse response `h` first? ::@:: Because the cascade can be reduced to one equivalent impulse response before being applied to the input, so the system structure is understood before the final output is generated. <!--SR:!2026-07-12,62,310!2026-07-18,67,310-->
