...
- In the worked case $h_1[n] = \delta[n-1] + \delta[n-4]$ and $h_2[n] = u[n-A-2] - u[n-A-6]$, why can the overall impulse response be predicted even before the MATLAB line `h = conv(h2, h1)` is executed? ::@:: Because each impulse in `h_1[n]` simply creates a shifted copy of `h_2[n]`, so the convolution can already be read structurally as the sum of two shifted rectangles. <!--SR:!2026-07-10,60,310!2026-07-09,59,310-->
