...
- For the MATLAB fragment `x = exp(1j*20*pi*t)`, what four MATLAB calls split the same complex waveform into the four standard readable views? ::@:: `real(x)`, `imag(x)`, `abs(x)`, and `angle(x)`. <!--SR:!2026-07-07,54,310!2026-07-19,64,310-->
...
- In the MATLAB fragment `[x, fs] = audioread('song1.wav'); Ts = 1/fs; duration = length(x)/fs;`, why is the file metadata really sampling information rather than mere bookkeeping? ::@:: Because the sample array `x` and the sampling frequency `fs` together determine the sampling interval, playback duration, and time interpretation of the sound. <!--SR:!2026-07-07,54,310!2026-08-05,67,310-->
