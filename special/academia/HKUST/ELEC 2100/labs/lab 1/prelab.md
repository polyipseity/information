...
- In the MATLAB pair `sound(x, fs)` and `sound(x, 2*fs)`, what changes conceptually and what stays fixed? ::@:: The stored sample values `x` stay fixed, but the declared playback rate changes, so the same discrete record is interpreted on a different time scale and pitch impression. <!--SR:!2026-07-10,57,310!2026-08-02,64,310-->
