...
- In the MATLAB fragment `x = exp(1j*20*pi*t)` together with the plots `real(x)`, `imag(x)`, `abs(x)`, and `angle(x)`, which calls show Cartesian information and which calls show polar information? ::@:: `real(x)` and `imag(x)` show the Cartesian components, while `abs(x)` and `angle(x)` show the polar magnitude and phase. <!--SR:!2026-07-12,57,310!2026-07-08,55,310-->
