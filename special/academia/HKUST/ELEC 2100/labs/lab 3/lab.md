...
- In the Lab 3 MATLAB fragment `sample3a_f = sample3a_f .* (abs(sample3a_f) > 0.001)`, why is the phase plot deliberately masked by a magnitude threshold? ::@:: Because bins with nearly zero magnitude do not carry a stable physical phase, so the threshold suppresses meaningless angle values caused mainly by numerical noise. <!--SR:!2026-05-06,15,290!2026-05-06,14,290-->
...
- In the Lab 3 code `f_idx = ((0:N-1) - floor(N/2)) .* fs ./ N`, when $f_s = 100000\,\text{Hz}$ and $N = 100000$, why are the spectral peaks readable directly in hertz for this audio file? ::@:: Because the bin spacing is $f_s/N = 1\,\text{Hz}$, so each shifted DFT bin already lines up with an integer-hertz frequency label. <!--SR:!2026-05-08,16,290!2026-05-06,14,290-->
...
- In the Lab 3 spectrum with visible positive-frequency lines at $400\,\text{Hz}$, $5000\,\text{Hz}$, and $16000\,\text{Hz}$, why is the fundamental frequency taken to be $200\,\text{Hz}$ instead of $400\,\text{Hz}$? ::@:: Because the three visible lines are all integer multiples of $200\,\text{Hz}$, so $200\,\text{Hz}$ is their greatest common divisor and therefore the true fundamental spacing even though that line itself is missing. <!--SR:!2026-05-07,15,290!2026-05-08,16,290-->
- In the Lab 3 tone-identification step, what important caution does the answer $f_0 = 200\,\text{Hz}$ illustrate about a plotted magnitude spectrum? ::@:: It illustrates that the fundamental frequency of a periodic signal need not appear as the lowest visible nonzero line, because it can be absent while its harmonics are still present. <!--SR:!2026-05-07,16,290!2026-05-06,14,290-->
- In the Lab 3 table with positive-frequency indices $k = 2, 25, 80$, what do those harmonic numbers mean once $f_0 = 200\,\text{Hz}$ has been identified? ::@:: They mean the visible tones are the second, twenty-fifth, and eightieth harmonics of the same periodic signal. <!--SR:!2026-05-08,16,290!2026-05-08,16,290-->
...
- In the Lab 3 coefficient table, how does the positive-frequency phase $\angle X_{25} = -\pi/4$ reappear in the reconstructed waveform at $5000\,\text{Hz}$? ::@:: It reappears directly as the phase term of the corresponding cosine, so the $5000\,\text{Hz}$ component becomes $0.3\cos(10000\pi t - \pi/4)$. <!--SR:!2026-05-07,15,290!2026-05-07,15,290-->
...
- In the Lab 3 Butterworth design `[b, a] = butter(8, [2700 * 2 / fs, 10500 * 2 / fs])`, why are $2700\,\text{Hz}$ and $10500\,\text{Hz}$ reasonable cutoffs for the assignment signal? ::@:: Because they place the unwanted $400\,\text{Hz}$ and $16000\,\text{Hz}$ components outside the passband while keeping the desired $5000\,\text{Hz}$ component inside the passband. <!--SR:!2026-05-07,16,290!2026-05-08,16,290-->
- In the Lab 3 Butterworth design `[2700 * 2 / fs, 10500 * 2 / fs]`, why is each physical cutoff multiplied by `2 / fs` before being passed to `butter`? ::@:: Because MATLAB's digital Butterworth design expects frequencies normalized by the Nyquist frequency, so multiplying by `2 / fs` converts hertz into that normalized scale. <!--SR:!2026-05-07,16,290!2026-05-07,16,290-->
- In the Lab 3 call `[h, w] = freqz(b, a, fs / 2, fs)` for the detected tones at $400\,\text{Hz}$, $5000\,\text{Hz}$, and $16000\,\text{Hz}$, what is the point of requesting the response with the sampling frequency `fs` included? ::@:: It makes the returned frequency axis `w` appear directly in hertz, so the filter response can be read at those same physical tone frequencies. <!--SR:!2026-05-05,14,290!2026-05-08,16,290-->
...
- In the Lab 3 workflow, why are datatips added to both the magnitude-response and phase-response plots instead of only to the magnitude plot? ::@:: Because the lab needs both the attenuation information and the phase-shift information to explain the final filtered waveform in the time domain. <!--SR:!2026-05-07,15,290!2026-05-07,16,290-->
- In the Lab 3 datatip step, what should the response at $400\,\text{Hz}$ and $16000\,\text{Hz}$ look like if the Butterworth band-pass has been chosen correctly? ::@:: Their magnitude responses should be close to zero because those tones lie outside the designed passband. <!--SR:!2026-05-07,15,290!2026-05-05,14,290-->
...
- In the archived Lab 3 answer for original tones at $400\,\text{Hz}$, $5000\,\text{Hz}$, and $16000\,\text{Hz}$, why does the filtered output contain only one dominant cosine instead of the original three-tone mixture? ::@:: Because the designed band-pass keeps the middle $5000\,\text{Hz}$ component while strongly attenuating the lower $400\,\text{Hz}$ and higher $16000\,\text{Hz}$ components. <!--SR:!2026-05-08,16,290!2026-05-07,15,290-->
...
- In the Lab 3 interpretation step, why is a one-cosine time-domain output a good sanity check for the filter design? ::@:: Because the response analysis predicted that only the in-band spectral line should remain, so the time-domain waveform should collapse from a three-tone mixture to one dominant sinusoid. <!--SR:!2026-05-06,14,290!2026-05-07,15,290-->
