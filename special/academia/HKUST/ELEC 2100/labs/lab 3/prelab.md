...
- In the MATLAB fragment `f_shift = (-N/2:N/2-1) * fs/N`, why does the factor `fs/N` convert bin numbers into physical frequency? ::@:: Because adjacent DFT bins are separated by one cycle per record, which equals `fs/N` hertz for a record of `N` samples taken at sampling rate `fs`.
