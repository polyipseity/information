...
The generalized-function viewpoint says that the impulse is not an ordinary finite-valued waveform at all. Instead, for a smooth compactly supported test function $f(t)$, the defining rule is $\int_{-\infty}^{\infty}\delta(t)f(t)dt=f(0)$. The impulse is therefore defined by its action under integration. Test functions are smooth localized probes: smoothness is what makes later differentiation rules legitimate, and compact support is what removes boundary terms in integration-by-parts arguments.

The pulse-limit and generalized-function viewpoints fit together. If $\delta_\varepsilon$ has unit area and support shrinking to the origin, then $\int\delta_\varepsilon(t)f(t)dt=\int\delta_\varepsilon(t)(f(t)-f(0))dt+f(0)\int\delta_\varepsilon(t)dt$. The second term is exactly $f(0)$ because the area is $1$, while the first term becomes small because $f(t)-f(0)$ is small on the shrinking support. That is the intuitive bridge from ordinary pulse families to the distributional sampling rule.
...
The generalized-function proofs are short and instructive. The parity law follows by testing against $f$ and substituting $\tau=-t$. The scaling law follows by substituting $u=at$ in the defining integral. The multiplication law follows by testing both sides against another smooth probe $g(t)$, which gives $\int f(t)\delta(t-t_0)g(t)dt=f(t_0)g(t_0)=\int f(t_0)\delta(t-t_0)g(t)dt$.
...
Several structural properties follow immediately. Because $\delta(t)$ is even, $\delta'(t)$ is odd. Because the positive and negative parts cancel, its total signed area is zero in the generalized sense. The multiplication law becomes more delicate than for the impulse itself: $f(t)\delta'(t)=f(0)\delta'(t)-f'(0)\delta(t)$. This says that the doublet is sensitive not only to the local value of the multiplier but also to its local slope.

A useful warning is that the term $f(0)\delta'(t)$ does not behave like an ordinary weighted area contribution. For example, integrating it against the constant test function $1$ gives $0$ because $\delta'$ annihilates constants. The nonzero correction comes from the accompanying $-f'(0)\delta(t)$ term, which captures how the multiplier is changing at the support point.

Higher derivatives continue the same pattern. Their generalized-function rule is $\int_{-\infty}^{\infty}\delta^{(n)}(t)f(t)dt=(-1)^n f^{(n)}(0)$. This means the nth impulse derivative extracts the order-$n$ derivative of the test function at the support point. Each higher derivative is therefore more singular in the sense that it reacts to finer local Taylor data rather than merely to value or first slope.
