...
- Why does the impulse multiplication law $f(t)\delta(t-t_0)=f(t_0)\delta(t-t_0)$ hold, and what must you not conclude from it? ::@:: Once the impulse pins the calculation to the single instant $t_0$, the varying factor $f(t)$ may be replaced by the single number $f(t_0)$. <br/> Formally, for every smooth probe $g(t)$ one gets $\int f(t)\delta(t-t_0)g(t)dt=f(t_0)g(t_0)=\int f(t_0)\delta(t-t_0)g(t)dt$. <br/> Since both sides act identically on every probe, the multiplication law follows. <br/> But the law simplifies only the coefficient; it does **not** remove the delta. The result is still the singular object $f(t_0)\delta(t-t_0)$, not merely the number $f(t_0)$.
