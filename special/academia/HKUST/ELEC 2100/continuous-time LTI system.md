...
- What is the difference between input-output and state-variable descriptions? ::@:: Input-output uses one higher-order differential equation relating excitation and response. State-variable uses several coupled first-order equations that also track internal variables.
- Why start with the input-output viewpoint? ::@:: It gives the fastest route to time-domain response analysis.
- Why use time-domain analysis when transforms are often more convenient? ::@:: Because direct solution is physically intuitive, keeps initial conditions visible, and forms the foundation for transform methods.
- What is the standard constant-coefficient input-output model? ::@:: $a_n r^{(n)}(t)+a_{n-1}r^{(n-1)}(t)+\cdots+a_0r(t)=b_m e^{(m)}(t)+b_{m-1}e^{(m-1)}(t)+\cdots+b_0e(t)$.
- What exponential form produces the characteristic equation? ::@:: Substituting $r_h(t)=e^{st}$ gives $a_n s^n+a_{n-1}s^{n-1}+\cdots+a_0=0$.
- What is the classical complete-solution decomposition? ::@:: $\text{complete solution}=\text{homogeneous solution}+\text{particular solution}$.
- What does the homogeneous solution represent? ::@:: The system's natural modes — the motion caused by stored energy.
- What does the particular solution represent? ::@:: The part of the response forced by the external excitation.
...
- What is zero-input response? ::@:: Response when the external input is zero and only the system's initial stored energy remains.
- What is zero-state response? ::@:: Response when the initial state is zero and only the external excitation acts.
- What is zero-input response in the differential-equation viewpoint? ::@:: Solution of the homogeneous ODE with nonzero initial conditions.
- For $L[r]=M[e]$, what equations define $r_{\mathrm{zi}}$ and $r_{\mathrm{zs}}$? ::@:: $L[r_{\mathrm{zi}}]=0$ with stored-energy conditions; $L[r_{\mathrm{zs}}]=M[e]$ with zero initial conditions.
- How does linearity combine them? ::@:: $r(t)=r_{\mathrm{zi}}(t)+r_{\mathrm{zs}}(t)$.
- What is the difference between natural and forced response? ::@:: Natural response is the system's own modes; forced response is the part imposed by the external excitation.
- Why keep zero-input/zero-state separate from natural/forced? ::@:: Zero-input vs zero-state classifies by source and initial conditions; natural vs forced classifies by dynamical interpretation.
- What is transient response? ::@:: The part that dies away during adjustment.
- What is steady-state response? ::@:: The long-time behaviour after the transient has settled.
- Why can zero-state response contain both transient and steady-state parts? ::@:: Zero-state only means the response is caused by the external input, not that it is already at its long-time form.
- What question does each pair answer? ::@:: Zero-input vs zero-state: what caused it. Natural vs forced: which dynamical component. Transient vs steady-state: how it behaves over time.
...
- Why are capacitor voltage and inductor current state variables? ::@:: They store the system's electric and magnetic energy.
- Why are they usually continuous? ::@:: A capacitor resists instantaneous voltage change; an inductor resists instantaneous current change.
- How does integrating $i_C(t)=C\frac{dv_C(t)}{dt}$ across $[0^-,0^+]$ prove continuity? ::@:: It gives $C\bigl(v_C(0^+)-v_C(0^-)\bigr)=\int_{0^-}^{0^+}i_C(t)\,dt$; if the current is finite and nonimpulsive, the integral is $0$, so $v_C(0^+)=v_C(0^-)$.
- How does integrating $v_L(t)=L\frac{di_L(t)}{dt}$ across $[0^-,0^+]$ prove continuity? ::@:: It gives $L\bigl(i_L(0^+)-i_L(0^-)\bigr)=\int_{0^-}^{0^+}v_L(t)\,dt$; if the voltage is finite and nonimpulsive, $i_L(0^+)=i_L(0^-)$.
- When can a state variable jump? ::@:: When an impulse excitation injects a finite change in zero time.
- Why can an impulse create a jump? ::@:: It concentrates finite area into zero time, making it singular enough to change a stored-energy variable instantaneously.
- If $i_C(t)=A\delta(t)$, what is the capacitor voltage jump? ::@:: $v_C(0^+)-v_C(0^-)=A/C$.
- If $v_L(t)=B\delta(t)$, what is the inductor current jump? ::@:: $i_L(0^+)-i_L(0^-)=B/L$.
- What is the key lesson? ::@:: State continuity is the ordinary rule from energy-storage laws; jumps are exceptional singular-forcing events.
...
- Impulse response of $r(t)=ae(t)$? ::@:: $h(t)=a\delta(t)$ — scales input by gain $a$.
- Impulse response of $r(t)=\frac{d}{dt}e(t)$? ::@:: $h(t)=\delta'(t)$ — differentiation is singular.
- Impulse response of $r(t)=\int_{-\infty}^{t} e(\tau)\,d\tau$? ::@:: $h(t)=u(t)$ — a unit impulse becomes a unit step.
- Impulse response of $r(t)=e(t-\tau_0)$? ::@:: $h(t)=\delta(t-\tau_0)$ — shifts the impulse without reshaping.
- What family pattern connects these examples? ::@:: Scaling changes weight, differentiation raises singular order, integration accumulates into a step, delay shifts in time.
...
- Why does $h(t)\neq 0$ for $t<0$ imply noncausality? ::@:: The system would react before the impulse is applied, requiring future information.
- BIBO stability condition? ::@:: $\int_{-\infty}^{\infty}|h(t)|\,dt<\infty$.
- Why does absolute integrability guarantee BIBO stability? ::@:: Convolution with a bounded input uses weights with finite total absolute mass, so the output cannot grow without bound.
- What inequality proves this? ::@:: If $|e(t)|\le M$, then $|r_{\mathrm{zs}}(t)|\le M\int_{-\infty}^{\infty}|h(\lambda)|\,d\lambda$.
- Why are these tests useful? ::@:: They infer major system properties from one signal instead of testing many inputs.
