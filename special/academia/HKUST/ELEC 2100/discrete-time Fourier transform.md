...
- Does the notation $X(e^{j\Omega})$ make sense here, and what does it mean? ::@:: Yes. It does **not** mean exponentiating $X$. It means evaluating a frequency-domain function $X(\cdot)$ at the point $e^{j\Omega}$. In practice, one may read it simply as the DTFT at digital frequency $\Omega$. <br/> So the only exponential here is the point $e^{j\Omega}$, not the transform symbol $X$. <br/> Here $\Omega=\omega T$ is digital angular frequency, equivalently $\Omega/(2\pi)$ cycles per sample, usually read on $[-\pi,\pi]$. <!--SR:!2000-01-01,1,250!2026-04-12,4,274-->
...
- How does the time-shift property give an engineering derivation of the DTFT formula? ::@:: Write $x[n]=\sum_k x[k]\delta[n-k]$. <br/> Use the basic pair $\delta[n]\longleftrightarrow 1$. <br/> Then apply the shift rule to get $\delta[n-k]\longleftrightarrow e^{-j\Omega k}$. <br/> By linearity, $X(e^{j\Omega})=\sum_k x[k]e^{-j\Omega k}$. <br/> Here $X(e^{j\Omega})$ again denotes the DTFT value at digital frequency $\Omega$, where $\Omega=\omega T$ and $\Omega/(2\pi)$ is cycles per sample. <!--SR:!2000-01-01,1,250!2026-04-12,4,274-->
...
- Why can the DTFT pair be viewed as a Fourier-series pair in the variable $\Omega$? ::@:: Because $X(e^{j\Omega})$ is $2\pi$-periodic in $\Omega$, so it can be expanded as a complex Fourier series. Comparing $X(e^{j\Omega})=\sum_n x[n]e^{-jn\Omega}$ and $x[n]=\frac{1}{2\pi}\int_{-\pi}^{\pi}X(e^{j\Omega})e^{jn\Omega}\,d\Omega$ with the standard $2\pi$-periodic Fourier-series formulas shows that the sequence values $x[n]$ are exactly the Fourier-series coefficients of the periodic spectral function. <br/> Here $X(e^{j\Omega})$ is the periodic function of $\Omega$ whose coefficients are $x[n]$, with $\Omega=\omega T$ and one principal period on $[-\pi,\pi]$. <!--SR:!2000-01-01,1,250!2026-04-11,3,254-->
...
- Why must you be careful about conventions when linking DTFT with Fourier series? ::@:: Because different texts place the sign in the exponential and the factor $1/(2\pi)$ in different places. In this course the negative sign is in $X(e^{j\Omega})=\sum_n x[n]e^{-jn\Omega}$ and the factor $1/(2\pi)$ is in $x[n]=\frac{1}{2\pi}\int_{-\pi}^{\pi}X(e^{j\Omega})e^{jn\Omega}\,d\Omega$. <br/> Here $X(e^{j\Omega})$ is the course's notation for the periodic DTFT function, with $\Omega=\omega T$ and one principal period on $[-\pi,\pi]$. <!--SR:!2000-01-01,1,250!2026-04-12,4,274-->
...
- How are $f$, $\omega$, and $\Omega$ related? ::@:: They satisfy $\omega=2\pi f$ and $\Omega=\omega T=2\pi f/f_s$. <br/> So $\Omega$ is digital angular frequency, and $\Omega/(2\pi)=f/f_s$ is cycles per sample. Distinct digital frequencies are usually read on the principal interval $[-\pi,\pi]$. <!--SR:!2026-04-12,4,274!2000-01-01,1,250-->
- Why is $\Omega$ the natural DTFT variable? ::@:: Because the DTFT describes sequences indexed by sample number, so frequency is measured in radians per sample rather than radians per second. <br/> Here $\Omega=\omega T$, equivalently $\Omega/(2\pi)$ cycles per sample, and the principal interval is $[-\pi,\pi]$. <!--SR:!2000-01-01,1,250!2026-04-12,4,270-->
...
- Why does the sampled-spectrum viewpoint make DTFT periodicity obvious? ::@:: Because the sampled continuous-time spectrum repeats every $\omega_s$, and after scaling by $T$ that spacing becomes exactly $2\pi$ in digital frequency. <br/> Here digital frequency means $\Omega=\omega T$, equivalently $\Omega/(2\pi)$ cycles per sample, with principal interval $[-\pi,\pi]$. <!--SR:!2000-01-01,1,250!2026-04-12,4,274-->
...
- What is a standard sufficient condition for a DTFT to converge absolutely? ::@:: It is $\sum_{n=-\infty}^{\infty}|x[n]|<\infty$. <!--SR:!2026-04-12,4,274!2000-01-01,1,250-->
