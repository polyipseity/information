...
- What does a 2D convolution kernel do when it slides across an image patch? ::@:: It is a small two-dimensional weighting pattern that slides across a signal defined on two coordinates, such as an image, and forms each output value as a weighted local sum. <!--SR:!2026-08-14,73,330!2026-07-22,67,330-->
...
- What identity writes a signal as an integral of weighted shifted impulses? ::@:: It is $e(t)=\int_{-\infty}^{\infty} e(\tau)\,\delta(t-\tau)\,d\tau$. <!--SR:!2026-07-22,66,310!2026-07-26,70,330-->
...
- What does $\delta(t-\tau)$ represent in the impulse decomposition integral? ::@:: It represents a unit impulse placed at source time $\tau$ and observed at time $t$. <!--SR:!2026-07-21,66,330!2026-07-24,68,330-->
...
- In the convolution integral, what does the variable $t$ represent? ::@:: It represents the observation time at which the output is being evaluated. <!--SR:!2026-07-27,71,330!2026-07-19,64,330-->
...
- Why does convolution mix source time $\tau$ and observation time $t$? ::@:: Because it tracks how each earlier input event contributes later when the output is observed at time $t$. <!--SR:!fsrs,2027-04-25T00:00:00.000Z,287,287.11697064,1,2,7,0,0,2026-07-12T00:00:00.000Z!2026-07-20,65,330-->
...
- Why is convolution more than just multiplication inside an integral? ::@:: Because it represents accumulation of delayed system memories, not merely local pointwise interaction. <!--SR:!2026-07-27,71,330!2026-07-22,67,330-->
...
- What is the associative property of convolution? ::@:: It is $(f*h_1)*h_2=f*(h_1*h_2)$. <!--SR:!fsrs,2027-04-30T00:00:00.000Z,291,291.42724554,1,2,7,0,0,2026-07-13T00:00:00.000Z!2026-07-22,67,330-->
- How does associativity model a cascade of LTI subsystems with impulse responses $h_1$ then $h_2$? ::@:: If the input passes through subsystems with impulse responses $h_1$ and then $h_2$, the overall impulse response is $h=h_1*h_2$. <!--SR:!2026-07-18,62,310!2026-07-19,64,330-->
...
- How does writing a delay as convolution with $\delta(t-t_0)$ prove the time-shift property? ::@:: Since $f(t-t_0)=f*\delta(t-t_0)$, associativity gives $\bigl(f_1*\delta(t-t_0)\bigr)*f_2=(f_1*f_2)*\delta(t-t_0)=g(t-t_0)$, and commutativity gives the same result when the delay is applied to the second factor instead. <!--SR:!fsrs,2027-04-14T00:00:00.000Z,278,278.46760619,1,2,7,0,0,2026-07-10T00:00:00.000Z!2026-07-20,65,330-->
...
- How does the sampling property of the impulse give $f*\delta=f$? ::@:: Evaluate $(f*\delta)(t)=\int f(\tau)\delta(t-\tau)\,d\tau$ using the sampling property of the impulse to get $f(t)$. <!--SR:!2026-07-22,67,330!2026-07-21,66,330-->
...
- How does the convolution integral produce $f(t-t_0)$ when the kernel is $\delta(t-t_0)$? ::@:: Evaluate $(f*\delta(t-t_0))(t)=\int f(\tau)\delta(t-\tau-t_0)\,d\tau$, so the impulse samples the input at $\tau=t-t_0$. <!--SR:!2026-07-22,67,330!2026-08-16,75,330-->
...
- How does the accumulation identity $f(t)*u(t)=\int_{-\infty}^{t}f(\lambda)\,d\lambda$ derive from the support of $u(t-\tau)$? ::@:: Since $u(t-\tau)=1$ only when $\tau\le t$, the convolution integral reduces from $\int_{-\infty}^{\infty}f(\tau)u(t-\tau)\,d\tau$ to $\int_{-\infty}^{t}f(\tau)\,d\tau$. <!--SR:!2026-07-22,67,330!2026-07-18,63,310-->
...
- What is the $k$-th-order derivative-transfer rule for convolution? ::@:: For every integer $k\ge 0$ for which the generalized derivatives exist, $g^{(k)}(t)=f^{(k)}(t)*h(t)=f(t)*h^{(k)}(t)$. <!--SR:!2026-08-16,75,330!2026-07-21,66,330-->
...
- Why is it useful to move the derivative onto whichever factor is simpler? ::@:: It lets you place the derivative on whichever factor is simpler instead of differentiating the full convolution integral directly. <!--SR:!2026-07-18,63,310!fsrs,2027-05-12T00:00:00.000Z,300,300.01984473,1,2,7,0,0,2026-07-16T00:00:00.000Z-->
...
- Worked example: Given $f(t)=u(t)-2u(t-1)+u(t-2)$ and $h(t)=u(t)-u(t-1)$, what zero-state antiderivative $f^{(-1)}(t)$ is useful for computing $g(t)=f*h$? ::@:: Step 1: define $f^{(-1)}(t)=\int_{-\infty}^{t}f(\lambda)\,d\lambda$. <br/> Step 2: for $t<0$, $f^{(-1)}(t)=0$. <br/> Step 3: for $0\le t<1$, $f^{(-1)}(t)=t$. <br/> Step 4: for $1\le t<2$, $f^{(-1)}(t)=2-t$. <br/> Step 5: for $t\ge 2$, $f^{(-1)}(t)=0$. <!--SR:!2026-07-19,64,330!fsrs,2027-04-20T00:00:00.000Z,283,282.79716409,1,2,7,0,0,2026-07-11T00:00:00.000Z-->
...
- Worked example: Given $f(t)=u(t)-2u(t-1)+u(t-2)$ and $h(t)=u(t)-u(t-1)$, what is the final piecewise form of $g(t)=f*h$? ::@:: Step 1: for $t<0$, both $f^{(-1)}(t)$ and $f^{(-1)}(t-1)$ are $0$, so $g(t)=0$. <br/> Step 2: for $0\le t<1$, use $f^{(-1)}(t)=t$ and $f^{(-1)}(t-1)=0$, so $g(t)=t$. <br/> Step 3: for $1\le t<2$, use $f^{(-1)}(t)=2-t$ and $f^{(-1)}(t-1)=t-1$, so $g(t)=3-2t$. <br/> Step 4: for $2\le t<3$, use $f^{(-1)}(t)=0$ and $f^{(-1)}(t-1)=3-t$, so $g(t)=t-3$. <br/> Step 5: for $t\ge 3$, both terms vanish again, so $g(t)=0$. <!--SR:!2026-07-21,66,330!2026-08-11,70,330-->
- Why is the antiderivative-plus-impulse-pair trick useful in this example? ::@:: Because differentiating the gate turns it into a short sum of shifted impulses, so convolution becomes subtraction of shifted antiderivatives instead of a longer direct overlap computation. <!--SR:!2026-07-19,64,330!2026-07-22,67,330-->
...
- Why is the graphical method useful when formulas are awkward? ::@:: Because it turns convolution into a support-overlap geometry problem, making the correct intervals easier to see than in direct symbolic integration. <!--SR:!2026-08-13,72,330!2026-07-20,65,330-->
...
- If $f_1$ is supported on $[A,B]$ and $f_2$ is supported on $[C,D]$, what interval supports the convolution $f_1*f_2$? ::@:: Its support runs from $A+C$ to $B+D$. <!--SR:!2026-08-17,76,330!2026-07-22,67,330-->
...
- What are the main differences between the discrete-time unit sample $\delta[n]$ and the continuous-time Dirac delta $\delta(t)$? ::@:: $\delta[n]$ is an ordinary sequence equal to $1$ at one index and $0$ elsewhere, whereas $\delta(t)$ is a generalized function defined by unit area under integration; the former is drawn as a stem, the latter as a symbolic impulse arrow, and their unit conventions differ. <!--SR:!2026-07-22,67,330!2026-08-17,76,330-->
...
- What algebraic properties does discrete convolution share with continuous convolution? ::@:: It shares commutativity, associativity, and distributivity. <!--SR:!2026-08-14,73,330!2026-07-21,65,310-->
- What is the discrete impulse-sequence property inside convolution? ::@:: It is $x[n]*\delta[n-m]=x[n-m]$. <!--SR:!2026-07-19,64,330!2026-07-25,69,330-->
- Why is the property $x[n]*\delta[n-m]=x[n-m]$ useful? ::@:: It turns a shifted unit sample into a pure shift operator, making impulse-sequence calculations fast and transparent. <!--SR:!2026-07-22,67,330!2026-08-17,76,330-->
- If $x[n]$ is supported on $[N_{1,\min},N_{1,\max}]$ and $h[n]$ is supported on $[N_{2,\min},N_{2,\max}]$, what interval supports $x*h$? ::@:: The convolution output is supported on $[N_{1,\min}+N_{2,\min},\,N_{1,\max}+N_{2,\max}]$. <!--SR:!2026-07-22,67,330!2026-07-26,76,330-->
...
- Worked example: Given $x[n]=1$ for $0\le n\le 4$ and $0$ otherwise, what is $y[n]=x[n]*x[n]$? ::@:: Step 1: each nonzero product equals $1$, so $y[n]$ counts overlapping samples. <br/> Step 2: for $0\le n\le 4$, the count grows to $n+1$. <br/> Step 3: for $4\le n\le 8$, the count shrinks to $9-n$. <br/> Step 4: outside $0\le n\le 8$, the overlap is empty. <br/> Step 5: hence the nonzero values are $1,2,3,4,5,4,3,2,1$. <!--SR:!2026-08-12,71,330!2026-07-22,67,330-->
- Why does the convolution of two length-5 rectangular sequences become triangular? ::@:: Because the number of overlapping sample pairs first increases, then reaches a maximum, and then decreases symmetrically as one sequence slides past the other. <!--SR:!2026-07-22,67,330!2026-08-16,75,330-->
...
- Worked example: Given $h(t)=u(t)-u(t-T_s)$ and $f_s(t)=0.5\delta(t)+\delta(t-T_s)+1.5\delta(t-2T_s)+2\delta(t-3T_s)$, what is the convolution output $f(t)$? ::@:: Step 1: convolve each weighted impulse separately, so each one produces a weighted shifted copy of $h$. <br/> Step 2: sum the four pieces to get $f(t)=0.5h(t)+h(t-T_s)+1.5h(t-2T_s)+2h(t-3T_s)$. <br/> Step 3: expand each shifted rectangle over its own interval. <br/> Step 4: read off the staircase levels $0.5$, $1$, $1.5$, and $2$ on $[0,T_s)$, $[T_s,2T_s)$, $[2T_s,3T_s)$, and $[3T_s,4T_s)$. <!--SR:!2026-07-24,68,330!2026-07-22,67,330-->
...
- Worked example: Given $f(t)=u(t)-u(t-1)$, what is $s(t)=f(t)*f(t)$? ::@:: Step 1: write $s(t)=\int_{\max(0,t-1)}^{\min(1,t)}1\,d\tau$. <br/> Step 2: for $t<0$, the overlap is empty, so $s(t)=0$. <br/> Step 3: for $0<t<1$, the overlap length is $t$, so $s(t)=t$. <br/> Step 4: for $1<t<2$, the overlap length is $2-t$, so $s(t)=2-t$. <br/> Step 5: for $t>2$, the overlap is empty again, so $s(t)=0$. <!--SR:!2026-07-21,66,330!fsrs,2027-04-25T00:00:00.000Z,287,287.11697064,1,2,7,0,0,2026-07-12T00:00:00.000Z-->
...
- What general convolution-shape lesson is taught by the self-convolution of a unit-width pulse? ::@:: The output height tracks overlap length, so identical rectangular pulses convolved with themselves produce a triangular or tent-shaped waveform. <!--SR:!2026-07-25,69,330!2026-07-20,65,330-->
