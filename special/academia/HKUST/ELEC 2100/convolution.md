...
- What are the main steps of the graphical convolution method? ::@:: Write the integral in $\tau$, keep one signal as $f_1(\tau)$, time-reverse the other to $f_2(-\tau)$, shift it to $f_2(t-\tau)$, multiply the overlap, and integrate the overlap area as $t$ varies.
...
- What is the discrete impulse-sequence property? ::@:: $x[n]*\delta[n-m]=x[n-m]$.
- Why is $x[n]*\delta[n-m]=x[n-m]$ useful? ::@:: It turns a shifted unit sample into a pure shift operator, making impulse-sequence calculations fast and transparent.
- If $x[n]$ is supported on $[N_{1,\min},N_{1,\max}]$ and $h[n]$ on $[N_{2,\min},N_{2,\max}]$, what interval supports $x*h$? ::@:: The convolution output is supported on $[N_{1,\min}+N_{2,\min},\,N_{1,\max}+N_{2,\max}]$.
