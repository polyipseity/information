...
- What role does convolution play in computing the zero-state response of an LTI system? ::@:: It is the central time-domain operation for computing zero-state response of linear time-invariant systems. <!--SR:!2026-07-12,58,310!2026-05-18,17,310-->
- How does the lecture derive convolution from decomposing a signal into shifted impulses? ::@:: By decomposing the input into shifted impulses, transferring each impulse through the system, and summing the shifted impulse responses. <!--SR:!2026-05-18,17,310!2026-07-28,72,330-->
- What does a 2D convolution kernel do when it slides across an image patch? ::@:: It is a small two-dimensional weighting pattern that slides across a signal defined on two coordinates, such as an image, and forms each output value as a weighted local sum. <!--SR:!2026-05-18,17,310!2026-07-22,67,330-->
...
- What are the mechanical steps of 2D convolution: flip the kernel, shift it, multiply on the overlap, and sum? ::@:: Flip the kernel in both coordinates, shift it to the output location of interest, multiply pointwise over the overlap region, and sum the overlapping products. <!--SR:!2026-07-12,58,310!2026-05-18,17,310-->
...
- What does $e(\tau)$ represent as the weight of the source-time impulse in the decomposition integral? ::@:: It is the weight assigned to the impulse located at source time $\tau$. <!--SR:!2026-07-15,60,310!2026-05-18,17,310-->
...
- Why does decomposing an input into shifted impulses make LTI zero-state analysis straightforward? ::@:: Because time invariance tells us how each shifted impulse is transferred, and linearity lets us sum all the resulting partial outputs. <!--SR:!2026-07-10,56,310!2026-05-18,17,310-->
...
- How does time invariance turn a unit impulse at time $\tau$ into an output signal? ::@:: It turns the impulse at time $\tau$ into the shifted impulse response $h(t-\tau)$. <!--SR:!2026-07-28,72,330!2026-05-19,18,323-->
...
- What do the two factors in the integrand $e(\tau)h(t-\tau)$ mean conceptually? ::@:: $e(\tau)$ gives the input weight contributed from source time $\tau$, while $h(t-\tau)$ gives how the system carries that unit impulse to the observation time $t$. <!--SR:!2026-07-08,54,310!2026-05-18,17,310-->
- Why is convolution the natural tool for zero-state response of an LTI system? ::@:: Because it is exactly the formal sum of all shifted-impulse responses produced by decomposing the input into weighted impulses. <!--SR:!2026-07-26,76,330!2026-05-19,18,323-->
...
- In the convolution integral, what does $e(\tau)$ represent? ::@:: It represents the strength of the input at source time $\tau$. <!--SR:!2026-05-18,17,310!2026-07-12,58,310-->
...
- How does convolution commutativity let you swap the roles of input and impulse response in response analysis? ::@:: It means one may view the input as being spread by the impulse response or the impulse response as being weighted by the input, and the same output results. <!--SR:!2026-05-18,17,310!2026-07-22,67,330-->
- What is the distributive property of convolution? ::@:: It is $f*(h_1+h_2)=f*h_1+f*h_2$. <!--SR:!2026-07-01,54,310!2026-07-08,54,310-->
- How does distributivity model two parallel LTI systems with impulse responses $h_1$ and $h_2$? ::@:: If the same input is fed into two parallel subsystems with impulse responses $h_1$ and $h_2$ and their outputs are added, the overall impulse response is $h=h_1+h_2$. <!--SR:!2026-07-12,57,310!2026-05-19,18,323-->
...
- Which block-diagram operation corresponds to adding impulse responses, and which corresponds to convolving them? ::@:: Parallel connection corresponds to addition of impulse responses, and cascade connection corresponds to convolution of impulse responses. <!--SR:!2026-05-18,17,310!2026-07-26,70,330-->
...
- What does convolution with $\delta(t-t_0)$ do to a signal? ::@:: It delays the signal by $t_0$, giving $f(t-t_0)$. <!--SR:!2026-05-18,17,310!2026-07-24,68,330-->
- How does the convolution integral produce $f(t-t_0)$ when the kernel is $\delta(t-t_0)$? ::@:: Evaluate $(f*\delta(t-t_0))(t)=\int f(\tau)\delta(t-\tau-t_0)\,d\tau$, so the impulse samples the input at $\tau=t-t_0$. <!--SR:!2026-07-22,67,330!2026-05-18,17,310-->
- Why does $\delta(t-t_1)*\delta(t-t_2)=\delta(t-(t_1+t_2))$ mean pure delays add? ::@:: Because delaying by $t_1$ and then by $t_2$ is convolution with $\delta(t-t_1)*\delta(t-t_2)=\delta(t-(t_1+t_2))$, so the total delay is $t_1+t_2$. <!--SR:!2026-07-25,69,330!2026-05-18,17,310-->
- Why is $\delta(t)$ the impulse response of an ideal through-connection? ::@:: Because convolving with $\delta(t)$ leaves the signal unchanged, which is exactly the behaviour of an ideal through connection. <!--SR:!2026-07-26,70,330!2026-05-19,18,323-->
- Why is $\delta(t-t_0)$ the impulse response of an ideal delay line? ::@:: Because convolution with $\delta(t-t_0)$ reproduces the same waveform later in time without changing its shape, so it is the exact kernel for a pure delay. <!--SR:!2026-07-27,71,330!2026-05-18,17,310-->
- If each input event produces a fixed payment of $4$ dollars exactly $2$ seconds later, what impulse response models the rule and what output does it give for input $x(t)$? ::@:: Step 1: a pure delay of $2$ seconds with fixed amount $4$ is modeled by $h(t)=4\delta(t-2)$. <br/> Step 2: convolving with a shifted impulse delays and scales the input. <br/> Step 3: therefore $y(t)=x(t)*4\delta(t-2)=4x(t-2)$. <br/> Step 4: the output keeps the same shape, shifted right by $2$ seconds and multiplied by $4$. <!--SR:!2026-07-22,67,330!2026-05-18,17,310-->
...
- What is the $k$-th-order derivative-transfer rule for convolution? ::@:: For every integer $k\ge 0$ for which the generalized derivatives exist, $g^{(k)}(t)=f^{(k)}(t)*h(t)=f(t)*h^{(k)}(t)$. <!--SR:!2026-05-18,17,310!2026-07-21,66,330-->
- Why does $f*\delta^{(k)}=f^{(k)}$ mean $\delta^{(k)}$ acts like a differentiation kernel? ::@:: Because $f(t)*\delta^{(k)}(t)=f^{(k)}(t)$, so inserting $\delta^{(k)}$ into a convolution is the same as applying a kth derivative operator. <!--SR:!2026-05-18,17,310!2026-07-08,54,310-->
...
- Worked example: Given $f(t)=u(t)-2u(t-1)+u(t-2)$ and $h(t)=u(t)-u(t-1)$, how can $g(t)=f*h$ be rewritten using the integration-differentiation property? ::@:: Step 1: rewrite it as $g(t)=f^{(-1)}(t)*h'(t)$. <br/> Step 2: substitute $h'(t)=\delta(t)-\delta(t-1)$. <br/> Step 3: use $f*\delta(t-t_0)=f(t-t_0)$ term by term to obtain $g(t)=f^{(-1)}(t)-f^{(-1)}(t-1)$. <!--SR:!2026-07-24,68,330!2026-05-19,18,323-->
- Worked example: Given $f(t)=u(t)-2u(t-1)+u(t-2)$ and $h(t)=u(t)-u(t-1)$, what is the final piecewise form of $g(t)=f*h$? ::@:: Step 1: for $t<0$, both $f^{(-1)}(t)$ and $f^{(-1)}(t-1)$ are $0$, so $g(t)=0$. <br/> Step 2: for $0\le t<1$, use $f^{(-1)}(t)=t$ and $f^{(-1)}(t-1)=0$, so $g(t)=t$. <br/> Step 3: for $1\le t<2$, use $f^{(-1)}(t)=2-t$ and $f^{(-1)}(t-1)=t-1$, so $g(t)=3-2t$. <br/> Step 4: for $2\le t<3$, use $f^{(-1)}(t)=0$ and $f^{(-1)}(t-1)=3-t$, so $g(t)=t-3$. <br/> Step 5: for $t\ge 3$, both terms vanish again, so $g(t)=0$. <!--SR:!2026-07-21,66,330!2026-05-18,17,310-->
...
- What is usually the hardest part of analytical convolution in practice? ::@:: It is usually determining the correct overlap interval on which both factors are nonzero, not performing the final antiderivative. <!--SR:!2026-07-24,68,330!2026-05-18,17,310-->
- Why do the limits of integration in convolution often shrink from $(-\infty,\infty)$? ::@:: Because causality or finite support forces many parts of the integrand to be zero, so only the overlap region contributes. <!--SR:!2026-05-18,17,310!2026-07-28,72,330-->
- Worked example: Given $e(t)=u(t)-u(t-t_0)$ with $t_0>0$ and $h(t)=e^{-t}u(t)$, why is it useful to write $r(t)=r_1(t)-r_2(t)$? ::@:: Step 1: split the pulse as $e(t)=u(t)-u(t-t_0)$. <br/> Step 2: apply linearity to get $r=e*h=u*h-u(t-t_0)*h=r_1-r_2$. <br/> Step 3: each term is then a simpler step-response calculation. <!--SR:!2026-07-11,57,310!2026-05-18,17,310-->
...
- Why is the graphical method useful when formulas are awkward? ::@:: Because it turns convolution into a support-overlap geometry problem, making the correct intervals easier to see than in direct symbolic integration. <!--SR:!2026-05-18,17,310!2026-07-20,65,330-->
...
- Why does the rectangular-rectangle graphical example produce a trapezoidal output? ::@:: Because the overlap area first grows linearly, then stays constant during full overlap, and finally shrinks linearly as the rectangles separate. <!--SR:!2026-05-18,17,310!2026-07-11,57,310-->
- If $f_1$ is supported on $[A,B]$ and $f_2$ is supported on $[C,D]$, what interval supports the convolution $f_1*f_2$? ::@:: Its support runs from $A+C$ to $B+D$. <!--SR:!2026-05-18,17,310!2026-07-22,67,330-->
- Why does the support rule for continuous-time convolution become $A+C\le t\le B+D$? ::@:: Because a nonzero overlap requires both $A\le \tau\le B$ and $C\le t-\tau\le D$, which is equivalent to overlap between $[A,B]$ and $[t-D,t-C]$; that overlap exists exactly when $A+C\le t\le B+D$. <!--SR:!2026-05-18,17,310!2026-07-13,58,310-->
...
- How can discrete-time convolution be viewed as continuous-time convolution of impulse trains? ::@:: Embed the sequences as $x_\delta(t)=\sum_n x[n]\delta(t-n)$ and $h_\delta(t)=\sum_n h[n]\delta(t-n)$; then the coefficient of $\delta(t-k)$ in $x_\delta*h_\delta$ is $\sum_m x[m]h[k-m]=(x*h)[k]$. <!--SR:!2026-05-19,18,323!2026-07-10,56,310-->
- What is the discrete convolution sum for zero-state response? ::@:: It is $y[n]=\sum_{m=-\infty}^{\infty}x[m]h[n-m]$. <!--SR:!2026-08-02,83,343!2026-05-18,17,310-->
...
- What compact notation is used for the discrete convolution sum? ::@:: It is written as $y[n]=(x*h)[n]$. <!--SR:!2026-05-18,17,310!2026-07-11,57,310-->
- What do the indices $m$ and $n$ mean in the sum $y[n]=\sum_{m=-\infty}^{\infty}x[m]h[n-m]$? ::@:: $m$ is the source index from which input contributions originate, while $n$ is the observation index at which the output is evaluated. <!--SR:!2026-05-18,17,310!2026-07-08,54,310-->
...
- What are the main differences between the discrete-time unit sample $\delta[n]$ and the continuous-time Dirac delta $\delta(t)$? ::@:: $\delta[n]$ is an ordinary sequence equal to $1$ at one index and $0$ elsewhere, whereas $\delta(t)$ is a generalized function defined by unit area under integration; the former is drawn as a stem, the latter as a symbolic impulse arrow, and their unit conventions differ. <!--SR:!2026-07-22,67,330!2026-05-18,17,310-->
...
- What algebraic properties does discrete convolution share with continuous convolution? ::@:: It shares commutativity, associativity, and distributivity. <!--SR:!2026-05-18,17,310!2026-07-21,65,310-->
...
- Why is the property $x[n]*\delta[n-m]=x[n-m]$ useful? ::@:: It turns a shifted unit sample into a pure shift operator, making impulse-sequence calculations fast and transparent. <!--SR:!2026-07-22,67,330!2026-05-18,17,310-->
...
- Why does the discrete support rule become $N_{1,\min}+N_{2,\min}\le n\le N_{1,\max}+N_{2,\max}$? ::@:: Because a nonzero term requires both $N_{1,\min}\le m\le N_{1,\max}$ and $N_{2,\min}\le n-m\le N_{2,\max}$, and those two intervals overlap exactly on that range of $n$. <!--SR:!2026-07-26,76,330!2026-05-18,17,310-->
...
- What are the main ways listed in the lecture for computing discrete convolution? ::@:: The lecture lists direct analytical summation, graphical inversion-and-shift, use of algebraic properties, and direct sum of pairwise products. <!--SR:!2026-07-28,72,330!2026-05-18,17,310-->
- What is the core step-by-step process behind graphical discrete convolution? ::@:: Sequence inversion, shift, multiplication, and summation. <!--SR:!2026-05-18,17,310!2026-07-08,54,310-->
- Worked example: Given $h[n]=\alpha^n u[n]$ with $0<\alpha<1$ and input $u[n]$, what is the unit-step response $y[n]=u[n]*h[n]$? ::@:: Step 1: write $y[n]=\sum_m u[m]\alpha^m u[n-m]$. <br/> Step 2: the one-sided supports require $0\le m\le n$. <br/> Step 3: therefore $y[n]=\sum_{m=0}^{n}\alpha^m$. <br/> Step 4: evaluate the geometric sum to get $y[n]=\frac{1-\alpha^{n+1}}{1-\alpha}u[n]$. <!--SR:!2026-07-25,69,330!2026-05-18,17,310-->
...
- Worked example: Given $x[n]=1$ for $0\le n\le 4$ and $0$ otherwise, what is $y[n]=x[n]*x[n]$? ::@:: Step 1: each nonzero product equals $1$, so $y[n]$ counts overlapping samples. <br/> Step 2: for $0\le n\le 4$, the count grows to $n+1$. <br/> Step 3: for $4\le n\le 8$, the count shrinks to $9-n$. <br/> Step 4: outside $0\le n\le 8$, the overlap is empty. <br/> Step 5: hence the nonzero values are $1,2,3,4,5,4,3,2,1$. <!--SR:!2026-05-18,17,310!2026-07-22,67,330-->
- Why does the convolution of two length-5 rectangular sequences become triangular? ::@:: Because the number of overlapping sample pairs first increases, then reaches a maximum, and then decreases symmetrically as one sequence slides past the other. <!--SR:!2026-07-22,67,330!2026-05-18,17,310-->
...
- What compact overlap formula produces the triangular self-convolution of $f(t)=u(t)-u(t-1)$? ::@:: It is $s(t)=\int_{\max(0,t-1)}^{\min(1,t)}1\,d\tau=\min(1,t)-\max(0,t-1)$ whenever the overlap interval is nonempty. <!--SR:!2026-07-12,58,310!2026-05-18,17,310-->
- Worked example: Given $f(t)=u(t)-u(t-1)$, when does $s(t)=f(t)*f(t)$ reach its maximum value, and why? ::@:: Step 1: the output equals the overlap length of two unit-width pulses. <br/> Step 2: that overlap increases as $t$ moves from $0$ to $1$. <br/> Step 3: at $t=1$ the two pulses overlap fully, so the overlap is maximal. <br/> Step 4: after $t=1$ the overlap shrinks again, so the maximum occurs at $t=1$. <!--SR:!2026-05-18,17,310!2026-07-16,61,310-->
