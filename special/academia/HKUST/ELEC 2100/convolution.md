...
- What do the two factors in the integrand $e(\tau)h(t-\tau)$ mean conceptually? ::@:: $e(\tau)$ gives the input weight contributed from source time $\tau$, while $h(t-\tau)$ gives how the system carries that unit impulse to the observation time $t$.
- Why is convolution the natural tool for zero-state response of an LTI system? ::@:: Because it is exactly the formal sum of all shifted-impulse responses produced by decomposing the input into weighted impulses.
...
- How is the commutative property proved from the convolution integral? ::@:: Use the substitution $\lambda=t-\tau$ in $(f_1*f_2)(t)=\int f_1(\tau)f_2(t-\tau)\,d\tau$ to rewrite it as $\int f_2(\lambda)f_1(t-\lambda)\,d\lambda=(f_2*f_1)(t)$.
- What is the intuition behind the commutative property in response analysis? ::@:: It means one may view the input as being spread by the impulse response or the impulse response as being weighted by the input, and the same output results.
- What is the distributive property of convolution? ::@:: It is $f*(h_1+h_2)=f*h_1+f*h_2$.
- How does the distributive property explain parallel LTI systems? ::@:: If the same input is fed into two parallel subsystems with impulse responses $h_1$ and $h_2$ and their outputs are added, the overall impulse response is $h=h_1+h_2$.
- What is the associative property of convolution? ::@:: It is $(f*h_1)*h_2=f*(h_1*h_2)$.
- How does the associative property explain cascade or series LTI systems? ::@:: If the input passes through subsystems with impulse responses $h_1$ and then $h_2$, the overall impulse response is $h=h_1*h_2$.
- Why can the order of LTI subsystems in cascade be swapped? ::@:: Because convolution is commutative, so $h_1*h_2=h_2*h_1$.
- What block-diagram meanings correspond to addition and convolution of impulse responses? ::@:: Parallel connection corresponds to addition of impulse responses, and cascade connection corresponds to convolution of impulse responses.
...
- What is the time-shift property of convolution? ::@:: If $g(t)=f_1(t)*f_2(t)$, then delaying either factor by $t_0$ delays the output by the same amount, so $f_1(t-t_0)*f_2(t)=g(t-t_0)$ and $f_1(t)*f_2(t-t_0)=g(t-t_0)$.
- Why does the time-shift property fit the LTI viewpoint? ::@:: Because delaying the excitation before the system should simply delay the response, and convolution preserves exactly that behaviour.
- What is the graphical intuition behind the time-shift property of convolution? ::@:: A pure delay slides one factor horizontally without changing its shape, so the overlap picture at time $t$ is exactly the old overlap picture at time $t-t_0$; the output graph is therefore shifted rigidly by the same amount.
- How does the delayed-Dirac-delta viewpoint derive the time-shift property? ::@:: Since $f(t-t_0)=f*\delta(t-t_0)$, associativity gives $\bigl(f_1*\delta(t-t_0)\bigr)*f_2=(f_1*f_2)*\delta(t-t_0)=g(t-t_0)$, and commutativity gives the same result when the delay is applied to the second factor instead.
- What is the result of convolving a signal with the unit impulse? ::@:: It leaves the signal unchanged: $f(t)*\delta(t)=f(t)$.
- How is the identity $f(t)*\delta(t)=f(t)$ derived from the convolution integral? ::@:: Evaluate $(f*\delta)(t)=\int f(\tau)\delta(t-\tau)\,d\tau$ using the sampling property of the impulse to get $f(t)$.
- What is the result of convolving a signal with a shifted impulse $\delta(t-t_0)$? ::@:: It delays the signal by $t_0$, giving $f(t-t_0)$.
- How is the delay identity $f(t)*\delta(t-t_0)=f(t-t_0)$ derived from the convolution integral? ::@:: Evaluate $(f*\delta(t-t_0))(t)=\int f(\tau)\delta(t-\tau-t_0)\,d\tau$, so the impulse samples the input at $\tau=t-t_0$.
...
- If $g(t)=f(t)*h(t)$, what is the differentiation property of convolution? ::@:: One may differentiate either factor: $g'(t)=f'(t)*h(t)=f(t)*h'(t)$.
- How is the differentiation property $g'(t)=f*h'$ derived from the convolution integral? ::@:: Differentiate under the integral sign: if $g(t)=\int f(\tau)h(t-\tau)\,d\tau$, then $g'(t)=\int f(\tau)h'(t-\tau)\,d\tau=(f*h')(t)$.
- What is the kth-order differentiation property of convolution? ::@:: For every integer $k\ge 0$ for which the generalized derivatives exist, $g^{(k)}(t)=f^{(k)}(t)*h(t)=f(t)*h^{(k)}(t)$.
- How does the kth derivative of the Dirac delta encode kth differentiation inside convolution? ::@:: Because $f(t)*\delta^{(k)}(t)=f^{(k)}(t)$, so inserting $\delta^{(k)}$ into a convolution is the same as applying a kth derivative operator.
- Why is the differentiation property useful in practice? ::@:: It lets you place the derivative on whichever factor is simpler instead of differentiating the full convolution integral directly.
- How does integration interact with convolution conceptually? ::@:: Integrating the output is equivalent to convolving one factor with the integrated form of the other factor in the same zero-state accumulation sense.
- If $f^{(-1)}(t)=\int_{-\infty}^{t} f(\lambda)\,d\lambda$, how can $f*h$ be rewritten using one antiderivative and one derivative? ::@:: It can be rewritten as $f*h=f^{(-1)}*h'=f'*h^{(-1)}$ whenever the relevant zero-state derivatives and antiderivatives exist.
- Why do differentiation and integration move cleanly through convolution? ::@:: Because convolution is built from linear shifting, weighting, and addition, and differentiation and integration commute with those constructions under the usual regularity assumptions.
...
- Worked example: Given $h(t)=u(t)-u(t-1)$, what is $h'(t)$ and why is that useful in convolution? ::@:: Step 1: differentiate the rising edge $u(t)$ to get $\delta(t)$. <br/> Step 2: differentiate the falling edge $-u(t-1)$ to get $-\delta(t-1)$. <br/> Step 3: therefore $h'(t)=\delta(t)-\delta(t-1)$. <br/> Step 4: this is useful because convolution with shifted impulses becomes a weighted difference of shifted copies instead of a full overlap integral.
...
- Why is the graphical method useful when formulas are awkward? ::@:: Because it turns convolution into a support-overlap geometry problem, making the correct intervals easier to see than in direct symbolic integration.
