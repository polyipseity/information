...
- Why is the backward form $y[n]=x[n]+ay[n-1]$ preferred over the forward form $y[n+1]=x[n+1]+ay[n]$ for implementation? ::@:: The two forms are mathematically equivalent, because the forward form is obtained from the backward form by replacing $n$ with $n+1$. But $y[n]=x[n]+ay[n-1]$ is preferred for implementation because it computes the present output directly from the present input and the previously stored output, so it is already in the most convenient causal real-time form.
- How are $y[n]=x[n]+ay[n-1]$ and $y[n+1]=x[n+1]+ay[n]$ related? ::@:: They are the same first-order recursion written with different time indices. Replacing $n$ by $n+1$ in $y[n]=x[n]+ay[n-1]$ gives $y[n+1]=x[n+1]+ay[n]$, so the two equations are mathematically equivalent.
