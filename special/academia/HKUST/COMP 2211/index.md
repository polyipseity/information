...
- [artificial intelligence](../../../../general/artificial%20intelligence.md) (AI)  
  ...
  - artificial intelligence / commonly confused terms ::: artificial intelligence, deep learning, machine learning, neural network
    ...
    - deep learning ::: a subfield of _machine learning_ that uses _neural networks_ to analyze different factors with a structure similar to the human neural system
...
- artificial intelligence
  ...
  - artificial intelligence / libraries ::: Keras, PyTorch, TensorFlow, scikit-learn
    - Keras ::: (moderately customizable and flexible; moderate difficulty; good for beginners) It is the high-level application programming interface (API) built on top of TensorFlow. It is incredibly user-friendly and easy to pick up. It is good for fast experimentation.
...
- [Python](../../../../general/Python%20(programming%20language).md)
  ...
  - advanced Python
    ...
    - advanced Python / list comprehension ::: `[<expression> for <element> in <iterable> if <condition>]` <p> `if <condition>` is optionally. It produces a `list` consisting of elements after evaluating `<expression>` on each `<element>` in `<iterable>`. If `<condition>` is present and returns false for an `<element>`, `<expression>` is not evaluated and not added to the `list`.
...
- Python
  - advanced Python
    - advanced Python / dictionary comprehension ::: `{<key expression>: <value expression> for <element> in <iterable> if <condition>}` <p> `if <condition>` is optionally. It produces a `dict` consisting of the key-value pairs after evaluating `<key expression>: <value expression>` on each `<element>` in `<iterable>`. If `<condition>` is present and returns false for an `<element>`, `<key expression>: <value expression>` is not evaluated and not added to the `dict`.
    ...
    - advanced Python / object-oriented programming (OOP) ::: __Object-oriented programming__ structure programs by bundling related properties (instance variables) and behaviors (methods) into individual objects. Fundamental concepts include _class_ and _object_.
...
- [NumPy](../../../../general/NumPy.md) ::: __NumPy__ (__Numerical Python__, but no one uses this...) is a Python library supporting _high-performance, large, multi-dimensional arrays and matrices_, along with high-level mathematical functions to operate on these arrays. It is used for _numeric and scientific computing_.
  ...
  - NumPy / indexing ::: NumPy arrays can be indexed, sliced, integer-indexed, or boolean-indexed. The former two are also called __simple indexing__. The latter two are also called __advanced indexing__. If _any advanced indexing is involved_, the resulting array is a _copy_ of the original array. Otherwise, it is a _view_ of the original array.
    ...
    - NumPy / indexing / `a[i:j:n]` ::: Select elements with index i through j (exclusive), with increment n.
  ...
  - NumPy / data types ::: A NumPy array have elements all of the same type. NumPy provides a large set of numeric data types (e.g. `np.int64`, `np.float64`) and some other types. <p> NumPy tries to guess the data type when you create an array, but you can control this using the `dtype` optional parameter.
...
- [NumPy](../../../../general/NumPy.md)
  ...
  - [NumPy § `numpy.expand_dims`](../../../NumPy/API%20reference/generated/numpy.expand_dims.md) ::: `numpy.expand_dims(a, axis)` <p> A view is returned. <p> `axis` is an `int` or a `tuple` of `int`s specifying the position in the expanded axes where the new axis (or axes) is placed. (Annotation: The new axis is inserted after the specified position, similar to `list.insert`.)
...
- error measurement ::: mean absolute error (MAE), mean square error (MSE), mean absolute percentage error (MAPE), F1-score error
...
- [_k_-nearest neighbors algorithm](../../../../general/k-nearest%20neighbors%20algorithm.md)
  ...
  - _k_-nearest neighbors algorithm / classification, regression ::: For classification, we use the majority vote of the _k_ nearest neighbors and a tie-breaking method. For regression, we take the average of the output of the _k_ nearest neighbors.
...
- [_k_-means clustering](../../../../general/k-means%20clustering.md) ::: ___k_-means clustering__ is a method of vector quantization, originally from signal processing, that aims to partition _n_ observations into _k_ clusters in which each observation belongs to the cluster with the nearest mean (cluster centers or cluster centroid), serving as a prototype of the cluster.
  ...
  - _k_-means clustering / scikit-learn ::: `sklearn.cluster.KMeans(n_clusters=<number of clusters>, init=<initial centroids>, n_init=<number of times to initialize centroids>, max_iter=<maximum number of iterations>)` → `fit(training_data)` → `predict(new_data)`, `cluster_centers_`
...
- [_k_-means clustering](../../../../general/k-means%20clustering.md)
  ...
  - _k_-means clustering / advantages ::: easy to understand and implement, efficient given _k_ and the number of iterations are small, sensitive to initial centroids, unsuitable for non-hyper-ellipsoids (hyper-spheres) clusters
  - _k_-means clustering / disadvantages ::: applicable if a mean is defined (_k_-modes may be used for categorical data), _k_ is arbitrary, sensitive to outliers
...
- [perceptron](../../../../general/perceptron.md) ::: A __perceptron__ is a simple biological neuron model in an artificial neural network. It performs certain calculations to detect input data capabilities. It is also the name of an early algorithm for supervised learning of binary classifiers (i.e., only two classes).
  ...
  - [§ steps](../../../../general/perceptron.md#steps)
    - [perceptron § steps](../../../../general/perceptron.md#steps) / initialization ::: Initialize the weights arbitrary. Weights may be initialized to 0 or small random values.
