...
- discrete random variable ::@:: A __discrete random variable__ has a _countable_ (finite or infinite) range.
  - [probability mass function](../../../../general/probability%20mass%20function.md) (PMF, pmf) ::@:: A __probability mass function__ (__PMF__, __pmf__) is a function that gives the probability that a _discrete random variable_ is exactly equal to some value. It is given by: $$p_X(x) = P(X = x) \,.$$ It is not defined for a continuous random variable. <p> The PMF has the following properties: $$\begin{aligned} \sum_x p_X(x) & = 1 \\ p_X(x) & \ge 0 \,. \end{aligned}$$
    - probability mass function / graph ::@:: To graph a PMF, draw an empty _xy_ graph. Maybe add a title to the graph. Draw _solid_ vertical lines on where the probability mass returns nonzero, representing its value at that point. Draw a _filled_ circle on top of each vertical line.
...
- [variance](../../../../general/variance.md) ::@:: __Variance__ is the expected value of the squared deviation from the mean of a random variable. It is often represented by $\sigma ^{2}$, $s^{2}$, $\operatorname {Var} (X)$, $V(X)$, or $\mathbb {V} (X)$.
  - variance / discrete random variable ::@:: The variance fo a discrete random variable is $$\operatorname{Var}(X) = \sum_{x \in \mathcal X} \left((x - \mu)^2 p(x) \right) \,.$$ This is only defined if the above sum and $\mu$ exists, so it is possible for a discrete random variable to have undefined variance.
...
- report
  - When you see $\mathcal N(0, 100)$, do not mistake the 100 as the standard deviation! It is the variance. Almost screwed this up in the midterm examination...
    - $\mathcal N(0, 100)$ (0) ::@:: The mean of the normal distribution is 0. The standard deviation of the normal distribution is 10, not 100.
...
- [method of moments](../../../../general/method%20of%20moments%20(statistics).md) ::@:: You have _k_ parameters of a known distribution to estimate. Express the first _k_ raw population moments in terms of the _k_ parameters, assuming _they exist_: $\operatorname E[X], \ldots, \operatorname E\left[X^k\right]$. Calculate the _k_ raw sample moments: $\overline X, \ldots \overline {X^k}$. Equate them: $\operatorname E[X] = \overline X, \ldots, \operatorname E\left[X^k\right] = \overline {X^k}$. Now there are _k_ (nonlinear in general) equations to solve for _k_ parameters. If _a solution exist_, the solution produce the estimators for _k_ parameters.
...
- [interval estimation](../../../../general/interval%20estimation.md)
  - interval estimation / Why interval estimation instead of point estimation? ::@:: The latter cannot provide the precision (i.e. is the actual parameter close) and reliability (i.e. is the actual parameter _likely_ to be close) of the estimators. And sometimes, a range is more meaningful than a point.
  - interval estimation / terminology ::@:: __Random interval__ is the methodology. <p> Given a sample, the calculated random interval is a __confidence interval__ (or if using other methods, it could be __credible interval__, __likelihood interval__, etc. but we will not touch these).
...
- [confidence interval](../../../../general/confidence%20interval.md) ::@:: It is an interval which is expected to typically contain the parameter being estimated. More specifically, given a __confidence level__ $\gamma$ (95% and 99% are typical values), a CI is a random interval which contains the parameter being estimated $100\gamma$% of the time. <p> Note: This course uses $100(1 - \alpha)$% instead of $\gamma$%. So $\alpha$ is the probability that a random interval from a theoretical sample does not contain the population parameter.
  - confidence interval / definition ::@:: Wikipedia uses $$P(T_1 < \theta < T_2) \ge \gamma \,,$$, with equality replacing $\ge$ when the distribution is continuous. This course uses $$P(T_1 \le \theta \le T_2) \ge 1 - \alpha \,,$$ with equality replacing $\ge$ when the distribution is continuous.
  - confidence interval / construction ::@:: Simply find two statistics. <p> In practice, the two statistics are found _symmetrically_ (i.e. the confidence interval of a random sample has the same probability of lying on the left or right of and outside the actual population parameter), so that its precision is the highest (the interval is the smallest), given the underlying probability is unimodal and symmetric.
  - confidence interval / interpretations ::@:: It can be expressed in terms of a long-run frequency in _repeated samples_, probability with respect to _a single theoretical (yet to be realized) sample_, _statistical significance_, etc.
...
- [chi-squared distribution](../../../../general/chi-squared%20distribution.md) ::@:: It with $k$ _degrees of freedom_ is the distribution of a sum of the _squares_ of $k$ _independent standard normal random variables_. It is denoted $\chi^2(k)$ (in this course) or $\chi_k^2$.
  - chi-squared distribution / probability density function (PDF) ::@:: (Probably no need to memorize this...) $$f(y) = \begin{cases} \frac 1 {2^{n / 2} \Gamma(n / 2) } y^{n / 2 - 1} e^{-y / 2} & \text{if }y > 0, \\ 0 & \text{otherwise} \end{cases}$$
  - chi-squared distribution / shape ::@:: It looks like a right-skewed (left-leaning) hill. The higher the $k$, the lower the peak and the fatter the hill. Note that for $k = 1, 2$, the peak is at $x = 0$.
  - chi-squared distribution / additivity ::@:: If $V \sim \chi^2(v)$ and $W \sim \chi^2(w)$ are _independent_, then $V + W \sim \chi^2(v + w)$. This is obvious from the definition...
  - chi-squared distribution / statistics ::@:: Let $Y \sim \chi^2(k)$. Then $\operatorname E[Y] = k$ and $\operatorname{Var}(Y) = 2k$.
- [Student's t-distribution](../../../../general/Student's%20t-distribution.md) ::@:: It is continuous probability distribution that generalizes the standard normal distribution. Like the latter, it is symmetric around zero and bell-shaped. (This course denotes it by $t(\nu)$ instead of $t_\nu$.) <p> However, it has heavier tails and the amount of probability mass in the tails is controlled by the parameter $\nu$. For $\ \nu =1\ {}$ the Student's _t_ distribution $t_{\nu }$ becomes the standard [Cauchy distribution](../../../../general/Cauchy%20distribution.md), which has very ["fat" tails](../../../../general/fat-tailed%20distribution.md); whereas for $\ \nu \rightarrow \infty \ {}$ it becomes the standard normal distribution $\ {\mathcal {N} }(0,1)\,$ which has very "thin" tails.
  ...
  - Student's t-distribution / probability density function (PDF) ::@:: (Probably no need to memorize this...) $$f(w) = \frac {\Gamma((n + 1) / 2)} {\sqrt{\pi n} \Gamma(n / 2)} \left(1 + \frac {w^2} n\right)^{-(n + 1) / 2}$$
  - Student's t-distribution / brief history ::@:: William Sealy Gosset, known as "Student" <p> Student (1908). The probable error of a mean. _Biometrika_, 6(1), 1-25.
- [normal distribution](../../../../general/normal%20distribution.md)
  - normal distribution / sample mean ::@:: For _n_ samples of a population $X \sim \mathcal N\left(\mu_X, \sigma_X^2\right)$, we have $$\begin{aligned} \overline X & \sim \mathcal N\left(\mu_X, \sigma_X^2 / n\right) \\ (\overline X - \mu_X)\sqrt{n / \sigma_X^2} & \sim \mathcal N(0, 1) \,. \end{aligned}$$
- [Cochran's theorem](../../../../general/Cochran's%20theorem.md)
  - Cochran's theorem / special case ::@:: For _n_ samples of a population $X \sim \mathcal N\left(\mu_X, \sigma_X^2\right)$ and its _corresponding_ normalized population $Z \sim \mathcal N(0, 1)$, $\overline X$ and $S_{X, n - 1}^2$ (or $S_{X, n}^2$) are _independent_. Further, we have $$\begin{aligned} \sum_{i = 1}^n (Z_i - \overline Z)^2 & \sim \chi^2(n - 1) \\ \sum_{i = 1}^n \left(\frac {X_i - \overline X} {\sigma_X} \right)^2 = \frac {(n - 1) S_{X, n - 1}^2 } {\sigma_X^2} & \sim \chi^2(n - 1) \,. \end{aligned}$$ (For the second line, $n S_{X, n}^2 = (n - 1) S_{X, n - 1}^2$ would also work.)
    - Cochran's theorem / special case / intuition ::@:: The squared deviations from the mean (SDM) of a sample of a normal population, given the _population mean_ is known, has _n_ degrees of freedom. But only the sample mean is known, which has 1 degree of freedom. So if we compute the SDM of a sample using the _sample mean_ instead of the _population mean_, it has $n - 1$ degrees of freedom.
...
- [statistical hypothesis test](../../../../general/statistical%20hypothesis%20test.md) ::@:: It is a method of _statistical inference_ used to decide whether the data sufficiently supports a particular hypothesis. A statistical hypothesis test typically involves a _calculation of a test statistic_. Then a decision is made, either by comparing the test statistic to a _critical value_ or equivalently by _evaluating a p-value_ computed from the test statistic.
  - statistical hypothesis test / brief history (untaught) ::@:: (untaught) Historically, there were two approaches: Fisher's null hypothesis testing and Neyman–Pearson decision theory. <p> Nowadays, we are basically doing a hybrid of both.
    - statistical hypothesis test / brief history / Fisher's null hypothesis testing (untaught) ::@:: (untaught) It sets up a null hypothesis (but no alternative hypothesis), which need not be a nil (zero difference) hypothesis. <p> Then report the exact level of significance. But in any case, you are neither "accepting" nor "rejecting" the null hypothesis. If the result is not significant, make no decision and suspend judgement: you do not "accept" the null hypothesis. <p> As a result, there is no Type II (false negative) error, since if the result is not significant, you neither "accept" nor "reject".
    - statistical hypothesis test / brief history / Neyman–Pearson decision theory (untaught) ::@:: (untaught) It sets up two hypothesis, H1 and H2. Define a rejection region for each hypothesis. <p> If the data falls into the rejection region of H1, accept H2; otherwise accept H1. Accepting a hypothesis does not mean that you believe in it, but only that you act as if it were true. <p> As a result, there are Type I (false positive) error and Type II (false negative) error.
  - statistical hypothesis ::@:: It is an assertion or conjecture or statement about the population, usually formulated in terms of _population parameters_.
  - [null hypothesis](../../../../general/null%20hypothesis.md) ::@:: It is denoted $H_0$. It is the hypothesis that is assumed to be true and then tested to be rejected or not to be rejected formally. <p> It always contains a "=" sign (i.e. =, ≤, ≥).
  - [alternative hypothesis](../../../../general/alternative%20hypothesis.md) ::@:: It is denoted $H_1$. It is the hypothesis that contains the values of the parameter we would _accept_ if we _reject_ $H_0$. <o> It never contains a "=" sign, except in a _simple test_ (only two possible values for the parameter).
  - statistical hypothesis test / types ::@:: There are 4 types: simple test, one-sided left test (alternative is smaller), one-sided right test (alternative is larger), and two-sided test. <p> We do a two-sided test by default unless we have enough information (e.g. if a parameter being too high is acceptable, then we do a one-sided left test).
...
- [statistical hypothesis test](../../../../general/statistical%20hypothesis%20test.md)
  - statistical hypothesis test / methods ::@:: two _equivalent_ methods: by critical value, by _p_-value
  - statistical hypothesis test / by critical value ::@:: Define $H_0$ and $H_1$. Fix the significance level $\alpha$. Choose a test statistic. Compute the critical value(s). Reject or not reject $H_0$. Use judgement based on if $H_0$ is rejected or not.
  - statistical hypothesis test / by _p_-value ::@:: Define $H_0$ and $H_1$. Choose a test statistic. Compute the _p_-value. Use judgement based on the _p_-value.
...
- [statistical hypothesis test](../../../../general/statistical%20hypothesis%20test.md)
  ...
  - statistical hypothesis test / t-test
    - statistical hypothesis test / t-test / _R_ ::@:: We can use `t.test(<data>, alternative="less"/"greater"/"two.sided", mu=<H0 mean>)`. Also note this methods can also give the 95% confidence interval for the population mean as well. (But it does not have any direct relationship to hypothesis testing.)
...
- [linear regression](../../../../general/linear%20regression.md) ::@:: ... if the relationship is _linear_.
- [simple linear regression](../../../../general/simple%20linear%20regression.md) ::@:: ... if there is only one variable called _explanatory variable_, labeled by _x_, used to explain our target variable _y_.
  - simple linear regression / note ::@:: It is nowhere close to "simple" mathematically...
- [regression analysis](../../../../general/regression%20analysis.md)
  - regression analysis / brief history ::@:: Around the turn of the 20th century, geneticist Francis Galton discovered "regression toward the mean". Later, Sir Francis Galton referred to this as "regression toward mediocrity".
- [scatter plot](../../../../general/scatter%20plot.md) ::@:: It is a type of plot or mathematical diagram using Cartesian coordinates to display values for typically two variables for a set of data.
  - scatter plot / simple linear regression ::@:: In elementary mathematics, $y = mx + c$ is a perfect line on a graph. In statistics, things are never too clean. Data do not perfectly fit onto a line on a scatter plot.
  - scatter plot / _R_ ::@:: `plot(<x>, <y>, main=<title>, ylab=<y label>, xlab=<x label>)`
- [simple linear regression](../../../../general/simple%20linear%20regression.md)
  - simple linear regression / model ::@:: The model function is $$y = \beta_0 + \beta_1 x \,.$$ The actual data has __residual__ $e_i$ and may be modeled as using $$y_i = \beta_0 + \beta_1 x_i + \varepsilon_i \,.$$
  - simple linear regression / fit ::@:: Ordinary least square (OLS) is used to find the best fit line. That is, we _minimize_ $$\sum_{i = 1}^n \varepsilon_i^2 = \sum_{i = 1}^n (y_i - \beta_0 - \beta_1 x_i)^2 \,.$$
...
- [Pearson correlation coefficient](../../../../general/Pearson%20correlation%20coefficient.md) ::@:: It is a _correlation coefficient_ that measures linear correlation between two sets of data. <p> It is the ratio between the covariance of two variables and the product of their standard deviations; thus, it is essentially a normalized measurement of the covariance, such that the result always has a value between −1 and 1.
  - Pearson correlation coefficient / sample ::@:: $$r_{xy} = \frac {S_{XY} } {\sqrt{S_{XX} S_{YY} } } =\frac{\sum ^n _{i=1}(x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\sum ^n _{i=1}(x_i - \bar{x})^2} \sqrt{\sum ^n _{i=1}(y_i - \bar{y})^2} } = \frac {\sum x_i y_i - (\sum x_i) (\sum y_i) / n} {\sqrt{\sum x_i^2 - (\sum x_i)^2 / n} {\sqrt{\sum y_i^2 - (\sum y_i)^2 / n} } }$$ It is undefined when the denominator is 0. (It looks like the dot product of two vectors divided by the two Euclidean lengths of both vectors...)
    - Pearson correlation coefficient / sample / _R_ ::@:: `cor.test(<x>, <y>, method="pearson")$estimate`
  - Pearson correlation coefficient / population ::@:: $$\rho_{X,Y}= \frac{\operatorname{cov}(X,Y)}{\sigma_X \sigma_Y} = \frac {\operatorname E[(X - \mu_X) (Y - \mu_Y)] } {\sigma_X \sigma_Y}$$ It is undefined when the denominator is 0. <p> $r_{xy}$ is an estimator for $\rho_{X, Y}$.
...
- [Fisher transformation](../../../../general/Fisher%20transformation.md) (optional) ::@:: (optional) $$z={1 \over 2}\ln \left({1+r \over 1-r}\right)=\operatorname {arctanh} (r) \,,$$ where $r$ is the sample Pearson correlation coefficient.
  - Fisher transformation / brief history ::@:: R.A. Fisher in 1915 proposed this to a _transformed estimator_ and its _approximated normal distribution_, and then make a _back-transformation_ to produce the confidence interval for $\rho$.
  - Fisher transformation / properties ::@:: If the two random variables being considered is a bivariate normal distribution and the sample pairs are i.i.d., then $z = \frac 1 2 \ln \left(\frac {1 + r} {1 - r}\right)$ is _approximately_ $$\begin{aligned} Z & \sim \mathcal N\left(\frac 1 2 \ln \left(\frac {1 + \rho} {1 - \rho} \right), \frac 1 {N - 3 }\right) \\ \left(Z - \frac 1 2 \ln \left(\frac {1 + \rho} {1 - \rho}\right) \right) \sqrt{N - 3} & \sim \mathcal N(0, 1) \end{aligned} \,$$ where $N$ is the number of samples. (Note, $\frac 1 {N - 3}$ is the _variance_, while $\frac 1 {\sqrt{N - 3} }$ is the _standard deviation_.)
  - Fisher transformation / use ::@:: We can use this to produce a confidence interval for $\rho$.
  - Fisher transformation / back-transformation ::@:: $$r = \frac {\exp(2z) - 1} {\exp(2z) + 1} = \tanh(z)$$
  - Fisher transformation / _R_ ::@:: If you want to find the confidence interval for population Pearson correlation coefficient, use `CorCI(<sample coefficient>, <sample count>, conf.level=<confidence level>)` from the package `DescTools` (install using `install.packages("DescTools")`).
- [total sum of squares](../../../../general/total%20sum%20of%20squares.md) (TSS, SST (use in this course)) ::@:: For a set of observations, $y_{i},i\leq n$, it is defined as the sum over all squared differences between the observations and their overall [mean](mean.md) ${\bar {y} }$: $$\sum _{i=1}^{n}\left(y_{i}-{\bar {y} }\right)^{2} \,.$$
  - total sum of squares / linear regression ::@:: For wide classes of linear models, the total sum of squares equals the _explained sum of squares_ plus the _residual sum of squares_.
- [residual sum of squares](../../../../general/residual%20sum%20of%20squares.md) (RSS (avoid using in this course), SSE (use in this course))
- [explained sum of squares](../../../../general/explained%20sum%20of%20squares.md) (ESS, RSS (use in this course)) ::@:: It is a quantity used in describing how well a model, often a regression model, represents the data being modelled. <br/> It is calculated as: $$\sum _{i=1}^{n}\left({\hat {y} }_{i}-{\bar {y} }\right)^{2} \,,$$ where ${\hat {y} }_{i}$ is the value estimated by the regression line.
...
- [simple linear regression](../../../../general/simple%20linear%20regression.md)
  - simple linear regression / model assumptions ::@:: We assume the errors are: $$\varepsilon_i \sim \mathcal N\left(0, \sigma_\varepsilon^2\right) \quad i = 1, \ldots, n \quad (\text{i.i.d.}) \,.$$ __We assume this is the case henceforth.__
  - simple linear regression / slope estimator
    - simple linear regression / slope estimator / known error variance ::@:: $$\begin{aligned} \hat \beta_1 & \sim \mathcal N\left(\beta_1, \sigma_\varepsilon^2 / S_{XX}\right) \\ (\hat \beta_1 - \beta_1) \sqrt{S_{XX} / \sigma_\varepsilon^2} & \sim \mathcal N(0, 1) \end{aligned}$$
      - simple linear regression / slope estimator / known error variance / proof ::@:: $$\begin{aligned} \operatorname E[\hat \beta_1] & = \operatorname E\left[\frac {\sum_{i = 1}^n (x_i - \overline x) (Y_i - \overline Y)} {\sum_{i = 1}^n (x_i - \overline x)^2}\right] \\ & = \operatorname E\left[\frac {\sum_{i = 1}^n (x_i - \overline x) Y_i} {\sum_{i = 1}^n (x_i - \overline x)^2}\right] \\ & = \frac {\sum_{i = 1}^n (x_i - \overline x) \operatorname E[Y_i]} {\sum_{i = 1}^n (x_i - \overline x)^2} \\ & = \frac {\sum_{i = 1}^n (x_i - \overline x) (\beta_0 + \beta_1 x_i)} {\sum_{i = 1}^n (x_i - \overline x)^2} \\ & = \beta_1 \frac {\sum_{i = 1}^n (x_i - \overline x)^2} {\sum_{i = 1}^n (x_i - \overline x)^2} \\ & = \beta_1 \\ \operatorname{Var}(\hat \beta_1) & = \operatorname{Var}\left(\frac {\sum_{i = 1}^n (x_i - \overline x) (Y_i - \overline Y)} {\sum_{i = 1}^n (x_i - \overline x)^2} \right) \\ & = \operatorname{Var}\left(\frac {\sum_{i = 1}^n (x_i - \overline x) Y_i} {\sum_{i = 1}^n (x_i - \overline x)^2} \right) \\ & = \frac {\sum_{i = 1}^n (x_i - \overline x)^2 \operatorname {Var}(Y_i)} {\left(\sum_{i = 1}^n (x_i - \overline x)^2\right)^2} \\ & = \frac {\sum_{i = 1}^n (x_i - \overline x)^2 \operatorname {Var}(\beta_1 x_i + \varepsilon_i)} {\left(\sum_{i = 1}^n (x_i - \overline x)^2\right)^2} \\ & = \frac {\sigma_\varepsilon^2} {\sum_{i = 1}^n (x_i - \overline x)^2} \\ & = \sigma_\varepsilon^2 / S_{XX} \end{aligned}$$
  - simple linear estimator / intercept estimator
    - simple linear estimator / intercept estimator / known error variance ::@:: $$\begin{aligned} \hat \beta_0 & \sim \mathcal N\left(\beta_0, \overline {x^2} \sigma_\varepsilon^2 / S_{XX}\right) \\ (\hat \beta_0 - \beta_0) \sqrt{S_{XX} / \overline {x^2} \sigma_\varepsilon^2} & \sim \mathcal N(0, 1) \end{aligned}$$
      - simple linear estimator / intercept estimator / known error variance / proof ::@:: $$\begin{aligned} \operatorname E[\hat \beta_0] & = \operatorname E[\overline Y - \hat \beta_1 \overline x] \\ & = \operatorname E[\overline {\beta_0 + \beta_1 x + \varepsilon} - \hat \beta_1 \overline x] \\ & = \beta_0 + \beta_1 \overline x + 0 - \beta_1 \overline x \\ & = \beta_0 \\ \operatorname {Var}(\hat \beta_0) & = \operatorname{Var}(\overline Y - \hat \beta_1 \overline x) \\ & = \operatorname{Var}(\overline{\beta_0 + \beta_1 x + \varepsilon} - \hat \beta_1 \overline x) \\ & = 0 + 0 + \frac {\sigma_\varepsilon^2} {n} + \frac {\overline x^2 \sigma_\varepsilon^2} {S_{XX} } \\ & = \frac {\sigma_\varepsilon^2 \left(\overline {(x_i - \overline x)^2} + \overline x^2 \right)} {S_{XX} } \\ & = \frac {\overline {x^2} \sigma_\varepsilon^2} {S_{XX} } \end{aligned}$$
  - simple linear regression / error variance estimator ::@:: The estimator (the slides use $S^2$) is $$S_{\varepsilon, n - 2}^2 := \frac {\sum_{i = 1}^n (Y_i - \hat Y_i)^2} {n - 2} \,. $$ Its actual value (the slides use $s^2$) can be calculated by: $$s_{\varepsilon, n - 2}^2 = \frac {\sum_{i = 1}^n (y_i - \hat y_i)^2} {n - 2} = \frac {S_{YY} - b S_{XY} } {n - 2} = \frac {\mathrm{SSE} } {n - 2} \,.$$
    - simple linear regression / error variance estimator / properties ::@:: The following holds: $$\frac {(n - 2)s_{\varepsilon, n - 2}^2} {\sigma_{\varepsilon}^2} \sim \chi^2(n - 2) \,.$$ <p> It is an unbiased estimator of $\sigma_\varepsilon^2$.
- [mean squared error](../../../../general/mean%20squared%20error.md) ::@:: It, of an estimator (of a procedure for estimating an unobserved quantity) measures the _average of the squares of the errors_—that is, the average squared difference between the estimated values and the actual value.
...
- [simple linear regression](../../../../general/simple%20linear%20regression.md)
  - simple linear regression / slope estimator
    - simple linear regression / slope estimator / unknown error variance ::@:: $$(\hat \beta_1 - \beta_1) \sqrt{S_{XX} / S_{\varepsilon, n - 2}^2} \sim \mathcal t(n - 2)$$
      - simple linear regression / slope estimator / unknown error variance / confidence interval ::@:: $$b \pm t_{n - 2, \alpha / 2} \sqrt{S_{\varepsilon, n - 2}^2 / S_{XX} }$$
      - simple linear regression / slope estimator / unknown error variance / hypothesis testing ::@:: Compare using $$(b - b_1) \sqrt{S_{XX} / s_{\varepsilon, n - 2}^2}$$ against $t(n - 2)$. Use the absolute value of it if two-sided test.
  - simple linear estimator / intercept estimator
    - simple linear estimator / intercept estimator / unknown error variance ::@:: $$(\hat \beta_0 - \beta_0) \sqrt{S_{XX} / \overline {x^2} S_{\varepsilon, n - 2}^2} \sim \mathcal t(n - 2)$$
      - simple linear regression / intercept estimator / unknown error variance / confidence interval ::@:: $$a \pm t_{n - 2, \alpha / 2} \sqrt{\overline {x^2} S_{\varepsilon, n - 2}^2 / S_{XX} }$$
      - simple linear regression / intercept estimator / unknown error variance / hypothesis testing ::@:: Compare using $$(a - b_0) \sqrt{S_{XX} / \overline {x^2} s_{\varepsilon, n - 2}^2}$$ against $t(n - 2)$. Use the absolute value of it if two-sided test.
  - simple linear estimator / _R_: confidence level of estimators ::@:: If unknown error variance, then use `confint(lm(<y>~<x>), level=<confidence level>))`.
  - simple linear estimator / _R_: hypothesis testing of estimators ::@:: `summary(<model>)`, where `<model>` can be obtained by `lm(<y>~<x>[, data=<dataframe>])`. But note that the null hypothesis are respectively $\beta_0 = 0$ and $\beta_1 = 0$.
  - simple linear estimator / prediction ::@:: $$\hat y_{\mathrm{new} } = a + b x_{\mathrm{new} } \,,$$ which is a _point estimate_ of $y_{\mathrm{new} }$.
