...
- [Cochran's theorem](../../../../general/Cochran's%20theorem.md)
  - Cochran's theorem / special case ::@:: For _n_ samples of a population $X \sim \mathcal N\left(\mu_X, \sigma_X^2\right)$ and its _corresponding_ normalized population $Z \sim \mathcal N(0, 1)$, $\overline X$ and $S_{X, n - 1}^2$ (or $S_{X, n}^2$) are _independent_. Further, we have $$\begin{aligned} \sum_{i = 1}^n (Z_i - \overline Z)^2 & \sim \chi^2(n - 1) \\ \sum_{i = 1}^n \left(\frac {X_i - \overline X} {\sigma_X} \right)^2 = \frac {(n - 1) S_{X, n - 1}^2 } {\sigma_X^2} & \sim \chi^2(n - 1) \,. \end{aligned}$$ (For the second line, $n S_{X, n}^2 = (n - 1) S_{X, n - 1}^2$ would also work.)
    - Cochran's theorem / special case / intuition ::@:: The SSE of a sample of a normal population, given the _population mean_ is known, has _n_ degrees of freedom. But only the sample mean is known, which has 1 degree of freedom. So if we compute the SSE of a sample using the _sample mean_ instead of the _population mean_, it has $n - 1$ degrees of freedom.
...
- [statistical hypothesis test](../../../../general/statistical%20hypothesis%20test.md) ::@:: It is a method of _statistical inference_ used to decide whether the data sufficiently supports a particular hypothesis. A statistical hypothesis test typically involves a _calculation of a test statistic_. Then a decision is made, either by comparing the test statistic to a _critical value_ or equivalently by _evaluating a p-value_ computed from the test statistic.
  ...
  - critical values ::@:: They of a statistical test are the boundaries of the acceptance region of the test. The acceptance region is the set of values of the test statistic for which the null hypothesis is not rejected (not "_accepted_", however, even if the region is called "acceptance region"). Depending on the shape of the acceptance region, there can be one or more than one critical value.
    - critical values / determination ::@:: If we fix the Type I error/false positive $\alpha$, we determine the critical values by determining the distribution of the test statistic (e.g. $\overline X$), and then make the rejection regions cover enough of the distribution so that its area equals $\alpha$, while minimizing $\beta$. <p> For a one-sided test or simple test, there is usually only one critical value, and this usually minimizes $\beta$ automatically. For a two-sided test, there are usually only two critical values, chosen to have equal areas (i.e. $\alpha / 2$) on both sides, and this usually minimizes $\beta$ automatically.
...
- [statistical hypothesis test](../../../../general/statistical%20hypothesis%20test.md)
  ...
  - statistical hypothesis test / t-test
    - statistical hypothesis test / t-test / _R_ ::@:: We can use `t.test(<data>, alternative="less"/"greater"/"two.sided", mu=<H0 mean>)`. Also note this methods can also give the 95% confidence interval for the population mean as well.
...
- [simple linear regression](../../../../general/simple%20linear%20regression.md)
  - simple linear regression / model assumptions ::@:: We assume the errors are: $$\varepsilon_i \sim \mathcal N\left(\beta_1, \sigma_\varepsilon^2\right), i = 1, \ldots, n \quad (\text{i.i.d.}) \,.$$ __We assume this is the case henceforth.__
...
- [simple linear regression](../../../../general/simple%20linear%20regression.md)
  - simple linear regression / slope estimator
    - simple linear regression / slope estimator / unknown error variance ::@:: $$(\hat \beta_1 - \beta_1) \sqrt{S_{XX} / S_{\varepsilon, n - 2}^2} \sim \mathcal t(n - 2)$$
      ...
      - simple linear regression / slope estimator / unknown error variance / hypothesis testing ::@:: Compare using $$(b - b_1) \sqrt{S_{XX} / s_{\varepsilon, n - 2}^2} \,.$$ (use the absolute value of it if two-sided test)
  - simple linear estimator / intercept estimator
    - simple linear estimator / intercept estimator / unknown error variance ::@:: $$(\hat \beta_0 - \beta_0) \sqrt{S_{XX} / \overline {x^2} S_{\varepsilon, n - 2}^2} \sim \mathcal t(n - 2)$$
      ...
      - simple linear regression / intercept estimator / unknown error variance / hypothesis testing ::@:: Compare using $$(a - b_0) \sqrt{S_{XX} / \overline {x^2} s_{\varepsilon, n - 2}^2} \,.$$ (use the absolute value of it if two-sided test)
