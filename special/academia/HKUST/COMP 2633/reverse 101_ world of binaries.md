...
- `jmp <addr>` ::: Jump to `<addr>`. Sometimes, the instruction before it in the assembly program (`.i`) is `jcc`. If `jcc` did not jump due to not satisfying a condition, then this instruction represents the `else` branch.
...
- `syscall` ::: Perform a system call (syscall). A system call interacts with the operating system. The system call invoked depends on `eax`/`rax`, and parameters required by the system call depends on other registers.
  - `syscall` / write ::: `rax` is `1`, `rdi` is the file descriptor to be written to (`1` for stdout), `rsi` is the start address of the string to write, and `rdx` is the length of the string to write (the null terminator is irrelevant here)``
...
- `resb <size>` ::: Allocate `<size>` number of bytes and fill it with zeros. It is commonly used in `.bss`.
- `resd <size>` ::: Allocate `<size>` number of dwords (4 bytes, double word) and fill it with zeros. It is commonly used in `.bss`.
- `resq <size>` ::: Allocate `<size>` number of qwords (8 bytes, quadruple word) and fill it with zeros. It is commonly used in `.bss`.
- `resw <size>` ::: Allocate `<size>` number of word (2 bytes) and fill it with zeros. It is commonly used in `.bss`.
...
x86 and x86-64 makes it easy {{to make stacks (in both the data structure and memory allocation sense) and functions}} by {{providing the instructions `push`, `pop`, `call`, and `ret`}}. Remember that the stack grows {{in the negative direction (decreasing address)}}.
...
- `pop <dest>` ::: Copy the value at the address pointed by `esp`/`rsp` to `<dest>` and then increment `esp`/`rsp` by the data size. The data size is either 4 or 8 bytes depending on the architecture (but not the value at the stack top). It can also be 2 bytes if explicitly specified (`pop word <dest>`). Note that flags are also manipulated.
- `call <addr>` ::: Push (`push`) the address of the next instruction to the stack and jump to `<addr>`. It can help with implementing function calls in higher-level languages. Note that the _stack pointer_ and _base pointer_ are not modified, and needs to be done by the compiler or yourself.
- `ret` ::: Pop (`pop`) the address of an instruction from the stack and jump to it. This is a bit like `pop eip`/`pop rip`, but this is illegal because `eip`/`rip` cannot be modified directly.It can help with implementing function returns in higher-level languages. Note that the _stack pointer_ and _base pointer_ are not modified, and needs to be done by the compiler or yourself.
