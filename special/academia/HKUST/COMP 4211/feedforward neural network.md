...
- alternative name for feedforward neural networks ::@:: Feedforward neural networks are also commonly called multilayer perceptrons (MLPs). <!--SR:!2026-04-12,4,304!2026-04-12,4,270--> The name "multilayer" refers to having at least one hidden layer between input and output, and "perceptron" echoes the historical single-unit predecessor; despite the name, modern MLPs use smooth differentiable activations rather than the original threshold step function.
...
- function view of an FNN ::@:: An FNN defines a parameterized composition $f_{\theta}(x)=\Psi_{L}\circ \cdots \circ \Psi_{1}(x)$, where each layer map has the form $\Psi_{\ell}(u)=g^{(\ell)}((W^{(\ell)})^\top u+b^{(\ell)})$. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- learned feature map plus simple head ::@:: A feedforward neural network can be viewed as a learned feature map $x\mapsto a^{(L-1)}(x)$ followed by a comparatively simple output model on top of that learned representation. <!--SR:!2026-04-12,4,317!2026-04-12,4,312-->
- target function $f^{\ast}(x)$ ::@:: In the function-approximation viewpoint, the network parameters are learned so that $f_{\theta}(x)$ approximates a target function $f^{\ast}(x)$. <!--SR:!2026-04-12,4,317!2026-04-12,4,312-->
- shallow-model relation ::@:: Linear regression, logistic regression, and softmax regression are shallow special cases of the same general pattern: simple output heads on top of either the raw input or a fixed representation. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- reproducing-kernel Hilbert space (RKHS) contrast ::@:: In a reproducing-kernel Hilbert space (RKHS), predictors are linear in a fixed feature map or kernel representation, such as $f(x)=\langle w,\phi(x)\rangle_{\mathcal H}$ or an equivalent kernel expansion; the feature map is chosen before fitting and only the linear coefficients are learned. <br/> In a typical feedforward classifier, the hidden layers learn a representation $\phi_{\theta}(x)=a^{(L-1)}(x)$ from data, and the final head is often an affine or linear classifier on top of those learned features (for example logistic or softmax), though not every architecture uses a purely linear head. <br/> So the similarity is "linear prediction on top of a feature map," but the key difference is that an RKHS keeps the feature map fixed while an FNN learns the feature map jointly with the head by end-to-end training. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
...
- unit computation $z=W^\top x+b$ and $g(z)$ ::@:: A standard neural-network unit computes the pre-activation $z=W^\top x+b$ and then outputs $g(z)$. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- pre-activation or net input ::@:: The quantity $z=W^\top x+b$ is often called the pre-activation or net input of the unit. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- parameters of a standard neural-network unit: A unit computes $z=W^\top x+b$ and outputs $g(z)$. What are the learnable parameters and how many are there for a unit in a layer with $d$ inputs? ::@:: The learnable parameters are the incoming weight vector $W\in\mathbb{R}^d$ (one weight per input) and the scalar bias $b\in\mathbb{R}$, giving $d+1$ parameters in total. During training, gradient descent adjusts these parameters to minimize the overall network loss. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- why repeated local computation creates depth ::@:: Depth arises because each layer takes the previous layer's outputs as its inputs, producing a repeated composition of simple computations. <!--SR:!2026-04-12,4,317!2026-04-12,4,317-->
- affine-collapse proof ::@:: If $f_{1}(x)=A_{1}x+c_{1}$ and $f_{2}(u)=A_{2}u+c_{2}$, then $f_{2}(f_{1}(x))=A_{2}A_{1}x+(A_{2}c_{1}+c_{2})$, so composing affine maps still gives an affine map; induction extends this to any depth. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- why affine maps alone are insufficient ::@:: Without nonlinear activations, a multilayer stack of affine maps would still be just one affine map, so depth would be only a reparameterization rather than a source of new expressive power. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- what nonlinearity adds ::@:: A nonlinear activation lets different input regions be transformed differently, so the network can create curved decision boundaries and hierarchical feature maps. <!--SR:!2026-04-12,4,317!2026-04-12,4,312-->
...
- sigmoid activation: formula, range, and derivative derivation ::@:: The sigmoid activation is $\sigma(z)=\frac{1}{1+e^{-z}}=\frac{e^z}{1+e^z}=\frac{1+\tanh(z/2)}{2}$, so it maps scores to $(0,1)$. <br/> Its derivative is $\sigma'(z)=\sigma(z)(1-\sigma(z))$ because differentiating $(1+e^{-z})^{-1}$ gives $\frac{e^{-z}}{(1+e^{-z})^2}=\frac{1}{1+e^{-z}}\cdot\frac{e^{-z}}{1+e^{-z}}=\sigma(z)(1-\sigma(z))$. <br/> So sigmoid is smooth and probabilistic, but its derivative becomes small in the saturation tails. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- classical activation overview ::@:: Sigmoid and tanh are classical smooth bounded saturating activations: mathematically clean and still useful at output heads, but often harder to optimize in deep hidden stacks than rectifier-style alternatives. <!--SR:!2026-04-12,4,312!2026-04-12,4,317-->
- sigmoid characteristics ::@:: Sigmoid is bounded, monotone, and smooth, but it is not zero-centered and it saturates strongly in the tails, which is why hidden-layer training can be slow. <!--SR:!2026-04-12,4,312!2026-04-12,4,317-->
- tanh activation: formula, zero-centering, and derivative derivation ::@:: The hyperbolic tangent is $\tanh(z)=\sinh(z)/\cosh(z)=\frac{e^z-e^{-z}}{e^z+e^{-z}}=2\sigma(2z)-1$, so it is a zero-centered sigmoid-like activation with range $(-1,1)$. <br/> Its derivative is $\tanh'(z)=1-\tanh^2(z)=\operatorname{sech}^2(z)$ because differentiating $\sinh(z)/\cosh(z)$ by the quotient rule gives $\frac{\cosh^2(z)-\sinh^2(z)}{\cosh^2(z)}=1-\tanh^2(z)$. <br/> So tanh keeps the same smooth saturation idea as sigmoid, but its outputs are centered around $0$. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- why tanh is often preferable to sigmoid ::@:: Tanh is zero-centered and odd, so hidden activations are often easier to optimize than with sigmoid, although tanh still saturates for large $|z|$. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- output-layer saturation nuance ::@:: Vanishing gradient at a sigmoid or tanh output unit is usually less concerning than in hidden layers, because it occurs only once and often means prediction error is already small. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- given $z=1$, compute and compare $\sigma(z)$, $\sigma'(z)$, $\tanh(z)$, and $\tanh'(z)$ ::@:: $\sigma(1)=\frac{1}{1+e^{-1}}\approx 0.731$, $\sigma'(1)=0.731(1-0.731)\approx 0.197$; $\tanh(1)\approx 0.762$, $\tanh'(1)=1-0.762^2\approx 0.419$; tanh has roughly twice the derivative at $z=1$, confirming it flows gradient better in the linear range than sigmoid. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
...
- rectifier family overview ::@:: The rectifier family keeps positive-side behavior close to identity while suppressing negative inputs; the term _rectifier_ means turning signed inputs into mostly nonnegative outputs by clipping or attenuating negatives. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- ReLU activation: formula, piecewise derivative, and corner caveat ::@:: ReLU is $\operatorname{ReLU}(z)=\max\{0,z\}=\frac{z+|z|}{2}=z\mathbf{1}[z>0]$, so it keeps the positive side and clips the negative side to $0$. <br/> Its derivative is $1$ for $z>0$ and $0$ for $z<0$ because each branch is linear on its own side. <br/> At $z=0$ the left and right slopes disagree, so the function is not classically differentiable there and implementations use a subgradient convention. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- why ReLU often learns faster ::@:: ReLU has constant positive-side slope and no positive-side saturation, which usually makes hidden-layer optimization easier than with sigmoid-like activations. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- dead-neuron problem ::@:: A ReLU neuron is dead when it stays in the zero-output region for all training examples and therefore receives no useful gradient updates. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- dead-neuron mitigation by bias initialization ::@:: A common mitigation for dead ReLUs is to initialize biases to small positive values so units start slightly active instead of immediately stuck on the zero side. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- leaky or parametric ReLU: formula, derivative, and why the negative slope helps ::@:: The general form is $g(z,\alpha)=\max\{0,z\}+\alpha\min\{0,z\}=\begin{cases}z,&z\ge 0,\\ \alpha z,&z<0,\end{cases}$, with fixed small $\alpha$ for leaky ReLU and learned $\alpha$ for parametric ReLU. <br/> Its derivative is $1$ for $z>0$ and $\alpha$ for $z<0$ because each branch is linear with that slope. <br/> At $z=0$ it is generally non-differentiable unless $\alpha=1$, so implementations again use a subgradient convention. <br/> The point of the negative-side slope is to leak some gradient on the left and reduce the dead-neuron problem. <!--SR:!2026-04-12,4,317!2026-04-12,4,312-->
- softplus activation: formula, derivative derivation, and relation to sigmoid ::@:: Softplus is $\zeta(z)=\log(1+e^z)=\log(1+e^{-z})+z$, so it is a smooth approximation to ReLU. <br/> Its derivative is $\zeta'(z)=\frac{e^z}{1+e^z}=\sigma(z)$ by direct differentiation of $\log(1+e^z)$. <br/> So softplus is both an antiderivative of sigmoid and a smooth ReLU-like nonlinearity. <!--SR:!2026-04-12,4,312!2026-04-12,4,317-->
- ELU formula, mnemonic, and $\alpha$ intuition ::@:: $\operatorname{ELU}_{\alpha}(z)=\begin{cases}z,&z>0,\\ \alpha(e^z-1),&z\le 0,\end{cases}$ with derivative $1$ for $z>0$ and $\alpha e^z$ for $z<0$; mnemonic: _bend below zero toward a floor_; $\alpha$ controls both the negative saturation level $-\alpha$ and the negative-side slope near zero. <!--SR:!2026-04-12,4,317!2026-04-12,4,312-->
- rectifier-family recall ::@:: ReLU, leaky ReLU, ELU, and softplus all keep an approximately identity positive side; they differ mainly in how they treat the negative side to balance sparsity, smoothness, and gradient flow. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
...
- Swish derivative: Swish is $\operatorname{Swish}_\beta(z)=z\sigma(\beta z)$ where $\sigma(t)=\frac{1}{1+e^{-t}}$. Compute $\operatorname{Swish}_\beta'(z)$ using the product rule. ::@:: By the product rule on $z\cdot\sigma(\beta z)$: $\operatorname{Swish}_\beta'(z)=\sigma(\beta z)+z\cdot\frac{d}{dz}\sigma(\beta z)=\sigma(\beta z)+z\cdot\beta\sigma(\beta z)(1-\sigma(\beta z))$. At $\beta=1$ this reduces to the SiLU derivative $\sigma(z)+z\sigma(z)(1-\sigma(z))$. <!--SR:!2026-04-12,4,270!2026-04-12,4,307-->
...
- given binary label $y=1$ and logit $z=0.5$, compute the binary cross-entropy loss $-\log\sigma(z)$ ::@:: $\sigma(0.5)\approx 0.622$; loss $=-\log(0.622)\approx 0.476$. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
...
- given logits $z=(1,2,0)$ and one-hot target $t=(0,1,0)$ (class $2$ is correct), compute the softmax probabilities and multiclass cross-entropy loss ::@:: $Z=e^1+e^2+e^0\approx 2.718+7.389+1.000=11.107$; $p_1\approx 0.245$, $p_2\approx 0.665$, $p_3\approx 0.090$; loss $=-\log(p_2)\approx 0.407$. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
