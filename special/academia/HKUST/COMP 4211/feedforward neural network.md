...
- fan-in and fan-out ::@:: Fan-in $n_{\mathrm{in}}$ is the number of inputs feeding one unit in a layer, and fan-out $n_{\mathrm{out}}$ is the number of outputs that layer sends forward to the next layer. <!--SR:!2026-04-12,4,312!2026-04-12,4,317-->
- variance heuristic derivation: For one unit $j$ in layer $\ell$, let $z_j^{(\ell)}=\sum_{i=1}^{n_{\mathrm{in}}}W_{ij}^{(\ell)}a_i^{(\ell-1)}+b_j^{(\ell)}$. Assume the terms are independent across $i$, each $W_{ij}^{(\ell)}$ and $a_i^{(\ell-1)}$ is centered, the pairs $(W_{ij}^{(\ell)},a_i^{(\ell-1)})$ have the same distribution across $i$, and the bias variance at initialization is negligible. Derive the heuristic formula for $\operatorname{Var}(z_j^{(\ell)})$. ::@:: Variance here means the squared spread around the mean. <br/> Step 1: start from $z_j^{(\ell)}=\sum_i W_{ij}^{(\ell)}a_i^{(\ell-1)}+b_j^{(\ell)}$, so $\operatorname{Var}(z_j^{(\ell)})=\operatorname{Var}\!\left(\sum_i W_{ij}^{(\ell)}a_i^{(\ell-1)}\right)+\operatorname{Var}(b_j^{(\ell)})$. <br/> Step 2: under the independence assumption across $i$, the variance of a sum is the sum of variances, so $\operatorname{Var}(z_j^{(\ell)})\approx\sum_i\operatorname{Var}(W_{ij}^{(\ell)}a_i^{(\ell-1)})+\operatorname{Var}(b_j^{(\ell)})$. <br/> Step 3: for one term, independence and zero means give $\operatorname{Var}(W_{ij}^{(\ell)}a_i^{(\ell-1)})=\mathbb E[(W_{ij}^{(\ell)})^2(a_i^{(\ell-1)})^2]-(\mathbb E[W_{ij}^{(\ell)}a_i^{(\ell-1)}])^2=\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})$. <br/> Step 4: because all $i$-terms share the same distribution, the sum becomes $n_{\mathrm{in}}\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})$. <br/> Ignoring the usually tiny bias-variance term at initialization gives the heuristic $\operatorname{Var}(z_j^{(\ell)})\approx n_{\mathrm{in}}\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})$. <!--SR:!2026-04-12,4,317!2026-04-12,4,317-->
- too-small initial weights ::@:: If initial weights are too small, the activation and gradient scales shrink from layer to layer until they become too weak to support useful learning. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- too-large initial weights ::@:: If initial weights are too large, the activation and gradient scales grow from layer to layer until they become too large for stable learning. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- Xavier initialization ::@:: Xavier initialization balances fan-in and fan-out with variance of order $\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}$, using either a normal form $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}\bigr)$ or a variance-matched uniform form with bounds $\pm\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}$. <!--SR:!2026-04-12,4,312!2026-04-12,4,317-->
- Xavier mnemonic ::@:: A mnemonic for Xavier initialization is _balance both sides_: average fan-in and fan-out to keep scale stable across the layer. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- normal versus uniform initialization intuition ::@:: The normal variant directly sets a target variance, while the uniform variant chooses bounds so the variance matches that target and very large draws, typically more than three standard deviations away, are excluded by bounded support. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- normal-form versus bounded-uniform intuition for Xavier and He ::@:: For both Xavier and He, the normal form states the target variance directly; the uniform form is a variance-matched bounded surrogate often interpreted as clipping unlikely normal tails (roughly beyond the three-standard-deviation region) while keeping controlled spread. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- course-slide Xavier convention ::@:: The course slides list a Xavier-style uniform rule $\operatorname{Unif}\bigl[-\sqrt{1/n_{\mathrm{in}}},\sqrt{1/n_{\mathrm{in}}}\bigr]$; although this is likely a simplified or inconsistent variant, use the course-stated form in assignments or exams when explicitly requested. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- He initialization ::@:: He initialization for ReLU-like activations uses variance of order $\frac{2}{n_{\mathrm{in}}}$, commonly in normal form $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}}\bigr)$ (the form emphasized in this course), with a variance-matched uniform alternative also possible. <!--SR:!2026-04-12,4,312!2026-04-12,4,317-->
- He mnemonic ::@:: A mnemonic for He initialization is _half-alive ReLUs_: because rectifiers often keep only about half the inputs active, the variance is doubled relative to the naive $1/n_{\mathrm{in}}$ scaling. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
- why initialization schemes exist ::@:: Initialization schemes are designed to preserve reasonable activation and gradient scale across layers rather than to provide arbitrary starting values. <!--SR:!2026-04-12,4,312!2026-04-12,4,312-->
