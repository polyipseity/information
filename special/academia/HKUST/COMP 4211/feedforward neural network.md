...
- overview ::@:: A feedforward neural network is a layered model that maps inputs to outputs by repeated affine-transform-plus-activation steps, such as $z^{(\ell)}=(W^{(\ell)})^\top a^{(\ell-1)}+b^{(\ell)}$ and $a^{(\ell)}=g^{(\ell)}(z^{(\ell)})$, without recurrent loops in one forward pass. <!--SR:!2026-07-15,60,310!2026-05-21,18,327-->
- feedforward neural network versus earlier models ::@:: Earlier models often used hand-crafted features, whereas an FNN learns the feature transformation jointly with the prediction rule. <!--SR:!2026-05-20,17,309!2026-05-21,17,309-->
- alternative name for feedforward neural networks ::@:: Feedforward neural networks are also commonly called multilayer perceptrons (MLPs).  The name "multilayer" refers to having at least one hidden layer between input and output, and "perceptron" echoes the historical single-unit predecessor; despite the name, modern MLPs use smooth differentiable activations rather than the original threshold step function. <!--SR:!2026-05-21,18,324!2026-05-17,14,290-->
- deep-learning transition ::@:: In the deep-learning viewpoint, one first learns a representation $h=f(x)$ and then models the output distribution conditioned on that learned representation. <!--SR:!2026-05-21,18,324!2026-05-20,17,309-->
- simpler-model special case ::@:: If the learned representation is just the raw input or if there is no hidden layer, an FNN reduces to a familiar shallow model such as linear regression, logistic regression, or softmax regression. <!--SR:!2026-05-20,17,309!2026-05-22,18,327-->
...
- function view of an FNN ::@:: An FNN defines a parameterized composition $f_{\theta}(x)=\Psi_{L}\circ \cdots \circ \Psi_{1}(x)$, where each layer map has the form $\Psi_{\ell}(u)=g^{(\ell)}((W^{(\ell)})^\top u+b^{(\ell)})$. <!--SR:!2026-05-21,18,332!2026-05-19,18,332-->
- learned feature map plus simple head ::@:: A feedforward neural network can be viewed as a learned feature map $x\mapsto a^{(L-1)}(x)$ followed by a comparatively simple output model on top of that learned representation. <!--SR:!2026-05-22,19,337!2026-05-21,18,332-->
- target function $f^{\ast}(x)$ ::@:: In the function-approximation viewpoint, the network parameters are learned so that $f_{\theta}(x)$ approximates a target function $f^{\ast}(x)$. <!--SR:!2026-05-20,19,337!2026-05-22,18,332-->
- shallow-model relation ::@:: Linear regression, logistic regression, and softmax regression are shallow special cases of the same general pattern: simple output heads on top of either the raw input or a fixed representation. <!--SR:!2026-05-21,18,332!2026-05-22,18,332-->
- reproducing-kernel Hilbert space (RKHS) contrast ::@:: In a reproducing-kernel Hilbert space (RKHS), predictors are linear in a fixed feature map or kernel representation, such as $f(x)=\langle w,\phi(x)\rangle_{\mathcal H}$ or an equivalent kernel expansion; the feature map is chosen before fitting and only the linear coefficients are learned. <br/> In a typical feedforward classifier, the hidden layers learn a representation $\phi_{\theta}(x)=a^{(L-1)}(x)$ from data, and the final head is often an affine or linear classifier on top of those learned features (for example logistic or softmax), though not every architecture uses a purely linear head. <br/> So the similarity is "linear prediction on top of a feature map," but the key difference is that an RKHS keeps the feature map fixed while an FNN learns the feature map jointly with the head by end-to-end training. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
...
- unit computation $z=W^\top x+b$ and $g(z)$ ::@:: A standard neural-network unit computes the pre-activation $z=W^\top x+b$ and then outputs $g(z)$. <!--SR:!2026-05-21,18,332!2026-05-22,18,332-->
- pre-activation or net input ::@:: The quantity $z=W^\top x+b$ is often called the pre-activation or net input of the unit. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
- parameters of a standard neural-network unit: A unit computes $z=W^\top x+b$ and outputs $g(z)$. What are the learnable parameters and how many are there for a unit in a layer with $d$ inputs? ::@:: The learnable parameters are the incoming weight vector $W\in\mathbb{R}^d$ (one weight per input) and the scalar bias $b\in\mathbb{R}$, giving $d+1$ parameters in total. During training, gradient descent adjusts these parameters to minimize the overall network loss. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
- why repeated local computation creates depth ::@:: Depth arises because each layer takes the previous layer's outputs as its inputs, producing a repeated composition of simple computations. <!--SR:!2026-05-20,19,337!2026-05-23,19,337-->
- affine-collapse proof ::@:: If $f_{1}(x)=A_{1}x+c_{1}$ and $f_{2}(u)=A_{2}u+c_{2}$, then $f_{2}(f_{1}(x))=A_{2}A_{1}x+(A_{2}c_{1}+c_{2})$, so composing affine maps still gives an affine map; induction extends this to any depth. <!--SR:!2026-05-19,18,332!2026-05-22,18,332-->
- why affine maps alone are insufficient ::@:: Without nonlinear activations, a multilayer stack of affine maps would still be just one affine map, so depth would be only a reparameterization rather than a source of new expressive power. <!--SR:!2026-05-22,18,332!2026-05-19,18,332-->
- what nonlinearity adds ::@:: A nonlinear activation lets different input regions be transformed differently, so the network can create curved decision boundaries and hierarchical feature maps. <!--SR:!2026-05-22,19,337!2026-05-22,18,332-->
...
- layerwise notation $a^{(0)}, z^{(\ell)}, a^{(\ell)}$ ::@:: In layerwise notation, $a^{(0)}=x$, each pre-activation is $z^{(\ell)}=(W^{(\ell)})^\top a^{(\ell-1)}+b^{(\ell)}$, and each activation is $a^{(\ell)}=g^{(\ell)}(z^{(\ell)})$. <!--SR:!2026-05-22,18,324!2026-05-20,16,290-->
- first hidden layer formula ::@:: A first hidden layer can be written as $z^{(1)}=(W^{(1)})^\top x+b^{(1)}$ and $a^{(1)}=g^{(1)}(z^{(1)})$. <!--SR:!2026-05-21,17,309!2026-05-20,17,309-->
- recursive forward propagation ::@:: Forward propagation applies the same pattern repeatedly: matrix mixing plus bias shift to get $z^{(\ell)}$, then activation to get $a^{(\ell)}$, then pass $a^{(\ell)}$ forward. <!--SR:!2026-05-21,17,309!2026-05-21,17,309-->
- why weight matrices are convenient ::@:: Matrix notation collects the weights of many units in one object, making forward propagation compact and efficient. <!--SR:!2026-05-21,17,309!2026-05-21,18,327-->
- computational rhythm of forward propagation ::@:: Forward propagation proceeds as pre-activation $\to$ activation $\to$ next layer, repeated until the final output is produced. <!--SR:!2026-05-18,17,309!2026-05-22,18,327-->
...
- forward-pass mnemonic ::@:: A useful forward-pass mnemonic is _mix, shift, bend, repeat_: $W^{(\ell)}$ mixes coordinates, $b^{(\ell)}$ shifts thresholds, $g^{(\ell)}$ bends the representation, and the result is passed to the next layer. <!--SR:!2026-05-22,18,327!2026-05-21,17,309-->
- matrix interpretation in an FNN ::@:: The weight matrix recombines coordinates from the previous layer, so each new unit can detect a learned direction or pattern rather than copying a single old coordinate. <!--SR:!2026-05-20,17,309!2026-05-19,16,290-->
- hierarchy of representations ::@:: The repeated-map view makes it clear that depth means applying the same affine-plus-nonlinearity template to increasingly abstract intermediate representations. <!--SR:!2026-05-21,17,309!2026-05-21,18,327-->
...
- given $x=\begin{bmatrix}1\\0\end{bmatrix}$, $W^{(1)}=\begin{bmatrix}2&-1\\1&3\end{bmatrix}$, $b^{(1)}=\begin{bmatrix}0.5\\0.5\end{bmatrix}$, $g^{(1)}=\operatorname{ReLU}$, compute $z^{(1)}$ and $a^{(1)}$ ::@:: $z^{(1)}=(W^{(1)})^\top x+b^{(1)}=\begin{bmatrix}2\\-1\end{bmatrix}+\begin{bmatrix}0.5\\0.5\end{bmatrix}=\begin{bmatrix}2.5\\-0.5\end{bmatrix}$; $a^{(1)}=\operatorname{ReLU}(z^{(1)})=\begin{bmatrix}2.5\\0\end{bmatrix}$, so the second unit is switched off. <!--SR:!2026-05-22,19,337!2026-05-21,18,332-->
- continuing from $a^{(1)}=\begin{bmatrix}2.5\\0\end{bmatrix}$, $W^{(2)}=\begin{bmatrix}1\\-1\end{bmatrix}$, $b^{(2)}=0$, identity output, compute $z^{(2)}$ and the network output $f_\theta(x)$ ::@:: $z^{(2)}=(W^{(2)})^\top a^{(1)}+b^{(2)}=\begin{bmatrix}1&-1\end{bmatrix}\begin{bmatrix}2.5\\0\end{bmatrix}=2.5$; identity output gives $f_\theta(x)=2.5$. <!--SR:!2026-05-21,18,332!2026-05-22,18,332-->
...
- universal approximation statement ::@:: A one-hidden-layer sigmoid network is a universal approximator in the existence sense: for continuous $f^{\ast}$ on compact $K$ and any $\varepsilon>0$, some sufficiently wide shallow network can achieve $\sup_{x\in K}|f_{\theta}(x)-f^{\ast}(x)|<\varepsilon$. <!--SR:!2026-07-03,49,327!2026-05-21,17,309-->
- why universal approximation is not the end of the story ::@:: Universal approximation is only an existence statement and does not imply that shallow networks are the most efficient or easiest to train. <!--SR:!2026-05-22,18,327!2026-05-21,18,324-->
- why depth can reduce parameter count ::@:: Depth can exploit compositional structure by reusing intermediate features, so some targets that are representable by shallow networks may require far fewer parameters when represented deeply. <!--SR:!2026-05-20,17,309!2026-05-21,17,309-->
- shallow-versus-deep lesson ::@:: The practical value of depth is not just representability, but also parameter efficiency and more structured representation learning. <!--SR:!2026-05-21,17,309!2026-05-20,17,309-->
...
- rigorous meaning of signal ::@:: In deep-learning initialization discussions, _signal_ means the scale of forward pre-activations/activations and backward gradients, often summarized by statistics such as mean, variance, or typical magnitude across layers. <!--SR:!2026-05-21,17,309!2026-05-22,18,327-->
- why initialization matters ::@:: Initialization matters because forward activations and backward gradients can vanish if weights are too small and explode if weights are too large. <!--SR:!2026-05-22,18,327!2026-05-22,18,327-->
- activation-initialization linkage ::@:: Initialization and activation choice must work together so that activations and gradients remain numerically useful across depth. <!--SR:!2026-05-22,18,324!2026-05-21,18,327-->

### why initialization matters

Let $n_{\mathrm{in}}$ denote fan-in (the number of inputs feeding one unit in the current layer) and $n_{\mathrm{out}}$ denote fan-out (the number of outputs this layer sends to the next layer). Starting from $z_j^{(\ell)}=\sum_{i=1}^{n_{\mathrm{in}}} W_{ij}^{(\ell)} a_i^{(\ell-1)}+b_j^{(\ell)}$, assume for a heuristic derivation that the pairs $(W_{ij}^{(\ell)},a_i^{(\ell-1)})$ are mutually independent across $i$, each weight is independent of its matching activation, both families are centered, and the variances are shared across $i$. For intuition one may imagine centered symmetric families such as Gaussian draws or uniform draws about $0$; the variance heuristic itself does not depend on the exact shape beyond those mean/variance assumptions. Then $\mathbb{E}[z_j^{(\ell)}]\approx 0$ and $\operatorname{Var}(z_j^{(\ell)})=\operatorname{Var}\!\left(\sum_i W_{ij}^{(\ell)}a_i^{(\ell-1)}\right)+\operatorname{Var}(b_j^{(\ell)})\approx\sum_i\operatorname{Var}(W_{ij}^{(\ell)}a_i^{(\ell-1)})+\operatorname{Var}(b_j^{(\ell)})\approx n_{\mathrm{in}}\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})+\operatorname{Var}(b_j^{(\ell)})$. Ignoring small-bias variance at initialization gives the standard rule of thumb $\operatorname{Var}(z_j^{(\ell)})\approx n_{\mathrm{in}}\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})$.
...
The lecture highlights two practical initialization schemes. Xavier initialization is designed for roughly symmetric activations such as sigmoid and tanh and uses variance of order $\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}$, for example $W_{ij}\sim \mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}\bigr)$ or $W_{ij}\sim \operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}},\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}\bigr]$. The normal form directly sets the target variance. For the uniform form, if $X\sim\operatorname{Unif}[-r,r]$, then $\operatorname{Var}(X)=\frac{(2r)^2}{12}=\frac{r^2}{3}$. So to match a target variance $\sigma^2$, one chooses $r=\sqrt{3}\,\sigma$. With Xavier's target $\sigma^2=\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}$, this gives $r=\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}$. A useful mnemonic is that Xavier _balances both sides_ of the layer by averaging fan-in and fan-out.

The course slides also present a Xavier-style uniform rule $W_{ij}\sim\operatorname{Unif}\bigl[-\sqrt{1/n_{\mathrm{in}}},\sqrt{1/n_{\mathrm{in}}}\bigr]$. Relative to the standard Xavier formulas, this is likely a simplified or inconsistent convention. The standard Xavier pair is the normal convention $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}\bigr)$ together with the variance-matched uniform convention $\operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}},\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}\bigr]$. However, for assignments and examinations one should follow the course-stated convention when explicitly asked.

He (Kaiming) initialization is adapted to rectifier-style activations such as ReLU and uses a larger variance, typically $\operatorname{Var}(W_{ij})\approx \frac{2}{n_{\mathrm{in}}}$, for example $W_{ij}\sim \mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}}\bigr)$ or $W_{ij}\sim \operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}}},\sqrt{\frac{6}{n_{\mathrm{in}}}}\bigr]$. This is the same variance-matching logic as above: with target variance $\sigma^2=\frac{2}{n_{\mathrm{in}}}$, the corresponding uniform bound is again $r=\sqrt{3}\,\sigma=\sqrt{\frac{6}{n_{\mathrm{in}}}}$. The mnemonic here is _He for half-alive ReLUs_: because a ReLU often zeros out about half of its inputs, one compensates with a larger variance so surviving activations do not become too small. In the course material, He initialization is presented in its normal-variance form, which is the version to use by default in course calculations.

For both Xavier and He, the normal-distribution formulas are the direct variance-target statements, while the uniform alternatives are variance-matched bounded surrogates. In summary, the four concrete distributions are Xavier normal $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}\bigr)$, Xavier uniform $\operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}},\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}\bigr]$, He normal $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}}\bigr)$, and He uniform $\operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}}},\sqrt{\frac{6}{n_{\mathrm{in}}}}\bigr]$. A common intuition is that in a normal distribution, values beyond roughly $3\sigma$ are rare, so a bounded initializer can be viewed as clipping unlikely tails while keeping a controlled spread; the exact interval is then chosen to match the target variance.
...
- fan-in and fan-out ::@:: Fan-in $n_{\mathrm{in}}$ is the number of incoming activations/weights feeding one neuron, so it is the width of the previous layer as seen by that neuron. Fan-out $n_{\mathrm{out}}$ is the number of downstream units this layer feeds, so it is the width of the next layer as seen from the current one. They matter because initialization tries to keep signal scale stable when information moves across those two widths. <!--SR:!2026-05-20,19,337!2026-05-23,19,337-->
- variance heuristic derivation: For one unit $j$ in layer $\ell$, let $z_j^{(\ell)}=\sum_{i=1}^{n_{\mathrm{in}}}W_{ij}^{(\ell)}a_i^{(\ell-1)}+b_j^{(\ell)}$. Assume the pairs $(W_{ij}^{(\ell)},a_i^{(\ell-1)})$ are mutually independent across $i$, each weight is independent of its matching activation, all those variables are centered, the variances are shared across $i$, and the bias variance at initialization is negligible. Derive the heuristic formula for $\operatorname{Var}(z_j^{(\ell)})$ and state what these assumptions buy you. ::@:: Variance here means the squared spread around the mean. <br/> Step 1: start from $z_j^{(\ell)}=\sum_i W_{ij}^{(\ell)}a_i^{(\ell-1)}+b_j^{(\ell)}$, so $\operatorname{Var}(z_j^{(\ell)})=\operatorname{Var}\!\left(\sum_i W_{ij}^{(\ell)}a_i^{(\ell-1)}\right)+\operatorname{Var}(b_j^{(\ell)})$. <br/> Step 2: under mutual independence across $i$, cross-covariances vanish and the variance of a sum becomes the sum of variances, so $\operatorname{Var}(z_j^{(\ell)})\approx\sum_i\operatorname{Var}(W_{ij}^{(\ell)}a_i^{(\ell-1)})+\operatorname{Var}(b_j^{(\ell)})$. <br/> Step 3: for one term, independence and zero means give $\operatorname{Var}(W_{ij}^{(\ell)}a_i^{(\ell-1)})=\mathbb E[(W_{ij}^{(\ell)})^2(a_i^{(\ell-1)})^2]-(\mathbb E[W_{ij}^{(\ell)}a_i^{(\ell-1)}])^2=\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})$. <br/> Step 4: because all $i$-terms have the same variance, the sum becomes $n_{\mathrm{in}}\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})$. <br/> Ignoring the usually tiny bias-variance term at initialization gives the heuristic $\operatorname{Var}(z_j^{(\ell)})\approx n_{\mathrm{in}}\operatorname{Var}(W_{ij}^{(\ell)})\operatorname{Var}(a_i^{(\ell-1)})$. <br/> The assumptions therefore buy you exactly the simplifications that make the cross-terms disappear and make each summand contribute the same amount. <!--SR:!2026-05-23,19,337!2026-07-05,51,337-->
- too-small initial weights ::@:: If initial weights are too small, the activation and gradient scales shrink from layer to layer until they become too weak to support useful learning. <!--SR:!2026-05-23,19,337!2026-05-23,19,337-->
- too-large initial weights ::@:: If initial weights are too large, the activation and gradient scales grow from layer to layer until they become too large for stable learning. <!--SR:!2026-05-20,19,337!2026-05-20,19,337-->
- Xavier initialization ::@:: Xavier initialization balances fan-in and fan-out with variance of order $\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}$, using either a normal form $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}\bigr)$ or a variance-matched uniform form with bounds $\pm\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}$. <!--SR:!2026-05-23,19,337!2026-05-20,19,337-->
- Xavier mnemonic ::@:: A mnemonic for Xavier initialization is _balance both sides_: average fan-in and fan-out to keep scale stable across the layer. <!--SR:!2026-05-22,19,337!2026-05-22,19,337-->
- normal versus uniform initialization intuition ::@:: The normal variant directly sets a target variance $\sigma^2$. For $X\sim\operatorname{Unif}[-r,r]$, $\operatorname{Var}(X)=\frac{(2r)^2}{12}=\frac{r^2}{3}$, so matching the same target variance means choosing $r=\sqrt{3}\,\sigma$. That is why the Xavier normal variance $\sigma^2=\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}$ becomes the Xavier uniform bounds $\pm\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}$, and the He normal variance $\sigma^2=\frac{2}{n_{\mathrm{in}}}$ becomes the He uniform bounds $\pm\sqrt{\frac{6}{n_{\mathrm{in}}}}$. <!--SR:!2026-05-23,19,337!2026-05-23,19,337-->
- initialization distributions to remember ::@:: The four standard formulas in this note are Xavier normal $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}\bigr)$, Xavier uniform $\operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}},\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}\bigr]$, He normal $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}}\bigr)$, and He uniform $\operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}}},\sqrt{\frac{6}{n_{\mathrm{in}}}}\bigr]$. <!--SR:!2026-05-23,19,337!2026-05-22,19,337-->
- normal-form versus bounded-uniform intuition for Xavier and He ::@:: For both Xavier and He, the normal form states the target variance directly; the uniform form is a variance-matched bounded surrogate. The conversion factor is always $r=\sqrt{3}\,\sigma$ because $\operatorname{Var}(\operatorname{Unif}[-r,r])=r^2/3$. One can then interpret the bounded version as keeping the same variance while excluding very large normal-style tail draws. <!--SR:!2026-05-22,19,337!2026-05-22,19,337-->
- course-slide Xavier convention ::@:: The course slides list a Xavier-style uniform rule $\operatorname{Unif}\bigl[-\sqrt{1/n_{\mathrm{in}}},\sqrt{1/n_{\mathrm{in}}}\bigr]$. Compare it with the standard Xavier conventions: $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}+n_{\mathrm{out}}}\bigr)$ in normal form and $\operatorname{Unif}\bigl[-\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}},\sqrt{\frac{6}{n_{\mathrm{in}}+n_{\mathrm{out}}}}\bigr]$ in uniform form. The course version keeps only the fan-in term, so it is best treated as a course-specific simplified convention to follow when explicitly requested. <!--SR:!2026-05-20,19,337!2026-05-22,19,337-->
- He initialization ::@:: He initialization for ReLU-like activations uses variance of order $\frac{2}{n_{\mathrm{in}}}$, commonly in normal form $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}}\bigr)$ (the form emphasized in this course), with a variance-matched uniform alternative also possible. <!--SR:!2026-07-05,51,337!2026-05-23,19,337-->
- He mnemonic ::@:: A mnemonic for He initialization is _half-alive ReLUs_: because rectifiers often keep only about half the inputs active, the variance is doubled relative to the naive $1/n_{\mathrm{in}}$ scaling. <!--SR:!2026-05-22,19,337!2026-05-23,19,337-->
- why initialization schemes exist ::@:: Initialization schemes are designed to preserve reasonable activation and gradient scale across layers rather than to provide arbitrary starting values. <!--SR:!2026-05-22,19,337!2026-05-23,19,337-->
...
- sigmoid activation: formula, range, and derivative derivation ::@:: The sigmoid activation is $\sigma(z)=\frac{1}{1+e^{-z}}=\frac{e^z}{1+e^z}=\frac{1+\tanh(z/2)}{2}$, so it maps scores to $(0,1)$. <br/> Its derivative is $\sigma'(z)=\sigma(z)(1-\sigma(z))$ because differentiating $(1+e^{-z})^{-1}$ gives $\frac{e^{-z}}{(1+e^{-z})^2}=\frac{1}{1+e^{-z}}\cdot\frac{e^{-z}}{1+e^{-z}}=\sigma(z)(1-\sigma(z))$. <br/> So sigmoid is smooth and probabilistic, but its derivative becomes small in the saturation tails. <!--SR:!2026-07-02,48,332!2026-05-21,18,332-->
- classical activation overview ::@:: Sigmoid and tanh are classical smooth bounded saturating activations: mathematically clean and still useful at output heads, but often harder to optimize in deep hidden stacks than rectifier-style alternatives. <!--SR:!2026-05-22,18,332!2026-05-23,19,337-->
- sigmoid characteristics ::@:: Sigmoid is bounded, monotone, and smooth, but it is not zero-centered and it saturates strongly in the tails, which is why hidden-layer training can be slow. <!--SR:!2026-05-22,18,332!2026-05-23,19,337-->
- tanh activation: formula, zero-centering, and derivative derivation ::@:: The hyperbolic tangent is $\tanh(z)=\sinh(z)/\cosh(z)=\frac{e^z-e^{-z}}{e^z+e^{-z}}=2\sigma(2z)-1$, so it is a zero-centered sigmoid-like activation with range $(-1,1)$. <br/> Its derivative is $\tanh'(z)=1-\tanh^2(z)=\operatorname{sech}^2(z)$ because differentiating $\sinh(z)/\cosh(z)$ by the quotient rule gives $\frac{\cosh^2(z)-\sinh^2(z)}{\cosh^2(z)}=1-\tanh^2(z)$. <br/> So tanh keeps the same smooth saturation idea as sigmoid, but its outputs are centered around $0$. <!--SR:!2026-05-19,18,332!2026-07-02,48,332-->
- why tanh is often preferable to sigmoid ::@:: Tanh is zero-centered and odd, so hidden activations are often easier to optimize than with sigmoid, although tanh still saturates for large $|z|$. <!--SR:!2026-05-21,18,332!2026-05-22,18,332-->
- output-layer saturation nuance ::@:: Vanishing gradient at a sigmoid or tanh output unit is usually less concerning than in hidden layers, because it occurs only once and often means prediction error is already small. <!--SR:!2026-05-21,18,332!2026-05-22,18,332-->
- given $z=1$, compute and compare $\sigma(z)$, $\sigma'(z)$, $\tanh(z)$, and $\tanh'(z)$ ::@:: $\sigma(1)=\frac{1}{1+e^{-1}}\approx 0.731$, $\sigma'(1)=0.731(1-0.731)\approx 0.197$; $\tanh(1)\approx 0.762$, $\tanh'(1)=1-0.762^2\approx 0.419$; tanh has roughly twice the derivative at $z=1$, confirming it flows gradient better in the linear range than sigmoid. <!--SR:!2026-05-22,18,332!2026-05-21,18,332-->
...
- rectifier family overview ::@:: The rectifier family keeps positive-side behavior close to identity while suppressing negative inputs; the term _rectifier_ means turning signed inputs into mostly nonnegative outputs by clipping or attenuating negatives. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
- ReLU activation: formula, piecewise derivative, and corner caveat ::@:: ReLU is $\operatorname{ReLU}(z)=\max\{0,z\}=\frac{z+|z|}{2}=z\mathbf{1}[z>0]$, so it keeps the positive side and clips the negative side to $0$. <br/> Its derivative is $1$ for $z>0$ and $0$ for $z<0$ because each branch is linear on its own side. <br/> At $z=0$ the left and right slopes disagree, so the function is not classically differentiable there and implementations use a subgradient convention. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
- why ReLU often learns faster ::@:: ReLU has constant positive-side slope and no positive-side saturation, which usually makes hidden-layer optimization easier than with sigmoid-like activations. <!--SR:!2026-05-21,18,332!2026-05-21,18,332-->
- dead-neuron problem ::@:: A ReLU neuron is dead when it stays in the zero-output region for all training examples and therefore receives no useful gradient updates. <!--SR:!2026-05-22,18,332!2026-05-19,18,332-->
- dead-neuron mitigation by bias initialization ::@:: A common mitigation for dead ReLUs is to initialize biases to small positive values so units start slightly active instead of immediately stuck on the zero side. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
- leaky or parametric ReLU: formula, derivative, and why the negative slope helps ::@:: The general form is $g(z,\alpha)=\max\{0,z\}+\alpha\min\{0,z\}=\begin{cases}z,&z\ge 0,\\ \alpha z,&z<0,\end{cases}$, with fixed small $\alpha$ for leaky ReLU and learned $\alpha$ for parametric ReLU. <br/> Its derivative is $1$ for $z>0$ and $\alpha$ for $z<0$ because each branch is linear with that slope. <br/> At $z=0$ it is generally non-differentiable unless $\alpha=1$, so implementations again use a subgradient convention. <br/> The point of the negative-side slope is to leak some gradient on the left and reduce the dead-neuron problem. <!--SR:!2026-05-23,19,337!2026-05-22,18,332-->
- softplus activation: formula, derivative derivation, and relation to sigmoid ::@:: Softplus is $\zeta(z)=\log(1+e^z)=\log(1+e^{-z})+z$, so it is a smooth approximation to ReLU. <br/> Its derivative is $\zeta'(z)=\frac{e^z}{1+e^z}=\sigma(z)$ by direct differentiation of $\log(1+e^z)$. <br/> So softplus is both an antiderivative of sigmoid and a smooth ReLU-like nonlinearity. <!--SR:!2026-05-21,18,332!2026-05-22,19,337-->
- ELU formula, mnemonic, and $\alpha$ intuition ::@:: $\operatorname{ELU}_{\alpha}(z)=\begin{cases}z,&z>0,\\ \alpha(e^z-1),&z\le 0,\end{cases}$ with derivative $1$ for $z>0$ and $\alpha e^z$ for $z<0$; mnemonic: _bend below zero toward a floor_; $\alpha$ controls both the negative saturation level $-\alpha$ and the negative-side slope near zero. <!--SR:!2026-05-22,19,337!2026-06-08,25,312-->
- rectifier-family recall ::@:: ReLU, leaky ReLU, ELU, and softplus all keep an approximately identity positive side; they differ mainly in how they treat the negative side to balance sparsity, smoothness, and gradient flow. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
...
- smooth and gated activation overview ::@:: Smooth activations are differentiable and avoid hard corners, while gated activations multiply branches so one branch can modulate the information flow of another. <!--SR:!2026-05-22,18,324!2026-05-21,17,309-->
- SiLU formula ::@:: SiLU can be written as $\operatorname{SiLU}(z)=z\sigma(z)=z\,\operatorname{softplus}'(z)$, so it is a self-gated linear activation closely related to Swish and softplus. <!--SR:!2026-05-21,17,309!2026-05-21,18,324-->
- SiLU derivative: SiLU is defined as $\operatorname{SiLU}(z)=z\sigma(z)$ where $\sigma(z)=\frac{1}{1+e^{-z}}$. Compute $\operatorname{SiLU}'(z)$ using the product rule. ::@:: By the product rule on $z\cdot\sigma(z)$: $\operatorname{SiLU}'(z)=1\cdot\sigma(z)+z\cdot\sigma'(z)=\sigma(z)+z\sigma(z)(1-\sigma(z))$, using $\sigma'(z)=\sigma(z)(1-\sigma(z))$. This can be factored as $\sigma(z)(1+z(1-\sigma(z)))=\sigma(z)(1+z-z\sigma(z))$. <!--SR:!2026-05-21,17,309!2026-05-17,16,290-->
- swish activation ::@:: Swish generalizes SiLU as $\operatorname{Swish}_{\beta}(z)=z\sigma(\beta z)$ with fixed or learned $\beta$; the case $\beta=1$ is exactly SiLU. <!--SR:!2026-05-18,17,309!2026-05-21,18,327-->
- Swish derivative: Swish is $\operatorname{Swish}_\beta(z)=z\sigma(\beta z)$ where $\sigma(t)=\frac{1}{1+e^{-t}}$. Compute $\operatorname{Swish}_\beta'(z)$ using the product rule. ::@:: By the product rule on $z\cdot\sigma(\beta z)$: $\operatorname{Swish}_\beta'(z)=\sigma(\beta z)+z\cdot\frac{d}{dz}\sigma(\beta z)=\sigma(\beta z)+z\cdot\beta\sigma(\beta z)(1-\sigma(\beta z))$. At $\beta=1$ this reduces to the SiLU derivative $\sigma(z)+z\sigma(z)(1-\sigma(z))$. <!--SR:!2026-05-18,15,290!2026-05-22,18,327-->
- Mish formula and derivative ::@:: Mish uses $\operatorname{Mish}(z)=z\tanh(\operatorname{softplus}(z))$ with derivative $\tanh(\operatorname{softplus}(z))+z\,\operatorname{sech}^2(\operatorname{softplus}(z))\sigma(z)$. <!--SR:!2026-06-27,43,309!2026-06-10,27,270-->
- GELU activation: formula, derivative derivation, and Gaussian-gating view ::@:: GELU uses $\operatorname{GELU}(z)=z\Phi(z)$ and is often approximated by $\frac{z}{2}\bigl(1+\tanh(\sqrt{2/\pi}(z+0.044715z^3))\bigr)$, so it behaves like a Gaussian-smoothed gate. <br/> Its derivative is $\operatorname{GELU}'(z)=\Phi(z)+z\phi(z)$ by the product rule, since $\Phi'(z)=\phi(z)$. <br/> So GELU keeps a linear factor while gating it by the Gaussian cumulative distribution. <!--SR:!2026-05-20,17,309!2026-05-20,17,309-->
- Gaussian-CDF logistic approximation ::@:: A useful approximation in deep-learning practice is $\Phi(z)\approx\sigma(1.7z)$. <!--SR:!2026-05-20,17,309!2026-05-21,18,327-->
- why gated activations are interesting ::@:: Gated activations are interesting because multiplicative control allows input-dependent gain modulation, which can improve conditional routing of information and represent richer feature interactions than a single fixed pointwise nonlinearity. <!--SR:!2026-05-21,18,324!2026-05-19,18,327-->
- SwiGLU with five learnable scalars ::@:: In scalar form, $\operatorname{SwiGLU}(z)=\operatorname{Swish}_{\beta}(w_1z+b_1)(w_2z+b_2)$ has five learnable parameters $(w_1,b_1,\beta,w_2,b_2)$. <!--SR:!2026-05-20,17,309!2026-05-20,17,309-->
- SwiGLU special-case collapse ::@:: If $w_1=1$, $b_1=0$, $\beta=1$, $w_2=1$, and $b_2=0$, then $\operatorname{SwiGLU}(z)=\operatorname{Swish}(z)z=z^2\sigma(z)$. <!--SR:!2026-05-20,17,309!2026-05-21,18,327-->
- SwiGLU derivative ::@:: For $\operatorname{SwiGLU}(z)=\operatorname{Swish}_{\beta}(w_1z+b_1)(w_2z+b_2)$, the derivative is $w_1\operatorname{Swish}_{\beta}'(w_1z+b_1)(w_2z+b_2)+w_2\operatorname{Swish}_{\beta}(w_1z+b_1)$. <!--SR:!2026-05-20,16,290!2026-05-22,18,324-->
- modern-activation design lesson ::@:: Many modern activations are designed to improve gradient flow, smoothness, mild non-monotonicity, or explicit information gating rather than merely to replace one formula with another. <!--SR:!2026-05-22,18,327!2026-05-21,17,309-->
...
- activation-comparison axes ::@:: Common activation-comparison axes are bounded versus unbounded, monotone versus mildly non-monotone, smooth versus piecewise smooth, zero-centered versus not, saturation behavior, computational cost, and empirical training behavior. <!--SR:!2026-05-22,18,327!2026-05-20,16,290-->
- classical smooth saturators versus rectifiers ::@:: Sigmoid and tanh are bounded smooth saturating activations, whereas ReLU-family activations are mostly unbounded hard rectifiers with better positive-side gradient flow. <!--SR:!2026-05-22,18,327!2026-05-21,17,309-->
- ReLU-family tradeoff ::@:: ReLU is cheap and empirically strong but can create dead units; leaky ReLU and parametric ReLU trade a small negative slope for more reliable gradient flow. <!--SR:!2026-07-09,55,310!2026-05-21,17,309-->
- softplus empirical caveat ::@:: Although softplus is a smooth ReLU approximation, many practical hidden-layer setups still find ReLU-family activations stronger in training speed or final performance. <!--SR:!2026-05-20,17,309!2026-05-21,18,327-->
- modern smooth-or-gated tradeoff ::@:: SiLU, Swish, Mish, GELU, and SwiGLU are smoother or more strongly gated than ReLU and often perform better in modern deep models, though usually with more computation. <!--SR:!2026-05-21,17,309!2026-05-19,18,327-->
...
- vanishing-gradient chain-rule form with notation ::@:: Let $L$ be scalar loss, $a^{(\ell)}$ layer activations, $W^{(\ell)}$ layer weights, and $D^{(\ell)}=\operatorname{diag}(g'^{(\ell)}(z^{(\ell)}))$; then recursion of $\nabla_{a^{(\ell-1)}}L=W^{(\ell)}D^{(\ell)}\nabla_{a^{(\ell)}}L$ gives $\nabla_{a^{(0)}}L=\bigl(\prod_{\ell=1}^{L}(W^{(\ell)}D^{(\ell)})\bigr)\nabla_{a^{(L)}}L$. <!--SR:!2026-05-20,17,309!2026-05-22,18,327-->
- vanishing-gradient norm bound derivation: In a deep network with activations $a^{(\ell)}$, weight matrices $W^{(\ell)}$, derivative matrices $D^{(\ell)}=\operatorname{diag}(g'^{(\ell)}(z^{(\ell)}))$, and chain-rule product $\nabla_{a^{(0)}}L=\bigl(\prod_{\ell=1}^{L}(W^{(\ell)}D^{(\ell)})\bigr)\nabla_{a^{(L)}}L$, use a matrix norm — meaning a single number that measures how much a matrix can stretch a vector — to derive a bound showing when gradients shrink with depth. ::@:: A matrix norm $\|A\|$ measures the largest amplification factor of $A$, and submultiplicativity says $\|AB\|\le \|A\|\|B\|$. <br/> Take norms of both sides: $\|\nabla_{a^{(0)}}L\|=\|W^{(1)}D^{(1)}\cdots W^{(L)}D^{(L)}\nabla_{a^{(L)}}L\|$. <br/> Now repeatedly apply $\|ABu\|\le \|A\|\|B\|\|u\|$ to peel off one matrix factor at a time. <br/> This yields $\|\nabla_{a^{(0)}}L\|\le\bigl(\prod_{\ell=1}^{L}\|W^{(\ell)}\|\,\|D^{(\ell)}\|\bigr)\|\nabla_{a^{(L)}}L\|$.  <br/> Therefore, if the typical layer factors $\|W^{(\ell)}\|\|D^{(\ell)}\|$ are below $1$, then their product can shrink roughly exponentially with depth, so early-layer gradients become tiny. <br/> For sigmoid, for example, $\|D^{(\ell)}\|$ is often at most about $1/4$ and much smaller in saturation, which is why deep chains of such factors can vanish quickly. <!--SR:!2026-06-30,46,327!2026-06-28,45,309-->
- why sigmoid causes vanishing gradients ::@:: Sigmoid satisfies $0<\sigma'(z)=\sigma(z)(1-\sigma(z))\le 1/4$, and in saturation regions the derivative is much smaller, so repeated chain-rule multiplication can shrink gradients very quickly. <!--SR:!2026-05-21,18,327!2026-05-21,17,309-->
- tanh versus ReLU for vanishing gradients ::@:: Tanh can still vanish in saturation even though $\tanh'(0)=1$, whereas ReLU avoids positive-side shrinkage because its derivative is $1$ for $z>0$ but can die completely on the negative side. <!--SR:!2026-05-19,18,327!2026-05-22,18,327-->
- why output-layer vanishing is usually less problematic ::@:: Output-layer saturation is usually less severe than hidden-layer vanishing because there is typically only one output derivative factor, and small output gradients often mean prediction error is already small. <!--SR:!2026-05-22,18,327!2026-05-22,18,324-->
- practical meaning of vanishing gradients ::@:: Vanishing gradients make optimization front-heavy: late layers update while early representation layers barely move, causing slow learning, plateaus, and weak deep-feature formation. <!--SR:!2026-05-20,17,309!2026-05-21,17,309-->
...
- hidden representation ::@:: In an FNN, the hidden layers produce a learned representation $h$ that is then passed to a probabilistic output layer. <!--SR:!2026-05-21,18,327!2026-05-18,17,309-->
- representation-learning split ::@:: In the lecture's probabilistic view, the earlier layers learn $h=f(x)$ and the final layer defines the output distribution $P(y\mid h)$. <!--SR:!2026-05-18,14,290!2026-05-18,14,290-->
- same backbone, different heads ::@:: The same learned representation $h$ can be paired with different output distributions, such as Gaussian regression heads, Bernoulli sigmoid heads, or categorical softmax heads. <!--SR:!2026-05-21,17,309!2026-05-22,18,327-->
- FNN versus shallow probabilistic models ::@:: An FNN can be viewed as linear regression, logistic regression, or softmax regression applied after a learned feature transformation rather than directly on the raw input. <!--SR:!2026-05-18,17,309!2026-05-22,18,324-->
- probabilistic-head training principle ::@:: If the output head defines $p_{\theta}(y\mid h)$, then the standard single-example training loss is negative log-likelihood, $\ell(\theta;x,y)=-\log p_{\theta}(y\mid h(x))$. <!--SR:!2026-07-10,56,310!2026-05-21,18,324-->
...
- final affine head over learned representation ::@:: In an FNN, the logits are the last-layer affine outputs $z=W^\top h+b$, where the affine head acts on the learned representation $h$ rather than directly on the raw input. <!--SR:!2026-05-21,18,327!2026-05-18,17,309-->
- backbone versus head ::@:: The hidden network acts as a feature extractor or backbone, while the final affine map and output unit form the prediction head. <!--SR:!2026-05-18,17,309!2026-05-22,18,324-->
- logits as the bridge to prediction ::@:: Logits sit at the interface between representation learning and probabilistic prediction: they are the scores that the output unit converts into a conditional distribution. <!--SR:!2026-05-21,17,309!2026-05-21,17,309-->
- why the same representation can support different tasks ::@:: A learned representation $h$ can be paired with different output heads, so the same feature extractor can support regression or different kinds of classification. <!--SR:!2026-05-21,17,309!2026-05-21,18,327-->
...
- what a logit means ::@:: A logit is a pre-normalization score on the log-probability scale: in binary models it is the exact log-odds, and in softmax models it is an unnormalized log-probability score. <!--SR:!2026-05-21,18,327!2026-05-21,18,327-->
- binary logit identity ::@:: If $p=\sigma(z)$, then $\log\frac{p}{1-p}=z$, so the sigmoid-output score is literally the binary logit. <!--SR:!2026-05-20,17,309!2026-05-18,17,309-->
- binary logit odds-interpretation ::@:: In binary logistic models, increasing logit $z$ by $1$ multiplies odds $p/(1-p)$ by $e$, so logits measure additive changes on the log-odds scale. <!--SR:!2026-05-21,17,309!2026-05-22,18,324-->
- softmax-logit identity ::@:: In softmax, $\log p_k = z_k-\log\sum_c e^{z_c}$, so each class logit is an unnormalized log-probability differing from the true log-probability by a shared normalization term. <!--SR:!2026-05-21,17,309!2026-05-18,14,290-->
- softmax relative-score interpretation ::@:: Softmax probabilities depend on relative logits: adding the same constant to all $z_k$ does not change $p_k$, so only score differences matter. <!--SR:!2026-05-20,17,309!2026-05-20,17,309-->
- logits across simple and deep models ::@:: Linear regression, logistic regression, softmax regression, and feedforward neural networks all use raw affine scores; the deep model differs mainly because the features fed into the score are learned. <!--SR:!2026-05-22,18,327!2026-05-21,18,327-->
...
- linear-Gaussian output unit ::@:: For real-valued targets, the linear-Gaussian head assumes $y\mid x \sim \mathcal{N}(z,\sigma^2)$, so the last-layer score $z$ plays the role of the predicted mean. <!--SR:!2026-05-21,17,309!2026-05-20,16,290-->
- linear activation in linear-Gaussian head ::@:: In a linear-Gaussian output unit, the output activation is identity $g_{\text{out}}(z)=z$, so the network score is used directly as the Gaussian mean. <!--SR:!2026-05-22,18,324!2026-05-20,17,309-->
- linear-Gaussian loss derivation ::@:: From $p(y\mid z)=\frac{1}{\sqrt{2\pi\sigma^2}}\exp\bigl(-\frac{(y-z)^2}{2\sigma^2}\bigr)$, one sample gives $-\log p(y\mid z)=\frac{(y-z)^2}{2\sigma^2}+\frac{1}{2}\log(2\pi\sigma^2)$, and summing over data gives $\sum_i\frac{(y_i-z_i)^2}{2\sigma^2}+\frac{N}{2}\log(2\pi\sigma^2)$. <!--SR:!2026-05-20,17,309!2026-05-21,17,309-->
- why $\tfrac{1}{2}(y-z)^2$ is commonly used ::@:: With fixed $\sigma$, additive constants and positive scale factors from Gaussian NLL can be dropped, and using $\ell=\tfrac{1}{2}(y-z)^2$ is convenient because $\partial\ell/\partial z=z-y$ has no extra factor $2$. <!--SR:!2026-05-20,17,309!2026-05-18,17,309-->
- role of the logit in Gaussian regression head ::@:: In a Gaussian regression head, the logit $z$ is not a log-odds; it is simply the predicted mean score, just as in ordinary linear regression. <!--SR:!2026-05-19,16,290!2026-05-22,18,327-->
...
- binary output model in an FNN ::@:: For one binary target, an FNN can use $P(y\mid x)=\operatorname{Ber}(y\mid \sigma(z))$ with scalar logit $z$. <!--SR:!2026-05-19,18,327!2026-05-22,18,327-->
- binary cross entropy from sigmoid output ::@:: From the Bernoulli model $p(y\mid z)=\sigma(z)^y(1-\sigma(z))^{1-y}$, the negative log-likelihood becomes $-\bigl(y\log \sigma(z)+(1-y)\log(1-\sigma(z))\bigr)$. <!--SR:!2026-05-21,18,327!2026-05-17,16,290-->
- sigmoid head versus logistic regression ::@:: A sigmoid output layer in an FNN uses exactly the same Bernoulli likelihood and binary cross-entropy loss as logistic regression; only the features defining $z$ change from raw $x$ to learned $h$. <!--SR:!2026-05-21,17,309!2026-05-22,18,327-->
- why sigmoid is acceptable at the output layer ::@:: Sigmoid is acceptable as an output unit because the negative log-likelihood supplies a useful supervised signal even though sigmoid is problematic as a hidden-layer activation. <!--SR:!2026-05-21,18,327!2026-05-20,17,309-->
- sigmoid output-gradient formula ::@:: For Bernoulli negative log-likelihood, the output-logit gradient is $\partial\ell/\partial z=\sigma(z)-y$, so near-correct predictions naturally contribute small gradient. <!--SR:!2026-05-21,17,309!2026-05-20,16,290-->
- interpretation of $\sigma(z)-y \approx 0$ ::@:: Having $\sigma(z)-y \approx 0$ means the output probability closely matches the true binary label. <!--SR:!2026-05-19,18,327!2026-05-21,18,324-->
- given binary label $y=1$ and logit $z=0.5$, compute the binary cross-entropy loss $-\log\sigma(z)$ ::@:: $\sigma(0.5)\approx 0.622$; loss $=-\log(0.622)\approx 0.476$. <!--SR:!2026-05-22,18,332!2026-05-22,18,332-->
...
- softmax output formula ::@:: For multiclass targets, the softmax head uses $P(y=k\mid x)=\frac{e^{z_k}}{\sum_{c=1}^C e^{z_c}}$. <!--SR:!2026-05-18,15,290!2026-05-20,17,309-->
- why softmax defines a categorical distribution ::@:: Softmax outputs are nonnegative and sum to one, so they define a valid categorical distribution over classes. <!--SR:!2026-05-22,18,324!2026-05-22,18,324-->
- multiclass cross entropy general and one-hot cases ::@:: For target distribution $r$, categorical NLL is $\ell=-\sum_{k=1}^{C} r_k\log p_k = -\sum_{c=1}^{C} r_c z_c + \log\sum_{c=1}^{C} e^{z_c}$; with one-hot $t$ this reduces to $-\log p_y=-z_y+\log\sum_{c=1}^{C} e^{z_c}$, the same as softmax regression. <!--SR:!2026-05-22,18,324!2026-06-22,39,290-->
- softmax output-gradient formula ::@:: For softmax cross entropy with general target $r$, $\partial\ell/\partial z_k=p_k-r_k$ (one-hot special case $p_k-t_k$). Note this expression is a vector of length $C$, the number of classes. <!--SR:!2026-05-21,17,309!2026-05-21,18,324-->
- deep-classifier interpretation ::@:: A deep multiclass classifier can be understood as a learned feature transformation followed by a softmax regression head. <!--SR:!2026-05-18,17,309!2026-05-21,17,309-->
- given logits $z=(1,2,0)$ and one-hot target $t=(0,1,0)$ (class $2$ is correct), compute the softmax probabilities and multiclass cross-entropy loss ::@:: $Z=e^1+e^2+e^0\approx 2.718+7.389+1.000=11.107$; $p_1\approx 0.245$, $p_2\approx 0.665$, $p_3\approx 0.090$; loss $=-\log(p_2)\approx 0.407$. <!--SR:!2026-05-22,18,332!2026-07-01,47,332-->
...
- backpropagation purpose ::@:: Backpropagation computes the gradients of the loss with respect to all network parameters efficiently. <!--SR:!2026-05-22,18,327!2026-05-20,17,309-->
- why backpropagation is efficient ::@:: It reuses intermediate derivative information instead of differentiating each parameter independently from scratch. <!--SR:!2026-05-22,18,327!2026-05-21,17,309-->
- backpropagation stage mnemonic ::@:: A useful mnemonic is _store, compare, blame, update_: store forward activations, compare prediction with target, send blame backward, then update parameters from the resulting gradients. <!--SR:!2026-05-18,17,309!2026-05-21,18,324-->
- backpropagation versus objective ::@:: Backpropagation is the gradient-computation procedure, not the loss function itself. <!--SR:!2026-05-19,18,324!2026-05-22,18,327-->
- why deep learning needs backpropagation ::@:: Without backpropagation, training multilayer networks by gradient-based methods would be computationally impractical. <!--SR:!2026-05-20,16,290!2026-05-20,16,290-->
...
- why the chain rule is central ::@:: Backpropagation is a repeated application of the chain rule to the layered composition defining the network. <!--SR:!2026-05-20,17,309!2026-05-20,17,309-->
- local error signal $\delta = \frac{\partial L}{\partial z}$ ::@:: A backpropagation error signal measures how sensitive the loss is to the pre-activation of a unit. <!--SR:!2026-05-22,18,324!2026-05-21,17,309-->
- weight-gradient derivation from local error ::@:: If $z_j^{(\ell)}=\sum_i W_{ij}^{(\ell)}a_i^{(\ell-1)}+b_j^{(\ell)}$ and $\delta_j^{(\ell)}=\partial L/\partial z_j^{(\ell)}$, then $\partial L/\partial W_{ij}^{(\ell)}=a_i^{(\ell-1)}\delta_j^{(\ell)}$. <!--SR:!2026-05-22,18,324!2026-05-19,18,327-->
- bias-gradient derivation from local error ::@:: With $z_j^{(\ell)}=\sum_i W_{ij}^{(\ell)}a_i^{(\ell-1)}+b_j^{(\ell)}$ and $\delta_j^{(\ell)}=\partial L/\partial z_j^{(\ell)}$, we get $\partial L/\partial b_j^{(\ell)}=\delta_j^{(\ell)}$ because $\partial z_j^{(\ell)}/\partial b_j^{(\ell)}=1$. <!--SR:!2026-05-19,15,290!2026-05-19,16,290-->
- backward-propagation derivation from local error ::@:: With the same notation, $\partial L/\partial a_i^{(\ell-1)}=\sum_j W_{ij}^{(\ell)}\delta_j^{(\ell)}$, so local errors are exactly the quantities needed to propagate blame backward. <!--SR:!2026-05-21,17,309!2026-05-20,17,309-->
- given $z=wa+b$ with $a=2$, $w=0.5$, and downstream $\delta=\partial L/\partial z=-0.3$, compute $\partial L/\partial w$, $\partial L/\partial b$, and $\partial L/\partial a$ ::@:: We get $\partial L/\partial w=a\delta=-0.6$, $\partial L/\partial b=\delta=-0.3$, and $\partial L/\partial a=w\delta=-0.15$. <!--SR:!2026-05-21,17,309!2026-05-22,18,324-->
...
- repeated transposed-Jacobian viewpoint ::@:: If $a^{(\ell)}=\Psi_{\ell}(a^{(\ell-1)})$, then backpropagation uses $\nabla_{a^{(\ell-1)}}L = J_{\Psi_{\ell}}(a^{(\ell-1)})^\top \nabla_{a^{(\ell)}}L$, so the backward pass is repeated application of transposed local Jacobians in reverse order. <!--SR:!2026-05-22,18,327!2026-05-20,17,309-->
- matrix form of local backpropagation ::@:: For one layer, the local error is $\delta^{(\ell)}=g'^{(\ell)}(z^{(\ell)})\odot \nabla_{a^{(\ell)}}L$ and the previous-layer gradient is $\nabla_{a^{(\ell-1)}}L=W^{(\ell)}\delta^{(\ell)}$. <!--SR:!2026-05-21,17,309!2026-05-18,17,309-->
- notation in the Jacobian viewpoint ::@:: In $\nabla_{a^{(\ell-1)}}L = J_{\Psi_{\ell}}(a^{(\ell-1)})^\top \nabla_{a^{(\ell)}}L$, $\Psi_{\ell}:\mathbb R^{n_{\ell-1}}\to\mathbb R^{n_{\ell}}$ is the layer map and $J_{\Psi_{\ell}}$ is its local Jacobian matrix. <!--SR:!2026-05-20,17,309!2026-06-26,42,309-->
- given $W=\begin{bmatrix}1&2\\0&1\end{bmatrix}$, $D=\operatorname{diag}(0.5,0.1)$, and upstream gradient $\nabla_{a^{(\ell)}}L=\begin{bmatrix}0.4\\-0.2\end{bmatrix}$, compute $\delta=D\nabla_{a^{(\ell)}}L$ and $\nabla_{a^{(\ell-1)}}L=W\delta$ ::@:: We get $\delta=\begin{bmatrix}0.2\\-0.02\end{bmatrix}$ and $\nabla_{a^{(\ell-1)}}L=\begin{bmatrix}0.16\\-0.02\end{bmatrix}$. <!--SR:!2026-06-28,44,309!2026-05-21,17,309-->
- forward-backward mnemonic ::@:: A useful symmetry mnemonic is _forward: mix, shift, bend, repeat; backward: compare, slope, unmix, repeat_. <!--SR:!2026-05-21,18,327!2026-05-20,17,309-->
- local-error view versus Jacobian view ::@:: The local-error recursion with $\delta$ and the repeated-transposed-Jacobian view are the same backpropagation algorithm written in coordinate form versus matrix form. <!--SR:!2026-05-20,17,309!2026-05-21,17,309-->
...
- output-layer error term ::@:: At the output layer, a standard backpropagation error term is $\delta_k = \frac{\partial L}{\partial z_k}$. <!--SR:!2026-05-19,18,324!2026-05-21,17,309-->
- output-layer weight gradient ::@:: If $u_j$ feeds output unit $k$, then $\frac{\partial L}{\partial W_{kj}} = u_j\delta_k$. <!--SR:!2026-05-22,18,327!2026-05-17,14,290-->
- output-layer gradient interpretation ::@:: A neural-network output-layer weight gradient is _incoming activation times local blame_: the activation says how much signal arrived, and $\delta_k$ says how harmful or helpful that unit's pre-activation was for the loss. <!--SR:!2026-05-19,16,290!2026-05-19,16,290-->
- common output-layer delta formulas ::@:: For sigmoid plus binary cross entropy, $\delta=\sigma(z)-y$; for softmax plus multiclass cross entropy with target $r$, $\delta_k=p_k-r_k$. <!--SR:!2026-05-21,17,309!2026-05-20,17,309-->
- bias gradient at the output layer ::@:: Because $\partial z_k/\partial b_k=1$, the bias gradient at an output unit is just the local error: $\partial L/\partial b_k=\delta_k$. <!--SR:!2026-05-20,16,290!2026-05-21,18,327-->
- given binary output with incoming activation $u=0.8$, prediction $\hat y=0.7$, and label $y=1$, compute $\delta=\hat y-y$, $\partial\ell/\partial W=u\delta$, and $\partial\ell/\partial b$ ::@:: We get $\delta=-0.3$, $\partial\ell/\partial W=-0.24$, and $\partial\ell/\partial b=-0.3$. <!--SR:!2026-05-20,17,309!2026-05-19,15,290-->
...
- why hidden layers need backward propagation ::@:: Hidden units affect the loss only indirectly through later layers, so their error signals must be computed from downstream error terms. <!--SR:!2026-05-22,18,324!2026-05-21,18,327-->
- hidden-layer backprop formula ::@:: If unit $j$ at layer $\ell$ feeds units $k$ in layer $\ell+1$, then $\delta_j^{(\ell)}=g'(z_j^{(\ell)})\sum_k W_{jk}^{(\ell+1)}\delta_k^{(\ell+1)}$. <!--SR:!2026-05-22,18,324!2026-05-20,16,290-->
- hidden-layer backprop idea ::@:: A hidden-layer error term is obtained by weighting downstream errors by the relevant outgoing weights and multiplying by the local activation derivative, so hidden blame is downstream blame pulled back through one layer. <!--SR:!2026-05-20,17,309!2026-05-20,16,290-->
- role of $g'(z_j)$ in hidden-layer backprop ::@:: The factor $g'(z_j)$ tells how strongly changes in the hidden pre-activation change that hidden unit's output. <!--SR:!2026-05-18,17,309!2026-05-20,17,309-->
- hidden-layer vector recursion and bias gradient ::@:: In vector form, $\delta^{(\ell)}=D^{(\ell)}(W^{(\ell+1)}\delta^{(\ell+1)})$, then $\nabla_{W^{(\ell)}}L=a^{(\ell-1)}(\delta^{(\ell)})^\top$ and $\nabla_{b^{(\ell)}}L=\delta^{(\ell)}$. <!--SR:!2026-05-21,17,309!2026-05-21,18,327-->
- given hidden unit with outgoing weights $(2,-1)$, downstream deltas $(0.3,-0.1)$, local derivative $g'(z_j)=0.25$, and incoming activation $a_i=0.8$, compute $\delta_j$, $\partial L/\partial W_{ij}$, and $\partial L/\partial b_j$ ::@:: We get $\delta_j=0.25(2\cdot0.3+(-1)\cdot(-0.1))=0.175$, then $\partial L/\partial W_{ij}=a_i\delta_j=0.14$ and $\partial L/\partial b_j=0.175$. <!--SR:!2026-05-22,18,327!2026-07-01,47,327-->
- backpropagation as derivative dynamic programming ::@:: Backpropagation is efficient because it reuses downstream derivative information recursively instead of recomputing every chain-rule path from scratch. <!--SR:!2026-05-20,17,309!2026-05-20,17,309-->
...
- two-stage backpropagation algorithm ::@:: Backpropagation proceeds by first running a forward pass to store activations and pre-activations, then a backward pass to compute output errors, propagate them backward, and assemble gradients. <!--SR:!2026-05-19,15,290!2026-05-21,18,327-->
- why SGD naturally pairs with backpropagation ::@:: Backpropagation provides the gradients needed by gradient-descent and stochastic-gradient-descent updates of network parameters. <!--SR:!2026-05-22,18,324!2026-05-17,14,290-->
- per-sample, minibatch, and full-batch updates ::@:: Per-sample SGD uses $B=1$, minibatch SGD uses $1<B<N$, and full-batch gradient descent uses $B=N$. <!--SR:!2026-05-22,18,324!2026-05-21,17,309-->
- averaged minibatch update rule ::@:: A common update is $\theta\leftarrow\theta-\eta\,\frac{1}{B}\sum_{i\in\mathcal B}\nabla_{\theta}\ell_i$, where averaging by $B$ keeps gradient scale more stable across batch sizes. <!--SR:!2026-05-20,17,309!2026-05-19,18,324-->
- why divide by batch size ::@:: Dividing minibatch gradients by $B$ avoids implicit scaling of update magnitude with batch size; using summed gradients is equivalent only if the learning rate is rescaled accordingly. <!--SR:!2026-05-22,18,327!2026-05-18,15,290-->
- tensors in deep-learning frameworks ::@:: Deep-learning frameworks store parameters and activations in tensors, meaning multidimensional arrays with shapes such as $(B,D)$, $(B,C,H,W)$, or $(B,T,D)$, so forward and backward computations can be carried out efficiently in bulk. <!--SR:!2026-05-18,15,290!2026-05-17,14,290-->
- what a 1-D and 2-D tensor represent ::@:: A 1-D tensor is a vector, a 2-D tensor is a matrix, and higher-order tensors store structured batches such as images, sequences, or feature maps. <!--SR:!2026-05-18,15,290!2026-05-20,16,290-->
- backpropagation interpretation ::@:: Backpropagation can be interpreted as systematic blame assignment: compare prediction with target at the output, assign local blame to each unit, and convert that blame into parameter gradients. <!--SR:!2026-05-17,14,290!2026-05-20,17,309-->
- practical demo lesson about activations ::@:: The lecture demonstrations suggest that ReLU units are often easier to learn than sigmoid or tanh units in deep feedforward networks. <!--SR:!2026-05-20,17,309!2026-05-20,17,309-->
- practical demo lesson about depth and learning rate ::@:: The demonstration slides emphasize that deeper structure can help but the learning rate still needs careful adjustment during training. <!--SR:!2026-05-19,16,290!2026-05-21,17,309-->
