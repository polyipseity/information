...
- overview ::@:: A feedforward neural network is a layered model that maps inputs to outputs by repeated affine-transform-plus-activation steps, such as $z^{(\ell)}=(W^{(\ell)})^\top a^{(\ell-1)}+b^{(\ell)}$ and $a^{(\ell)}=g^{(\ell)}(z^{(\ell)})$, without recurrent loops in one forward pass. <!--SR:!2026-05-16,15,290!2026-05-21,18,327-->
...
- He initialization ::@:: He initialization for ReLU-like activations uses variance of order $\frac{2}{n_{\mathrm{in}}}$, commonly in normal form $\mathcal N\bigl(0,\frac{2}{n_{\mathrm{in}}}\bigr)$ (the form emphasized in this course), with a variance-matched uniform alternative also possible. <!--SR:!2026-05-15,12,317!2026-05-23,19,337-->
...
- sigmoid activation: formula, range, and derivative derivation ::@:: The sigmoid activation is $\sigma(z)=\frac{1}{1+e^{-z}}=\frac{e^z}{1+e^z}=\frac{1+\tanh(z/2)}{2}$, so it maps scores to $(0,1)$. <br/> Its derivative is $\sigma'(z)=\sigma(z)(1-\sigma(z))$ because differentiating $(1+e^{-z})^{-1}$ gives $\frac{e^{-z}}{(1+e^{-z})^2}=\frac{1}{1+e^{-z}}\cdot\frac{e^{-z}}{1+e^{-z}}=\sigma(z)(1-\sigma(z))$. <br/> So sigmoid is smooth and probabilistic, but its derivative becomes small in the saturation tails. <!--SR:!2026-05-15,11,312!2026-05-21,18,332-->
...
- ELU formula, mnemonic, and $\alpha$ intuition ::@:: $\operatorname{ELU}_{\alpha}(z)=\begin{cases}z,&z>0,\\ \alpha(e^z-1),&z\le 0,\end{cases}$ with derivative $1$ for $z>0$ and $\alpha e^z$ for $z<0$; mnemonic: _bend below zero toward a floor_; $\alpha$ controls both the negative saturation level $-\alpha$ and the negative-side slope near zero. <!--SR:!2026-05-22,19,337!2026-05-06,2,292-->
...
- Mish formula and derivative ::@:: Mish uses $\operatorname{Mish}(z)=z\tanh(\operatorname{softplus}(z))$ with derivative $\tanh(\operatorname{softplus}(z))+z\,\operatorname{sech}^2(\operatorname{softplus}(z))\sigma(z)$. <!--SR:!2026-05-15,11,289!2026-05-14,10,270-->
...
- ReLU-family tradeoff ::@:: ReLU is cheap and empirically strong but can create dead units; leaky ReLU and parametric ReLU trade a small negative slope for more reliable gradient flow. <!--SR:!2026-05-15,14,290!2026-05-21,17,309-->
...
- probabilistic-head training principle ::@:: If the output head defines $p_{\theta}(y\mid h)$, then the standard single-example training loss is negative log-likelihood, $\ell(\theta;x,y)=-\log p_{\theta}(y\mid h(x))$. <!--SR:!2026-05-15,14,290!2026-05-21,18,324-->
...
- multiclass cross entropy general and one-hot cases ::@:: For target distribution $r$, categorical NLL is $\ell=-\sum_{k=1}^{C} r_k\log p_k = -\sum_{c=1}^{C} r_c z_c + \log\sum_{c=1}^{C} e^{z_c}$; with one-hot $t$ this reduces to $-\log p_y=-z_y+\log\sum_{c=1}^{C} e^{z_c}$, the same as softmax regression. <!--SR:!2026-05-22,18,324!2026-05-14,10,270-->
...
- given logits $z=(1,2,0)$ and one-hot target $t=(0,1,0)$ (class $2$ is correct), compute the softmax probabilities and multiclass cross-entropy loss ::@:: $Z=e^1+e^2+e^0\approx 2.718+7.389+1.000=11.107$; $p_1\approx 0.245$, $p_2\approx 0.665$, $p_3\approx 0.090$; loss $=-\log(p_2)\approx 0.407$. <!--SR:!2026-05-22,18,332!2026-05-15,11,312-->
