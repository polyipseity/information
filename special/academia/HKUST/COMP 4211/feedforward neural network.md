...
- sigmoid activation: formula, range, and derivative derivation ::@:: The sigmoid activation is $\sigma(z)=\frac{1}{1+e^{-z}}=\frac{e^z}{1+e^z}=\frac{1+\tanh(z/2)}{2}$, so it maps scores to $(0,1)$. <br/> Its derivative is $\sigma'(z)=\sigma(z)(1-\sigma(z))$ because differentiating $(1+e^{-z})^{-1}$ gives $\frac{e^{-z}}{(1+e^{-z})^2}=\frac{1}{1+e^{-z}}\cdot\frac{e^{-z}}{1+e^{-z}}=\sigma(z)(1-\sigma(z))$. <br/> So sigmoid is smooth and probabilistic, but its derivative becomes small in the saturation tails.
...
- leaky or parametric ReLU: formula, derivative, and why the negative slope helps ::@:: The general form is $g(z,\alpha)=\max\{0,z\}+\alpha\min\{0,z\}=\begin{cases}z,&z\ge 0,\\ \alpha z,&z<0,\end{cases}$, with fixed small $\alpha$ for leaky ReLU and learned $\alpha$ for parametric ReLU. <br/> Its derivative is $1$ for $z>0$ and $\alpha$ for $z<0$ because each branch is linear with that slope. <br/> At $z=0$ it is generally non-differentiable unless $\alpha=1$, so implementations again use a subgradient convention. <br/> The point of the negative-side slope is to leak some gradient on the left and reduce the dead-neuron problem.
