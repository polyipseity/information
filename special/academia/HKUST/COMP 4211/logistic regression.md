...
- why the score-level gradient simplifies so cleanly ::@:: The expression being simplified is $-\Bigl(y_i\frac{1}{p_i}p_i(1-p_i)-(1-y_i)\frac{1}{1-p_i}p_i(1-p_i)\Bigr)$. <br/> The reciprocal factors $1/p_i$ and $1/(1-p_i)$ cancel the matching pieces of the sigmoid derivative $p_i(1-p_i)$, leaving $-\bigl(y_i(1-p_i)-(1-y_i)p_i\bigr)$. <br/> That inner expression simplifies to $-(y_i-p_i)=p_i-y_i$, so the long chain-rule formula collapses to the clean residual form prediction minus target.
