...
- common output-layer delta formulas: with $z$ the pre-activation score/logit, $a$ or $p$ the post-activation output, $r$ the target, and $\delta=\partial\ell/\partial z$, what are the main formulas used in this note? ::@:: Use $z$ for the score, $a$ or $p$ for the output, $r$ for the target, and $\delta=\partial\ell/\partial z$ for the output-layer delta. <br/> Sigmoid + binary cross entropy: $p=\sigma(z)$ and $\delta=p-r$. <br/> Softmax + categorical cross entropy: $p_c=\frac{e^{z_c}}{\sum_k e^{z_k}}$ and $\delta_c=p_c-r_c$. <br/> Gaussian output unit (identity/linear output) + mean squared error: $a=z$ and $\delta=a-r$. <br/> So the main memory rule is still prediction minus target at the output score.
