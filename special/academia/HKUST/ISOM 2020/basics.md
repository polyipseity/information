...
A note regarding accuracy is that {{there may be some inaccuracies involved when `float`s are involved, such as `0.1 + 0.2` is `0.30000000000000004` instead of `0.3`}}. <!--SR:!2024-09-25,18,302-->

A notable error (raising an error causes {{the program to stop and print out error messages}}) is that {{`/` (division), `//` (floor division), and `%` (remainder) raises `ZeroDivisionError` if the second operand is zero}}. However, `0 ** 0` (power) {{returns `1` instead of being undefined or raising an error}}. <!--SR:!2024-09-21,14,302!2024-09-23,16,302!2024-09-22,15,302-->
...
Some interesting facts about `float`s:  `print` {{always output at least 1 decimal place for `float`s, and outputs at most as many digits as needed to represent the number exactly}}. `1` {{is an `int` while `1.` and `1.0` are `float` representing the same value `1`}}. <!--SR:!2024-09-24,17,302!2024-09-20,13,290-->
