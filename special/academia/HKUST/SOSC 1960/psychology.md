...
- the scale of connectome mapping ::@:: Diffusion MR imaging and histological tract tracing, from 1 cm to 1 m over hours to years.
- the scale of social networks ::@:: About 10 cm to 1 m over minutes to years.

## measurement and modeling tools

An eye tracker records where the eyes look, giving the sequence of fixations and the regions a person attends to. Electroencephalography (EEG) records brain waves, the electrical activity of the brain at the scalp. Computational modeling writes a process down as an explicit model rather than a description: a model of face recognition, for instance, samples fixations at several scales and encodes each with a shared multi-scale convolutional network, chooses the next region of interest with a hidden Markov model over the mouth, the nose side, and the eye corner, and confines attention to that region with a Gaussian mask. A shared classifier scores each fixated region, and the losses of the three fixations are combined by their maximum into a multi-task loss.
