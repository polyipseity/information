...
If we further assume {@{the noise is _white_ Gaussian noise}@}, then {@{$\rho = \frac {(s_{o0} - s_{o1})^2} {\sigma_{n_T}^2} = \frac {2E_g} {N_0}$ applies}@}, thus the above expression can be {@{rewritten in terms of the _bit difference_ "virtual" energy $E_g$}@}: {@{$$\boxed{P_e(V_{\!th})= Q\!\left(\sqrt{\frac{(s_{o1} - s_{o0})^2 }{4\sigma_{n_T}^2 } }\right) = Q\!\left(\sqrt {\frac {E_g} {2 N_0} } \right)} \,.$$}@}
