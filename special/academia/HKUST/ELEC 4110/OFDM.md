...
In practice, implementing such a large set of sub‑carriers would require a separate transmitter front end (modulator, mixer, etc.) for each tone and the insertion of guard bands between adjacent carriers to prevent overlap.  These requirements are costly and waste spectral efficiency.  
