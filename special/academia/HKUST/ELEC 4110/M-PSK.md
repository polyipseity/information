...
{@{The $M$-ary PSK constellation}@} is {@{$M$ equally spaced points on the circumference of a unit circle in the signal space}@}. {@{Each point}@} corresponds to {@{one symbol}@} and is distinguished {@{solely by its phase angle}@}; {@{all symbols}@} have {@{identical amplitude}@}, giving {@{a regular polygon shape when plotted with orthogonal basis functions}@}.
...
Consequently, with {@{Gray coding}@} {@{most symbol errors}@}—those that are {@{statistically most likely}@} because the receiver is {@{confused between neighboring points}@}—result in {@{exactly one erroneous bit}@}, reducing {@{the bit-error rate}@}. Thus, for {@{modulation schemes using Gray coding}@}, we have {@{the following approximation}@}: {@{$$\boxed{P_{e, b} \approx \frac {P_{e, M} } k = \frac {P_{e, M} } {\log_2 M} } \,,$$}@} which is {@{accurate when SNR is moderate to high}@}.
...
{@{_Gray coding_}@} addresses this by ensuring that {@{adjacent constellation points differ in only one bit}@}. For PSK, {@{a Gray code}@} can be {@{built recursively}@}: start with {@{the two-point ($M=2$) code $[0,1]$}@}; for {@{each subsequent doubling of $M$}@}, append {@{the reverse of the existing sequence to itself \(e.g. $[00, 01, 01, 00]$\)}@} and {@{toggle the MSB of the new entries \(e.g. $[00, 01, 11, 10]$\)}@}. The result is {@{a binary sequence}@} where {@{successive symbols differ by a single bit}@}.
