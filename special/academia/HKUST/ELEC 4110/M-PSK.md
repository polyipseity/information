...
Consequently, with {@{Gray coding}@} {@{most symbol errors}@}—those that are {@{statistically most likely}@} because the receiver is {@{confused between neighboring points}@}—result in {@{exactly one erroneous bit}@}, greatly reducing {@{the overall bit-error rate}@}. Thus, for {@{modulation schemes using Gray coding}@}, we have {@{the following approximation}@}: {@{$$\boxed{P_{e, b} \approx \frac {P_{e, M} } k = \frac {P_{e, M} } {\log_2 M} } \,,$$}@} which is {@{accurate when SNR is moderate to high}@}.
