...
{@{The primary drawback}@} is the {@{linear increase in required bandwidth or symbol time}@}—an {@{$L$-order scheme consumes $L$ times more spectrum or time}@}. {@{This bandwidth penalty}@} can be {@{prohibitive in spectrally crowded environments}@}, while {@{the symbol time penalty}@} can be {@{prohibitive in time-sensitive applications}@}.

Moreover, when {@{the _total_ SNR at transmission is fixed}@}, each repeated copy {@{transmits and thus receives only a portion (typically $1 / L$) of that energy}@}, so {@{the _effective_ SNR at reception remains unchanged or even decrease}@}. (annotation: __this course__: Tested in {@{the final examination}@}.) For example, when using {@{maximal-ratio combining}@} and assuming {@{equal energy at each branch}@}, each branch's {@{_individual_ SNR is $1 / L$ of the _total_ SNR at transmission}@}, so the {@{_effective_ SNR at reception, given by the sum of the individual SNRs of each branch}@}, is {@{the same as the _total_ SNR at transmission}@}. If using {@{other types of diversity combining}@}, the effective SNR may {@{even be reduced}@}. Intuitively summarized, one does not {@{gain "extra" SNR for free by using diversity}@}.
...
These examples illustrate how {@{practical constraints—silicon area, power, and real‑estate costs}@}—often {@{necessitate suboptimal but cost‑effective diversity schemes}@} in {@{commercial Wi‑Fi chipsets}@}.
