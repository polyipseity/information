...
In the case of {@{M-FSK}@}, {@{$P(s_k \mid s_m)$}@} equals: {@{$$\boxed{P(s_k \mid s_m) = Q\left(\frac {\sqrt{2E_s} } {2\sqrt{N_0 / 2} } \right) = Q\left(\sqrt{\frac {E_s} {N_0} } \right) }$$}@} by considering {@{projections on the _normalized_ basis}@}, so {@{$d = \sqrt 2 \sqrt E_s = \sqrt{2E_s}$}@} and {@{$\sigma_n^2 = N_0 / 2$}@}. This applies for {@{every combination of $s_k$ and $s_m$ where $k \ne m$}@}, so {@{_symbol_ error upper bound}@}: {@{$$\boxed{P_{eM} \le (M - 1) Q\left(\sqrt{\frac {E_s} {N_0} } \right)} \,,$$}@} and consequently for {@{_bit_ error upper bound}@}: {@{$$\boxed{P_{e} = \frac M {2(M - 1)} P_{eM} \le \frac M 2 Q\left(\sqrt{\frac {E_s} {N_0} } \right)} \,.$$}@} Using {@{an approximation quite accurate for $x \ge 3$}@} \(different from {@{the approximation mentioned in binary modulation}@}\): {@{$$Q(x) \le \frac 1 2 e^{-x^2 / 2} \,,$$}@} the above is usually rewritten as {@{$$\boxed{P_{e} \approx \frac M 4 \exp\left(\frac {E_s} {2N_0} \right) = \frac M 4 \exp\left(\frac {kE_b} {2N_0} \right) } \,.$$}@}
