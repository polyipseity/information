...
- ELEC 4110
    ...
    - ELEC 4110 / instructor ::@:: - Professor Vincent: Hong Kong → HKU undergrad → 3 yrs at monopoly operator _Tandicom_ → chartered engineer path <br/> - PhD in the UK: peer-controversial (HK industry scarcity) → viewed as adventure, not guaranteed success <br/> - Post-PhD: Bell Labs US → realized academia was superficial vs. the depth needed for cutting-edge work
        ...
        - ELEC 4110 / instructor / post-PhD ::@:: After his doctorate, Vincent joined Bell Labs in the U.S., where he found that much of his academic knowledge was superficial compared to the depth needed for cutting-edge research and product development.
        - ELEC 4110 / instructor / teaching ::@:: The instructor emphasizes two questions: "how" a system operates and "why" it is designed that way. Step-by-step explanations cover the former; the latter requires a theoretical framework that translates engineering intuition into fundamental principles. He argues that deep theory—not just hands-on knowledge—is needed to design new systems rather than merely maintain existing ones.
        - ELEC 4100 / instructor / academia ::@:: Vincent contrasts undergraduate studies—centered on absorbing existing knowledge and passing exams—with doctoral research, which involves creating new knowledge amid uncertainty and extended timelines. He recounts peers who lingered into their eighth year without a clear graduation path, showing how uncertain the academic career path can be.
    ...
    - ELEC 4110 / culture ::@:: The lecturer stresses that while students won't be compelled to ask questions, curiosity matters. Classroom Q&A sessions normalize inquiry and reduce the stigma of uncertainty, building confidence in tackling new material and articulating ideas.
- [communication](../../../../general/communication.md) ::@:: Commonly defined as the transmission of information. Its precise definition is disputed and there are disagreements about whether unintentional or failed transmissions are included and whether communication not only transmits meaning but also creates it.
...
- [signal processing](../../../../general/signal%20processing.md) ::@:: An electrical engineering subfield that focuses on analyzing, modifying and synthesizing signals, such as sound, images, potential fields, seismic signals, altimetry processing, and scientific measurements.
- [analog signal](../../../../general/analog%20signal.md) ::@:: Any signal, typically a continuous-time signal, representing some other quantity, i.e., _analogous_ to another quantity.
    - analog signal / examples ::@:: ECG waveforms, voice recordings, etc.
    - analog signal / information ::@:: Information is encoded in measurable attributes such as amplitude, phase, or temporal shape; visualizing a waveform in the time domain reveals its structure and shows how the signal's form reflects the underlying content. <p> Entropy will formalize uncertainty in later courses; this course concentrates on intuitive signal interpretation.
- [time domain](../../../../general/time%20domain.md) ::@:: A representation of how a signal, function, or data set varies with time. Used for the analysis of mathematical functions, physical signals or time series of economic or environmental data.
...
- [periodic function](../../../../general/periodic%20function.md) ::@:: A function that repeats its values at regular intervals. <p> The length of the interval over which a periodic function repeats is called its __period__. Any function that is not periodic is called __aperiodic__.
...
- [sine wave](../../../../general/sine%20wave.md) ::@:: A periodic wave whose waveform (shape) is the trigonometric sine function.
    ...
    - sine wave / significance ::@:: Understanding a sinusoid's parameters—amplitude, frequency, and phase—comes before Fourier analysis or modulation, because sinusoids are the building blocks for more complex signals in communication theory. This representation underlies channel analysis, noise modeling, and bandwidth throughout the course.
- [phasor](../../../../general/phasor.md) ::@:: A complex number representing a sinusoidal function whose amplitude _A_ and initial phase _θ_ are time-invariant and whose angular frequency _ω_ is fixed.
...
- [Fourier transform](../../../../general/Fourier%20transform.md) ::@:: An integral transform that takes a function as input, and outputs another function that describes the extent to which various frequencies are present in the original function. The output of the transform is a complex-valued function of frequency.
...
- [Heaviside step function](../../../../general/Heaviside%20step%20function.md) ::@:: A step function named after Oliver Heaviside, the value of which is zero for negative arguments and one for positive arguments. Different conventions concerning the value _H_(0) are in use. (__this course__: _H_(0)&nbsp;=&nbsp;1)
...
- [ramp function](../../../../general/ramp%20function.md) ::@:: A unary real function, whose graph is shaped like a ramp. It can be expressed by numerous definitions, for example "0 for negative inputs, output equals input for non-negative inputs".
...
- [analog-to-digital converter](../../../../general/analog-to-digital%20converter.md) \(ADC\) ::@:: A system that converts an analog signal, such as a sound picked up by a microphone or light entering a digital camera, into a digital signal.
...
- analog-to-digital converter
    - analog-to-digital converter / bit depth
        - analog-to-digital converter / bit depth / effects ::@:: The error introduced equals the difference between the original sample and its nearest quantization level; the worst-case error is half the step size $\Delta / 2$. <p> Increasing the number of bits halves $\Delta$ (e.g., moving from 3 to 4 bits doubles the number of levels), reducing the maximum quantization error by a factor of two.
- [signal-to-noise ratio](../../../../general/signal-to-noise%20ratio.md) \(SNR, S/N\) ::@:: A measure used in science and engineering that compares the level of a desired signal to the level of background noise. SNR is defined as the ratio of signal power to noise power, often expressed in decibels.
    - signal-to-noise ratio / formula ::@:: $$\text{SNR} = \frac {\text{signal (without noise) power} } {\text{noise power} } \,.$$ However, it is often expressed in decibels \(dB\) instead: $$\text{SNR}_{\text{dB} } = 10 \log_{10} \text{SNR} \,.$$ 0 dB means the signal has the same power as noise. Every 10&nbsp;dB increase in SNR multiplies the signal power by 10, so that 10&nbsp;dB means 10 times, 20&nbsp;dB means 100 times, 30&nbsp;dB means 1000 times, etc.
        - signal-to-noise ratio / formula / amplitude ::@:: When working with signal amplitudes rather than powers, the SNR definition is still based on power. <p> Because electrical power is proportional to the square of voltage (or current), converting this amplitude ratio into decibels uses a factor of 20 instead of 10: $$\boxed{\;\text{SNR}_{\text{dB} }=20\,\log_{10}\!\left(\frac{A_{\text{signal} } }{A_{\text{noise} } }\right)\;} \,.$$ <p> This formula is equivalent to the power-based expression $10\log_{10}(\text{SNR})$ because $(A_{\text{signal} }/A_{\text{noise} })^2 = \text{SNR}$. A 10-fold increase in amplitude ratio corresponds to a 100-fold increase in \(power\) SNR.
    - signal-to-noise ratio / quantization ::@:: Under common assumptions—input is a sinusoid, noise uniformly distributed over $[-\Delta/2,\;\Delta/2]$—the SNR improves roughly $6.02\,\text{dB}$ per added bit: $$\text{SNR}_{\text{dB} } \approx 6.02\,n + C$$ where $n$ is bits per sample and $C$ depends on the signal shape \(for sine wave, $C = 20 \log_{10}\left(\sqrt{3 / 2}\right) \approx 1.761$\). <p> This rule of thumb guides how many bits are needed for a target quality. Real signals like speech have different dynamic ranges, so the actual SNR may deviate from this estimate.
...
- signal-to-noise ratio
    - signal-to-noise ratio / dynamic range \(DR\) ::@:: It—the ratio between maximum and minimum signal amplitudes—affects how many quantization levels are needed for a given SNR with non-sinusoidal inputs.
- [digital-to-analog converter](../../../../general/digital-to-analog%20converter.md) \(DAC\) ::@:: A system that converts a digital signal into an analog signal.
    ...
    - digital-to-analog converter / steps ::@:: reconstruction → lowpass filter
        ...
        - digital-to-analog converter / steps / lowpass filter ::@:: Discrete value is converted into continuous value. Holding the sampled value until the next sample yields a continuous-time signal with discrete value. A lowpass filter smoothens the signal and makes it continuous-valued. <p> The lowpass filter frequency cutoff is usually half the sample rate due to the _Nyquist–Shannon sampling theorem_. If the original signal does not have frequencies higher than this cutoff, the only remaining error is quantization noise.
- analog signal
...
- digital signal
    - digital signal / advantages ::@:: _Robust_ to degradation and noise, as small distortions of the waveform can be completely ignored. The mapping from waveform to bits introduces a decision threshold; an error occurs only if noise pushes the signal past that threshold.
    - digital signal / disadvantages ::@:: It cannot represent most physical signals _exactly_, as most of them are analog in nature.
    - digital signal / usage ::@:: Because of its robustness, it is used in storage and telecommunications.
        ...
        - digital signal / usage / telecommunications ::@:: Long-distance transmission causes path loss attenuation and thermal noise. If an analog signal is used, signal-to-noise ratio \(SNR\) decreases significantly at the receiver and may corrupt the waveform entirely. <p> If a digital signal is used, SNR remains sufficiently high because of its robustness to degradation and noise. As long as received bits are correctly interpreted, the information survives.
- [magnetic tape](../../../../general/magnetic%20tape.md) ::@:: A medium for magnetic storage made of a thin, magnetizable coating on a long, narrow strip of plastic film. Developed in Germany in 1928, based on the earlier magnetic wire recording from Denmark.
...
- [digital audio](../../../../general/digital%20audio.md) ::@:: A representation of sound recorded in, or converted into, digital form. In digital audio, the sound wave of the audio signal is typically encoded as numerical samples in a continuous sequence.
    - digital audio / sampling rate ::@:: Human hearing tops out around 20 kHz; to satisfy the Nyquist criterion, sample at least twice this bandwidth. <p> CDs use a slightly higher rate of 44.1 kHz for implementation margin, plus 16-bit quantization for high-fidelity representation.
    ...
    - digital audio / formats ::@:: Lossless formats like WAV preserve every bit of the original PCM stream, whereas lossy codecs such as MP3 discard perceptually insignificant bits, reducing size by roughly an order of magnitude.
- [digital video](../../../../general/digital%20video.md) ::@:: An electronic representation of moving visual images (video) in the form of encoded digital data. This is in contrast to analog video, which represents moving visual images in the form of analog signals.
    - digital video / historical context ::@:: In the early 1990s, analog audio could be digitized into a manageable data rate; however, a two-hour movie would require many CD-size discs if encoded directly with PCM. <p> The missing ingredient was efficient compression exploiting human visual perception (e.g., MPEG-1 for VCD).
    - digital video / compression ::@:: Once lossy video codecs were available in the late 1990s, entire movies fit onto a single disc or cartridge, making digital video commercially viable. Subsequent generations (DVD, Blu-ray) introduced higher-order MPEG variants (MPEG-2, MPEG-4) to reduce bitrate further while maintaining acceptable visual quality. <p> The trade-off: raw digitization gives robustness but enormous storage; compression reduces size at the cost of irreversibly discarding data.
- [public switched telephone network](../../../../general/public%20switched%20telephone%20network.md) \(PSTN\) ::@:: The aggregate of the world's telephone networks that are operated by national, regional, or local telephony operators. Provides infrastructure and services for public telephony.
...
- [amplifier](../../../../general/amplifier.md) ::@:: An electronic device that increases the magnitude of a signal (a time-varying voltage or current). A two-port electronic circuit that uses electric power from a power supply to increase the amplitude of a signal applied to its input terminals, producing a proportionally greater amplitude signal at its output.
    - amplifier / signal-to-noise ratio ::@:: An amplifier only boosts the received voltage; it also amplifies all existing noise, so the output SNR is never higher than the input SNR (and typically lower because of the amplifier's own noise figure).
        - amplifier / signal-to-noise ratio / implications ::@:: Adding a receiver-side amplifier cannot extend the usable link distance if the incoming SNR is already too low. The only practical way to increase range with a fixed power budget is to raise the transmitted power, which is limited by regulations and hardware capabilities.
- [data transmission](../../../../general/data%20transmission.md) ::@:: The transfer of data over a point-to-point or point-to-multipoint communication channel.
    - data transmission / analog ::@:: In an analog link, any waveform distortion directly corrupts the information because the signal is continuous.
    - data transmission / digital ::@:: Digital links perform A/D conversion (e.g., PCM), turning the waveform into a bitstream; the bits are then mapped onto a carrier such as on-off keying. <p> Distortions that still allow correct bit decision do not affect the recovered data: after decoding and DAC, the original waveform is reconstructed up to quantization noise. Quantization noise depends only on bit resolution and is independent of channel length, so it can be controlled separately from propagation loss.
...
- [data compression](../../../../general/data%20compression.md) ::@:: The process of encoding information using fewer bits than the original representation. Any particular compression is either lossy or lossless.
...
- Fourier transform
    - Fourier transform / definition ::@:: The Fourier transform of a complex-valued function $f(x)$ on the real line, is the complex valued function ${\widehat {f} }(\xi )$, defined by the integral $${\widehat {f} }(\xi )=\int _{-\infty }^{\infty }f(x)\ e^{-i2\pi \xi x}\,dx,\quad \forall \xi \in \mathbb {R} \,.$$ In this case $f(x)$ is \(Lebesgue\) integrable over the whole real line, i.e., the above integral converges to a continuous function ${\widehat {f} }(\xi )$ at all $\xi$ \(decaying to zero as $\xi \to \infty$\). <p> \(__this course__: __Important__. Prefer using the angular frequency one.\)
        - Fourier transform / definition / intuition ::@:: These integrals are conceptually "uncountable sums" capturing all sinusoidal components simultaneously.
...
- [Plancherel theorem](../../../../general/Plancherel%20theorem.md) ::@:: A result in harmonic analysis, proven by Michel Plancherel in 1910. A generalization of Parseval's theorem; often used in science and engineering, proving the unitarity of the Fourier transform.
...
- Fourier transform
    - Fourier transform / computation ::@:: Start with the definition and work through a few basic cases (impulse → flat spectrum; rectangular pulse → sinc). Then use properties: linearity, time-shifting ($x(t-t_0)$ adds a phase factor $e^{-j2\pi ft_0}$), frequency scaling, and conjugation. <p> This mirrors how differentiation is learned: first principle → examples → rules that avoid re-deriving each new function.
- [bandwidth](../../../../general/bandwidth%20(signal%20processing).md) ::@:: The difference between the upper and lower frequencies in a continuous band of frequencies. Typically measured in unit of hertz (symbol Hz).
    - bandwidth / intuition ::@:: Bandwidth can be defined as the frequency range over which a signal's energy (or magnitude) is significant. <p> Bandwidth thus links the abstract spectral description with concrete hardware requirements. For example, we can say "keep enough frequencies so that only 1% of the total energy is lost". This is _energy bandwidth_.
    - bandwidth / subcategories ::@:: It may refer specifically to two subcategories: _passband bandwidth_ and _baseband bandwidth_.
    - bandwidth / passband bandwidth ::@:: The difference between the upper and lower cutoff frequencies of, for example, a band-pass filter, a communication channel, or a signal spectrum.
    - bandwidth / baseband bandwidth ::@:: Equal to the upper cutoff frequency of a low-pass filter or baseband signal, which includes a zero frequency.
    - bandwidth / computation ::@:: In practice, we may compute bandwidth according to hardware requirements using different methods, obtaining different bandwidths. <p> Some possible definitions are: energy bandwidth, 3 db bandwidth, zero-crossing bandwidth, etc. Each definition yields a different numerical value; always state which convention you are using when quoting a bandwidth.
        - bandwidth / computation / energy ::@:: The bandwidth that keeps enough frequencies such that only a certain percentage of total energy is lost.
        - bandwidth / computation / 3 dB ::@:: The width between points where the amplitude spectrum has dropped by 3 dB (half power) from its peak.
        - bandwidth / computation / zero-crossing ::@:: Use the first zero crossing of the amplitude spectrum as the cutoff, assuming the amplitude spectrum starts off nonzero at zero frequency.
- spectral density
...
- bandwidth
    ...
    - bandwidth / in practice ::@:: In practice, if a _baseband_ signal is band-limited to $B$&nbsp;Hz, sampling at any rate greater than $2B$ samples per second guarantees perfect reconstruction (Nyquist–Shannon).
- Fourier transform
    - Fourier transform / computation
        - Fourier transform / computation / properties ::@:: Key properties—time shifting, scaling, modulation, convolution—let you manipulate spectra without re-deriving the full integral. These rules speed up transform computation for common signals.
- Nyquist–Shannon sampling theorem
    - Nyquist–Shannon sampling theorem / frequency domain ::@:: sampling is pointwise multiplication in the time domain → sampling is convolution in the frequency domain
        ...
        - Nyquist–Shannon sampling theorem / frequency domain / conclusion ::@:: If the sampling period satisfies $$T_s \le \frac{1}{2B}\;\;\Longleftrightarrow\;\; f_s \ge 2B \,,$$ then the shifted copies of $X(f)$ produced by the Dirac comb do not overlap. <p> Consequently a low-pass filter that keeps only the baseband component can recover the original signal without loss.
- frequency domain
    - frequency domain / compression ::@:: In the time domain, every sample appears equally important; you cannot prune samples without potentially large errors. In the frequency domain, many sinusoid coefficients have negligible amplitude; discarding them loses little energy (e.g., 1%). This selective pruning is the basis of audio/video codecs that store only the most significant spectral components.
        - frequency domain / compression / examples ::@:: The human ear perceives frequencies up to about 20 kHz. CDs use a sampling rate of 44.1 kHz—more than twice this limit, satisfying the Nyquist criterion. This margin allows practical anti-aliasing filters with realizable transition bands, avoiding the instability of a perfect cutoff.
    - frequency domain / algorithms ::@:: Operations like filtering, modulation, or convolution become simple multiplications or scalings in the frequency domain. Algorithms like FFT-based convolution exploit this property to achieve orders of magnitude speedups over time-domain equivalents.
    - frequency domain / transfer function ::@:: A system's transfer function is straightforward to understand when expressed as multiplication by its spectral response.
    - frequency domain / basis ::@:: The Fourier basis is one choice; wavelet transforms, Hadamard matrices, and other bases can provide sparser representations for specific signal types. <p> A basis that aligns with the signal's structure often compresses better or processes more easily than sinusoids alone.
...
- [wireless](../../../../general/wireless.md) ::@:: The transfer of information (telecommunication) between two or more points without the use of an electrical conductor, optical fiber or other continuous guided medium. The most common wireless technologies use radio waves.
    - wireless / historical motivation ::@:: Fixed-line infrastructure is prohibitively expensive in sparsely populated areas; wireless local loops are a cost-effective alternative that eliminates copper cabling from each home to the exchange.
    - wireless / Guglielmo Marconi ::@:: He achieved the first trans-Atlantic wireless telegraph, proving practical long-distance transmission; his work earned him the Nobel Prize and confirmed that wireless communication was feasible.
        - wireless / Guglielmo Marconi / Nikola Tesla ::@:: Nikola Tesla's early wireless experiments—oscillators, generators, filters, amplifiers—laid groundwork that influenced long-distance telegraphy. <p> Thomas McCune's trans-Atlantic wireless telegraph project was built on many of Tesla's patents, leading to a protracted legal battle in which Tesla ultimately prevailed. The episode shows how credit for foundational inventions can shift depending on legal outcomes and historical narrative.
        - wireless / Guglielmo Marconi / tests ::@:: McCune's team conducted trans-Atlantic wireless telegraph tests, using ships to explore how far radio waves could travel over the curved Earth where straight-line towers were ineffective. After initial failures, they discovered that certain frequencies were reflected by a layer of ionized plasma—the then unknown ionosphere—allowing signals to "bounce" across the Atlantic. This foreshadowed modern satellite communications, which rely on reflections to extend coverage beyond the horizon.
    - wireless / system ::@:: The system has three components: transmitter, wireless channel, and receiver. <p> Engineers have design freedom for the transmitter and receiver (e.g., AM, FM modulation schemes), but not for the channel; it is dictated by physical propagation conditions.
...
- wireless
    ...
    - wireless / channel ::@:: A physical medium that transmits signals between points; in wireless, it exhibits additional phenomena not present in wired links.
        - wireless / channel / general characteristics ::@:: Common impairments (present in both wired and wireless) include attenuation (loss of signal strength), thermal noise, filtering effects, and intersymbol interference \(ISI\) that limit usable bandwidth.
...
- [radio wave](../../../../general/radio%20wave.md) ::@:: They are a type of electromagnetic radiation with the lowest frequencies and the longest wavelengths in the electromagnetic spectrum, typically with frequencies below 300 gigahertz \(GHz\) and wavelengths greater than 1 millimeter \(3⁄64 inch\), about the diameter of a grain of rice.
    - radio wave / frequency ::@:: Radio waves span from kilohertz to gigahertz frequencies; they are defined by cycles per second (hertz). <p> The term _frequency_ is formally applied to periodic signals, but for practical purposes it extends to sinusoidal components of any signal.
    ...
    - radio wave / regulation and licensing ::@:: Every country/region has an authority (e.g., Hong Kong's Communications Authority) that divides the entire spectrum into bands assigned to specific applications: AM/FM, TV, mobile, satellite, etc.
        - radio wave / regulation and licensing / mechanisms ::@:: Two main mechanisms allocate licensed spectrum: <p> - _Auctions_ – used when supply is scarce and demand high; operators bid for a lease typically lasting ~15 years. <br/> - _Assignments_ – direct allocation by application where demand is lower; involves lower fees and less competition.
        - radio wave / regulation and licensing / enforcement ::@:: Regulators enforce compliance through spectrum monitoring and policing, detecting illegal transmitters (e.g., scammers using rogue base stations).
    - radio wave / ISM band ::@:: The ISM band at 2.4 GHz and the 5 GHz band are designated for "instrumental, scientific and medical" use; devices operating here need no license. Common consumer technologies—Wi-Fi, Bluetooth, wireless mice—operate in these bands, supporting low-cost, short-range communications. <p> Because unlicensed spectrum can be used by many users simultaneously, its chief concern is robustness to unpredictable interference rather than spectral efficiency.
    ...
    - radio wave / propagation ::@:: For an isotropic radiator, power spreads uniformly over a spherical surface; received power equals transmit power times receiver aperture divided by $4\pi d^2$. <p> The geometry of the expanding sphere explains why received power falls off with the square of distance—no material (ohmic) loss is involved. Even when beamforming concentrates energy in a narrow solid angle, the same geometric dilution applies; only the effective aperture changes.
        - radio wave / propagation / line-of-sight \(LOS\) ::@:: Required for high-frequency signals (>10 GHz); signal travels straight, minimal reflection or diffraction.
        - radio wave / propagation / non-line-of-sight \(NLOS\) ::@:: Lower-frequency signals can bend around obstacles via diffraction, reflection, or scattering, enabling indoor coverage.
        - radio wave / propagation / multipath ::@:: Multipath propagation causes multiple delayed copies of the transmitted wave to arrive at the receiver, producing constructive/destructive interference.
        - radio wave / propagation / path loss exponent ::@:: In indoor or urban environments, multipath causes path loss to follow $L(d) \propto d^{-\alpha}$, where $\alpha$ (the path-loss exponent) can exceed 4 in dense settings. Higher frequencies (e.g., mmWave at 28 GHz) have larger $\alpha$ and thus much smaller coverage radii than lower bands like 900 MHz. Regulatory bodies rely on empirical field measurements to determine frequency allocations that balance coverage, capacity, and interference.
        - radio wave / propagation / path loss ::@:: Larger $\alpha$ shrinks cell radius, forcing operators to deploy more base stations and increasing capital expenditure. <p> However, by limiting signal reach, path loss reduces inter-cell interference, enabling dense spatial reuse and higher network capacity. Without path loss, a single transmitter could cover the entire globe, making cellular cells meaningless.
    - radio wave / regulation and licensing
        - radio wave / regulation and licensing / parameters ::@:: A spectrum license is defined by three technical parameters: carrier frequency, allowable bandwidth, and power mask (maximum transmit power).
            - radio wave / regulation and licensing / parameters / carrier frequency ::@:: Center frequency of the band's frequency window. It determines penetration, diffraction, and scattering characteristics; lower frequencies travel farther through walls and obstacles.
            - radio wave / regulation and licensing / parameters / bandwidth ::@:: Width of the band's frequency window.
            - radio wave / regulation and licensing / parameters / transmit-power mask \(TPM\) ::@:: It sets a hard limit on transmit power and specifies required isolation from adjacent bands to avoid interference. It protects human receivers from excessive exposure and keeps adjacent-channel interference below acceptable thresholds.
- [carrier wave](../../../../general/carrier%20wave.md) ::@:: A periodic waveform (usually sinusoidal) that conveys information through a process called _modulation_.
    - carrier wave / carrier frequency ::@:: Low-frequency bands (e.g., ~2 GHz) have longer wavelengths (~15 cm), comparable to common obstacle sizes, supporting diffraction and robust non-line-of-sight (NLOS) reception. <p> High-frequency millimeter-wave bands (e.g., 28–30 GHz) have wavelengths in the millimeter range; obstacles much larger than this wavelength block signals, so line-of-sight (LOS) is almost mandatory.
        - carrier wave / carrier frequency / capacity ::@:: In a low-frequency band with limited spectral availability, allocating even 1 GHz of bandwidth can consume ~50% of the carrier's usable spectrum. <p> Higher frequency bands typically offer many gigahertz of contiguous spectrum, making large bandwidth allocations feasible and supporting very high data rates (e.g., 5G hotspots).
        - carrier wave / carrier frequency / exploit ::@:: In practice, operators pair low-frequency "coverage" slices with high-frequency "capacity" slices to balance coverage breadth against peak throughput.
        - carrier wave / carrier frequency / cost ::@:: Scarce low-frequency bands are usually auctioned because demand far exceeds supply, driving up prices. <p> Abundant high-frequency bands are often assigned by regulators (or lightly auctioned) since supply is ample and propagation penalties reduce operator incentive to bid aggressively.
- radio wave
    - radio wave / regulation and licensing
        - radio wave / regulation and licensing / examples ::@:: 5G base stations operating around 5 GHz were installed close to an airport. The FAA argued that out-of-band emissions could interfere with aircraft avionics at ~6 GHz, despite the transmitter's own TPM compliance. <p> This case shows the need for cross-industry coordination when allocating spectrum near critical services.
- [transmitter](../../../../general/transmitter.md) ::@:: An electronic device which produces radio waves with an antenna for signal transmission to a radio receiver.
    - transmitter / digital transmission ::@:: A digital transmitter takes a bitstream (e.g., 1&nbsp;000 bits) and converts it into an analog waveform that satisfies channel constraints. <p> The output must be band-limited to the licensed frequency range (band-pass signal). Modulation schemes (QAM, OFDM, etc.) map the digital data onto the analog carrier while maintaining spectral confinement.
- [radio receiver](../../../../general/radio%20receiver.md) ::@:: An electronic device that receives radio waves and converts the information carried by them to a usable form. Used with an antenna.
    - radio receiver / digital transmission ::@:: The receiver observes a noisy version of the transmitted waveform and must recover the original bits. <p> Recovery quality is measured by metrics such as bit error rate (BER) or frame error rate (FER).
...
- [additive white Gaussian noise](../../../../general/additive%20white%20Gaussian%20noise.md) \(AWGN\) ::@:: A basic noise model used in information theory to mimic the effect of many random processes that occur in nature.
...
- [cumulative distribution function](../../../../general/cumulative%20distribution%20function.md) (CDF, cdf) ::@:: A __cumulative distribution function__ (__CDF__, __cdf__) of a real-valued random variable _X_ is the probability that _X_ will take a value less than or equal to _x_: $$F_X(x) = P(X \le x) \,.$$ The probability that _X_ will take a value in between \(_a_, _b_\], where _a_ < _b_, is $$P(a < X \le b) = F_X(b) - F_X(a) \,.$$
...
- [normal distribution](../../../../general/normal%20distribution.md) ::@:: The __normal distribution__ or __Gaussian distribution__ is important in statistics and is often used in the natural and social sciences to represent real-valued random variables whose distributions are not known. Their importance is partly due to the central limit theorem. It is denoted $\mathcal N(\mu, \sigma^2)$, where $\mu \in \mathbb R$ is the mean and $\sigma^2 \in \mathbb R_{> 0}$ is the variance. <p> \(When you see $\mathcal N(0, 100)$, do not mistake the 100 as the standard deviation! It is the variance.\)
    ...
    - normal distribution / shape ::@:: All normal distributions have the bell shape regardless of their parameters. Changing the mean simply translates the shape, while increasing the variance fattens (wider) and flattens (lower peak) the shape.
    - normal distribution / standardization ::@:: Any normal distribution can be __standardized__ by defining the random variable $$Z = \frac {X - \mu} {\sigma} \qquad X = \sigma Z + \mu \,.$$ Further, $$z = \frac {x - \mu} \sigma$$ is also known as the __standard score__ of the data _x_. <p> After standardization, a standard normal table providing $\Phi(z)$ for different values of _z_ can evaluate the CDF of any normal distribution. (The table may not show negative values of _z_. In that case, use the CDF property above.)
- [joint probability distribution](../../../../general/joint%20probability%20distribution.md) ::@:: Given [random variables](../../../../general/random%20variable.md) $X,Y,\ldots$, that are defined on the same [probability space](../../../../general/probability%20space.md), the __multivariate__ or __joint probability distribution__ for $X,Y,\ldots$ is a [probability distribution](../../../../general/probability%20distribution.md) that gives the probability that each of $X,Y,\ldots$ falls in any particular range or discrete set of values specified for that variable.
    - joint probability distribution / expressions ::@:: The joint probability distribution can be expressed as a joint cumulative distribution function, a joint probability density function (for continuous variables), or a joint probability mass function (for discrete variables).
    - joint probability distribution / cumulative distribution function \(CDF\) ::@:: It gives the probability that both variables lie within specified ranges; it is analogous to the CDF for one variable but now two-dimensional. <p> If this joint CDF is smooth, its derivative with respect to both variables yields the _joint PDF_, which can be visualized as a surface or contour plot. A two-dimensional histogram approximates the joint PDF by counting samples in each bin; more data and finer bins approach the true continuous density.
    - joint probability distribution / marginal distribution ::@:: It gives the probabilities for any one variable with no reference to specific ranges of values for the other variables.
- [multivariate normal distribution](../../../../general/multivariate%20normal%20distribution.md) ::@:: A generalization of the one-dimensional (univariate) normal distribution to higher dimensions. A random vector is said to be k-variate normally distributed if every linear combination of its k components has a univariate normal distribution.
    - multivariate normal distribution / names ::@:: multivariate normal distribution, multivariate Gaussian distribution, joint normal distribution
    - multivariate normal distribution / notation and parameters ::@:: $${\mathcal {N} }({\boldsymbol {\mu } },\,{\boldsymbol {\Sigma } })$$ where <p> ___μ___ ∈ __R__<sup>_k_</sup> — [location](../../../../general/location%20parameter.md) <br/> __Σ__ ∈ __R__<sup>_k_ × _k_</sup> — [covariance](../../../../general/covariance%20matrix.md) \([positive semi-definite matrix](../../../../general/positive%20semi-definite%20matrix.md)\)
    - multivariate normal distribution / probability distribution function \(PDF\) ::@:: $$(2\pi )^{-k/2}\det({\boldsymbol {\Sigma } })^{-1/2}\,\exp \left(-{\frac {1}{2} }(\mathbf {x} -{\boldsymbol {\mu } })^{\mathrm {T} }{\boldsymbol {\Sigma } }^{-1}(\mathbf {x} -{\boldsymbol {\mu } })\right) \,,$$ <br/> exists only when __Σ__ is [positive-definite](../../../../general/positive-definite%20matrix.md)
...
- [Markov's inequality](../../../../general/Markov's%20inequality.md) ::@:: In probability theory, it gives an upper bound on the probability that a non-negative random variable is at least some positive constant. It is tight in the sense that for each chosen positive constant, there exists a random variable for which the inequality is an equality.
    - Markov's inequality / statement ::@:: If _X_ is a nonnegative random variable and _a_ \> 0, then the probability that _X_ is at least _a_ is at most the expectation of _X_ divided by _a_: $$\operatorname {P} (X\geq a)\leq {\frac {\operatorname {E} (X)}{a} }.$$
...
- [Q-function](../../../../general/Q-function.md) ::@:: The tail distribution function of the standard normal distribution. $Q(x)$ is the probability that a standard normal random variable takes a value larger than $x$.
    ...
    - Q-function / computation ::@:: No closed-form expression. A table of values or computers must be used. <p> In MATLAB, use the `qfunc(x)` function or `1 - normcdf(x)`.
- multivariate normal distribution
    ...
    - multivariate normal distribution / random vector ::@:: A vector $\mathbf{x}\in\mathbb{R}^n$ is Gaussian if it equals a linear transformation of a standard normal vector $\mathbf w$ plus a shift $\mathbf b$: $$\mathbf{x}=A\mathbf{w}+b \,.$$ <p> The _mean_ vector is $\mathbf b$. The _covariance_ matrix is $$K=\operatorname{Cov}\{\mathbf{x}\}=AA^{T} \,.$$
...
- [sample mean](../../../../general/sample%20mean%20and%20covariance.md)
    ...
    - sample mean / properties ::@:: _Unbiased_: $\operatorname E[\overline X] = \mu_X$. Its variance is inversely proportional to the number of samples: $\operatorname{Var}(\overline X) = \frac {\sigma_X^2} N$.
- central limit theorem
    - central limit theorem / classical CLT ::@:: If $\overline {X_n}$ is the _sample mean_ of a random sample (sequence of _i.i.d._ random variables) of size $n$ taken from _any_ population with population mean $\mu_X$ and _finite_ variance $\sigma_X^2 < \infty$, then as $n \to \infty$, the random variable $\overline {X_n}$ _converges in distribution_ to $\mathcal N\left(\mu_X, \sigma_X^2 / n\right)$. Equivalently, $\overline {Z_n} = (\overline {X_n} - \mu_X) \sqrt{n / \sigma_X^2}$ _converges in distribution_ to $\mathcal N(0, 1)$.
- [law of large numbers](../../../../general/law%20of%20large%20numbers.md) ::@:: A mathematical law that states that the average of the results obtained from a large number of independent random samples converges to the true value, if it exists. Given a sample of independent and identically distributed values, the sample mean converges to the true mean.
    - law of large numbers / weak law ::@:: The __weak law of large numbers__ \(also called [Khinchin](../../../../general/Aleksandr%20Khinchin.md)'s law\) states that given a collection of [independent and identically distributed](../../../../general/independent%20and%20identically%20distributed%20random%20variables.md) \(iid\) samples from a random variable with finite mean, the sample mean [converges in probability](../../../../general/convergence%20in%20probability.md) to the expected value $${\overline {X} }_{n}\ {\overset {P}{\rightarrow } }\ \mu \qquad {\textrm {when} }\ n\to \infty .$$ That is, for any positive number _ε_, $$\lim _{n\to \infty }\Pr \!\left(\,|{\overline {X} }_{n}-\mu |<\varepsilon \,\right)=1.$$
        - law of large numbers / weak law / interpretation ::@:: The weak law states that for any nonzero margin \(_ε_\), no matter how small, with a sufficiently large sample there will be a very high probability that the average of the observations will be close to the expected value; that is, within the margin.
    - law of large numbers / strong law ::@:: The __strong law of large numbers__ \(also called [Kolmogorov](../../../../general/Andrey%20Kolmogorov.md)'s law\) states that the sample average [converges almost surely](../../../../general/almost%20sure%20convergence.md#almost%20sure%20convergence) to the expected value $${\overline {X} }_{n}\ {\overset {\text{a.s.} }{\longrightarrow } }\ \mu \qquad {\textrm {when} }\ n\to \infty .$$ That is, $$\Pr \!\left(\lim _{n\to \infty }{\overline {X} }_{n}=\mu \right)=1.$$
        - law of large numbers / strong law / interpretation ::@:: As the number of trials _n_ goes to infinity, the probability that the average of the observations converges to the expected value is one. The modern proof of the strong law is more complex than that of the weak law, and relies on passing to an appropriate sub-sequence.
...
- stochastic process
    ...
    - stochastic process / autocorrelation ::@:: Its autocorrelation is $$R_X(t_1,t_2)=\mathbb{E}\{X(t_1)X^*(t_2)\} \,.$$ Some definitions conjugate the first variable instead. (annotation: __this course__: Conjugate the second variable.)
    - stochastic process / cross-correlation ::@:: The cross-correlation of two processes $X,Y$ is $$R_{XY}(t_1,t_2)=\mathbb{E}\{X(t_1)Y^*(t_2)\} \,.$$ Some definitions conjugate the first variable instead. (annotation: __this course__: Conjugate the second variable.)
- stationary process
...
- random variable
- [stochastic process](../../../../general/stochastic%20process.md) ::@:: A mathematical object usually defined as a family of random variables in a probability space, where the index of the family often has the interpretation of time.
- joint probability distribution
- moment
    - moment / description ::@:: A full joint distribution can yield all moments; the reverse is not true for general processes. <p> For Gaussian processes, first-order and second-order moments determine the entire process.
- [stationary process](../../../../general/stationary%20process.md) ::@:: A stochastic process whose statistical properties, such as mean and variance, do not change over time. The joint probability distribution of the process remains the same when shifted in time. The process is statistically consistent across different time periods.
...
- [white noise](../../../../general/white%20noise.md) ::@:: A random signal having equal intensity at different frequencies, giving it a constant power spectral density.
...
- [signal modulation](../../../../general/signal%20modulation.md) ::@:: The process of varying one or more properties of a periodic waveform in electronics and telecommunication for the purpose of transmitting information.
...
- [phase-shift keying](../../../../general/phase-shift%20keying.md) \(PSK\) ::@:: A digital modulation process which conveys data by changing the phase of a constant frequency carrier wave. The modulation is accomplished by varying the sine and cosine inputs at a precise time.
    - phase-shift keying / binary phase-shift keying (BPSK) ::@:: The simplest form of phase shift keying (PSK). Uses two phases which are separated by 180° and so can also be termed 2-PSK.
...
- [frequency-shift keying](../../../../general/frequency-shift%20keying.md) ::@:: A frequency modulation scheme in which digital information is encoded on a carrier signal by periodically shifting the frequency of the carrier between several discrete frequencies.
...
- ELEC 4110
    - ELEC 4110 / [binary modulation](binary%20modulation.md)
        - [§ receiver](binary%20modulation.md#receiver)
        - [§ bit error rate](binary%20modulation.md#bit%20error%20rate)
- [_M_-ary transmission](../../../../general/M-ary%20transmission.md) ::@:: A type of digital modulation. Instead of sending one bit at a time as in binary, multiple messages, M, are sent. The binary data stream is divided into _n_ tuples, where _n_=log₂&nbsp;_M_ bits. The signals can be represented as different frequencies, as in MFSK.
    - _M_-ary transmission / advantages ::@:: This type of transmission reduces channel _bandwidth_ \(compared to halving the symbol time, which doubles the bandwidth\) at the expense of higher bit error rates.
- ELEC 4110
...
- [decibel watt](../../../../general/decibel%20watt.md) (dBW) ::@:: A unit for the measurement of the strength of a signal expressed in decibels relative to one watt. It can express both very large and very small values of power in a short range of number.
...
- [Shannon–Hartley theorem](../../../../general/Shannon–Hartley%20theorem.md) ::@:: It tells the maximum rate at which information can be transmitted over a communications channel of a specified bandwidth in the presence of noise.
    - Shannon–Hartley theorem / noisy-channel coding theorem ::@:: An application of the noisy-channel coding theorem to the archetypal case of a continuous-time analog communications channel subject to Gaussian noise.
    - Shannon–Hartley theorem / conditions ::@:: The theorem establishes a bound on the maximum error-free information rate that can be transmitted with a specified bandwidth in the presence of noise, assuming bounded signal power and a Gaussian noise process with known power spectral density.
    ...
    - Shannon–Hartley theorem / noise ::@:: For AWGN of _two-sided_ PSD $N_0 / 2$, the _one-sided_ PSD is $N_0$, the noise power per _nonnegative_ frequency. The noise power over the _passband_ bandwidth $B$ is $N = N_0 B$.
    - Shannon–Hartley theorem / spectral efficiency ::@:: Rearranging the formula, we get _spectral efficiency_: $$\eta := \frac C B = \log_2\!\left(1+\frac{S}{N}\right)\quad(\text{bits/s/Hz}) \,.$$ To increase spectral efficiency, increase the signal-to-noise power ratio $S / N$. <p> The above gives an _upper bound_ for spectral efficiency $\eta$ given a fixed SNR $S / N$.
    - Shannon–Hartley theorem / energy efficiency ::@:: When discussing _energy efficiency_, we use SNR per symbol $E_s / N_0$ or _SNR per bit_ $E_b / N_0$. Using the latter: $$\frac {E_b} {N_0} = \frac {ST_b} {N / B} = \frac B C \frac S N = \frac B C \left(2^{C / B} - 1 \right) = \frac {2^\eta - 1} {\eta} \,.$$ An alternative using _SNR_ $S / N$: $$\frac {E_b} {N_0} = \frac {2^\eta - 1} {\eta} \implies \text{SNR} = \frac {E_b C} {N_0 B} = 2^\eta - 1 \,.$$ <p> This gives a _lower bound_ for SNR given a fixed spectral efficiency $\eta$. The _normalized SNR_: $$\text{SNR}_{\text{norm} } = \frac {\text{SNR} } {2^\eta - 1}$$ compares the _actual_ SNR with its lower bound.
    - Shannon–Hartley theorem / Shannon limit ::@:: We want to find the minimum _SNR per bit_ $E_b / N_0$ for transmitting any useful data. Using $$\frac {E_b} {N_0} = \frac {2^\eta - 1} {\eta} \,,$$ take the limit as $\eta \to 0$ \(approaching zero spectral efficiency, i.e., no data sent\), then $$\frac {E_b} {N_0} \to \frac 1 {\log_2 e} = \ln 2 \approx 0.693 \approx -1.59~\text{dB} \,.$$ That is, given AWGN of _two-sided_ PSD $N_0 / 2$, this is the lowest SNR per bit at which any data can be sent through the channel.
- ELEC 4110
    - ELEC 4110 / [M-ary transmission](M-ary%20transmission.md)
...
- ELEC 4110
    - ELEC 4110 / final examination
        ...
        - ELEC 4110 / final examination / expectations & support ::@:: Students should use the remaining weeks to address gaps the midterm results revealed. TA/Instructor is available for clarification; schedule via email first.
