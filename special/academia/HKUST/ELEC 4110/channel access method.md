...
In {@{a multi‑user wireless system}@}, {@{the _orthogonal resource partitioning_ principle}@} assigns each {@{user a distinct slice of the shared medium}@} so {@{signals can be separated at the receiver}@}. The base station observes only {@{the superposed signal $$y(t)=\sum_{k=1}^{K} x_k(t)+n(t),$$}@} where {@{$x_k(t)$}@} is {@{the contribution from user $k$}@} and {@{$n(t)$}@} is {@{additive noise}@}. Because {@{all users transmit simultaneously}@}, coordination is needed to keep {@{their signals orthogonal at the receiver}@}.

{@{The base station}@} must {@{_separate_ the individual user streams}@} from {@{the aggregate observation $y(t)$}@}. This requires {@{distinct, mutually orthogonal resources}@}. {@{The network protocol}@} coordinates {@{resource allocation among the $K$ users}@}, assigning them {@{different frequencies, time slots, or spreading codes}@}.

{@{Three orthogonal multiple‑access strategies}@} partition {@{the shared medium}@} in {@{different domains}@}:
...
All three require each {@{user's resource allocation}@} to be {@{_orthogonal_ to those of all others}@}, enabling clean {@{signal separation at the base station}@}.
...
{@{The TDMA approach}@} keeps {@{base‑station hardware small and cheap}@}: {@{a single transceiver}@} {@{handles all users}@}. {@{The _peak_ rate}@} seen by a user can be {@{much higher than its average data rate}@}; if {@{the modulation scheme}@} is not {@{tolerant of such peaks}@}, {@{an equaliser}@} may be {@{required at the receiver}@}.
...
{@{FDMA}@} typically requires {@{a separate radio front end for each occupied channel}@} at {@{the base station}@}. Consequently, {@{the number of transceivers grows linearly with _N_}@}, which can lead to {@{bulky hardware when many users are served simultaneously}@}. Because each {@{user's signal occupies its own frequency band}@}, {@{the receiver}@} does not {@{need a complex equaliser}@}; {@{link‑level processing}@} stays {@{simple}@}.
...
{@{Real filters}@} have {@{non‑ideal roll‑off}@}, requiring {@{guard bands between FDMA channels}@}. {@{TDMA}@} needs {@{guard times}@} to {@{accommodate different propagation delays among users}@}. {@{These overheads}@} reduce {@{the theoretical maximum number of usable channels}@}; {@{network planning}@} must accommodate them.

## hybrid TDMA/FDMA

Assume {@{a channel coherence bandwidth of 200&nbsp;kHz}@} and {@{prohibits equalizers}@}. Each {@{subchannel}@} must therefore stay {@{≤200&nbsp;kHz}@} to {@{remain flat‑fading}@}. {@{Splitting the 15&nbsp;MHz band}@} into {@{at least 75 such subchannels}@}, the design uses {@{a _hybrid TDMA/FDMA_ scheme}@}: {@{_Carriers_ (groups of subchannels)}@} are formed; {@{each carrier}@} is then {@{divided in time by TDMA}@}. This {@{hybrid arrangement}@} avoids {@{equalization}@} and allows {@{fewer RF units at the base station}@}.
...
{@{A necessary condition}@} is {@{perfect _synchronisation_}@} between {@{the transmitters and receivers}@}; otherwise {@{the orthogonality is lost}@}. Because of these constraints {@{deterministic CDMA}@} is usually applied only on {@{the _forward link_ or _downlink_ (from base station to mobile)}@} because {@{synchrony is easy to maintain}@}. On {@{the uplink}@}, {@{the mobile}@} must know {@{propagation delay $\tau$ and send a pilot}@} so {@{the base station can estimate it each frame}@}. {@{The required accuracy}@} is within {@{a fraction of a chip (~1&nbsp;µs)}@}—far {@{tighter than typical millisecond estimates}@}—so {@{many pilots are needed}@}. {@{The extra uplink pilot overhead}@} becomes {@{a bottleneck as the number of users grows}@}.

For example, {@{a base station}@} transmits {@{two users simultaneously}@}, each with {@{a distinct orthogonal code (using deterministic CDMA)}@}, and ensures {@{both chip sequences start at the same instant}@}. {@{Mobile receivers}@} use {@{a pilot signal}@} to find {@{the correct timing accurate within a fraction of a chip (~1&nbsp;µs)}@}; once {@{aligned}@}, {@{despreading with their own code (using random CDMA)}@} recovers {@{the intended symbol}@} while {@{the other user's contribution cancels out}@} thanks to {@{orthogonality}@}.
...
Although {@{the channels are not perfectly orthogonal and/or synchronous}@}, each user's signal is still {@{_despread_ by correlating with its own PN code}@}. There is, however, {@{residual interference from other users}@}, which makes it {@{_interference-limited_}@}. {@{The interference}@} is {@{reduced by the _processing gain_}@} {@{$$G_{\text{proc} } = \frac{T_{\text{samp} } }{T_{\text{chip} } },$$}@} where {@{$T_{\text{samp} }$}@} is {@{the symbol period}@} and {@{$T_{\text{chip} }$}@} is {@{the chip duration}@}. {@{Random CDMA}@} therefore supports {@{far more users than deterministic schemes}@}; it is {@{the preferred choice for the _reverse link_ or _uplink_ (mobile to base station)}@} because {@{many more users can be accommodated without costly timing advance}@}.

Because {@{channels are not perfectly orthogonal}@}, a user transmitting at high power can {@{drown out weaker users}@}—a phenomenon called {@{the _near–far problem_}@}. {@{The optimal operating point}@} occurs when {@{all users' _received powers_ are equal}@}. This is enforced by {@{a _power control_ algorithm}@} that adjusts {@{each transmitter's output}@} to compensate {@{for path loss and fading}@}.
...
In {@{a random‑access or contention‑based channel access method}@}, each {@{device independently decides when to transmit}@}. {@{The most common example}@} is {@{Wi‑Fi's CSMA/CA (Carrier Sense Multiple Access with Collision Avoidance)}@}, where {@{a node first senses the medium}@}; if it {@{detects no ongoing transmission}@} it {@{transmits immediately}@}, otherwise it {@{defers and retries after a random back‑off period}@}. {@{This "listen‑before‑talk" discipline}@} lets {@{multiple users share the same channel}@} without {@{explicit time or frequency allocation}@}.

{@{Random access}@} contrasts with {@{centralized schemes}@} such as {@{TDMA, FDMA, CDMA and OFDMA}@} where {@{a scheduler assigns specific time slots, frequencies, or codes to each user}@}. Coordinated methods offer {@{higher spectral efficiency}@} but {@{require dedicated signaling and scheduling infrastructure}@}. {@{Random access}@} trades {@{lower coordination overhead}@} for {@{higher collision probability and reduced capacity when many users contend simultaneously}@}.
