...
_Reactive programming_ can be compared with the classic _observer_ pattern, which then hints at a more functional alternative in Scala.
...
The `Signal` type is immutable; its value is computed when `apply()` is called. `Signal.Var` extends `Signal` with an `update` method, allowing the underlying function to change:

> [!example] __`Signal.Var` usage__
>
> `Signal.Var` extends `Signal` with an `update` method, allowing the underlying function to change:
>
> ```scala
...
> ```scala
> val x = Signal.Var(1)
> val y = Signal(x() * 2)   // y depends on current x
> x.update(2)               // y becomes 4
> ```
>
...
>
> The core of a Scala FRP library is the `Signal` abstraction and its mutable variant `Signal.Var`.  A `Signal[T]` represents a value that can change over time; a `Signal.Var[T]` extends it with an `update` method so callers can re‑define the underlying expression.  The implementation hides details inside the companion object `frp.Signal`.
>
> ```scala
...
`Signal.apply` constructs a signal from an expression.  A `Signal.Var` keeps the current expression and can be updated to a new one.
...
When a signal is read, it registers the calling signal as an observer, which is represented by an opaque type `Signal.Observer`.  If its value changes, all observers are re‑evaluated and the observer set is cleared first so that new dependencies (caused by re-evaluation) can be recorded.
...
The `noObserver` value in the companion object of `Signal` provides a default `Signal.Observer` for top‑level evaluations, ensuring that a signal can be read from ordinary code.
