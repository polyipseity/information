...
- load regulation definition: Define load regulation as $\Delta V_{O}/\Delta I_{O}$. ::@:: Load regulation quantifies how much the regulated output voltage changes when the *output current demand* changes. It is defined as $\Delta V_{O}/\Delta I_{O}$, and a smaller value means the regulator holds the output rail steadier as the load changes.
- why load regulation uses output current ::@:: Load regulation studies stability against output-side demand changes. Since the regulator is supposed to keep output voltage approximately constant, the varying quantity is naturally the output *current* drawn by the load, while the response of interest is the small resulting change in output voltage.
- line vs load regulation: what does each one test? ::@:: Line regulation tests stability of $V_O$ against *input-voltage* changes from the source side. Load regulation tests stability of $V_O$ against *output-current* changes from the load side.
