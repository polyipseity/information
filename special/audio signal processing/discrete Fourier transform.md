...
For {@{real sinusoids}@}, it is also easy to derive their DFT as {@{they can be written as a sum of 2 complex sinusoids with opposing frequencies}@}. For example, {@{a real cosine wave of cycle frequency $k_0 / N$: $$\cos_{k_0}[n] := A \cos(2\pi (k_0 / N) n) \qquad n = 0, \ldots, N - 1$$}@} can be written as {@{the weighted sum of 2 complex sinusoids with cycle frequency $k_0 / N$ and $-k_0 / N$}@}: {@{$$\cos_{k_0}[n] = A \cos(2\pi (k_0 / N) n) = \frac {A e^{j 2\pi (k_0 / N) n} } 2 + \frac {A e^{-j 2\pi (k_0 / N) n} } 2 \,.$$}@} By linearity, the nonzero DFT values are {@{$$\begin{aligned} \operatorname{Cos}_{k_0}[k_0] & = \frac {AN} 2 \\ \operatorname{Cos}_{k_0}[-k_0] = \operatorname{Cos}_{k_0}[N - k_0] & = \frac {AN} 2 \,. \end{aligned}$$}@} For {@{a real sine wave of cycle frequency $k_0 / N$: $$\sin_{k_0}[n] := A \sin(2 \pi (k_0 / N) n) \qquad n = 0, \ldots, N - 1 \,,$$}@} it can also be expressed as {@{the weighted sum of 2 complex sinusoids with cycle frequency $k_0 / N$ and $-k_0 / N$}@}: {@{$$\sin_{k_0}[n] := A \sin(2 \pi (k_0 / N) n) = \frac {A e^{j 2\pi (k_0 / N) n} } {2j} - \frac {A e^{-j 2\pi (k_0 / N) n} } {2j} \,.$$}@} By linearity, the nonzero DFT values are {@{$$\begin{aligned} \operatorname{Sin}_{k_0}[k_0] & = \frac {AN} {2j} = -j \frac {AN} 2 \\ \operatorname{Sin}_{k_0}[-k_0] = \operatorname{Sin}_{k_0}[N - k_0] & = -\frac {AN} {2j} = j \frac {AN} 2 \,. \end{aligned}$$}@} For IDFT, {@{the same applies}@}, except that {@{the IDFT values are divided by $N$ \(no $N$ in the resulting values\), and each occurrence of $j$ is replaced by $-j$}@}.
...
{@{The magnitude spectrum}@} is {@{the transformed signal under the modulus operation $\lvert X \rvert$ (length of the complex amplitude)}@}. For easier visualization, the magnitude may be {@{in logarithm scale}@}. The magnitude, intuitively, represents {@{the loudness of the complex sinusoid of a given frequency}@}.
