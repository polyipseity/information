...
- inspecting "Let $F(x)=\int_a^x\!f(x)\,\mathrm{d}x$, where $a$ is in $\mathbb{R}$ and $f(x)>0$ for every $x$ in $\mathbb{R}$. Find the constraints on $F^{-1}(x)$." :: Produce $F^{-1}$ from $F(x)$ by letting $x = F^{-1}(y)$. <!--SR:!2024-07-05,3,250-->
