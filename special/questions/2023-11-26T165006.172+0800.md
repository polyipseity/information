...
- inspecting "Let $f:[0,1]\to[0,+\infty)$ be a [function](function%20(mathematics).md) [continuous](continuous%20function.md) on $[0,1]$ and [differentiable](differentiable%20function.md) on $(0,1)$. 0 is the only [root](zero%20of%20a%20function.md) of $f$. Prove that there exists $x\in(0,1)$ such that $\frac{f'(x)}{f(x)}=k\frac{f'(1-x)}{f(1-x)}$ for all $k\in(0,+\infty)$." :: Apart from the observations on the previous line, note that $f(0) = f(1 - 1) = 0$, so something important happens when $x = 0$ and $x = 1$. Also, it may be convenient to rewrite the equation into $\frac {f'(x)} {f(x)} - k \frac {f'(1 - x)} {f(x)} = 0$.
