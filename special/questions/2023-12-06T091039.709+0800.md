...
- inspecting "$f$ is a [twice-differentiable](differentiable%20function.md) defined on the real [interval](interval%20(mathematics).md) $[a,b]$ and $f''>0$ on $(a,b)$" :: It has something to do with convexity. $f''>0$ implies strict convexity. Recall its property: values of the function between the endpoints (exclusive) are below a line connecting the endpoints.
