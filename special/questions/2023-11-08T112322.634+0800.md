...
w^2&>\frac{y^2}{12}\\
w&>\frac{y}{2\sqrt3}\\
\mathrm{When\ }A'&<0,w<\frac{y}{2\sqrt3}\mathrm{\ obviously}\\
...
&=\frac{y^2}2\\
\mathrm{As\ }w&\to0^+,\\
A&\to\frac1{16}y^3(0^+)^{-1}+\frac12y(0^+)+y^{-1}(0^+)^3\\
&\to+\infty\\
...
A_\mathrm{max}&=\max\left(\frac{y^2}2,\frac12x^2y^{-1}\left(x-\sqrt{x^2-y^2}\right)\right)
\end{aligned}$$
