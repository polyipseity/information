...
- inspecting "A force is defined by $\vec{F}=-2.65xy^2\hat{j}$. By considering the displacement from the origin to $(4,4)$, find out if the force is conservative." :: Guess that the force is not conservative. Consider 2 ways to move from the origin to $(4, 4)$: a straight line, and moving in the $x$-axis only and then the $y$-axis only. <!--SR:!2024-07-05,3,250-->
- evaluating path integrals :: Parameterize the path by writing the path as a function of a real variable. <!--SR:!2024-07-05,3,250-->
