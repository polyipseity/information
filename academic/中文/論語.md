#flashcards/中文/論語 #academic/中文

# 論語

%%
```Python
# 08e5b0a3-f78a-46af-bf50-eb9b12f7fa1e generate data
data = gen.common.seq_to_code((
		"學而",
		"為政",
		"八佾",
		"里仁",
		"公冶長",
		"雍也",
		"述而",
		"泰伯",
		"子罕",
		"鄉黨",
		"先進",
		"顏淵",
		"子路",
		"憲問",
		"衛靈公",
		"季氏",
		"陽貨",
		"微子",
		"子張",
		"堯曰",
	),
	index=1,
	prefix='{mem lnk:_（開始）_}',
	suffix='{mem lnk:_（完成）_}',)
__env__.result = gen.Results(
	gen.Result(
		location=__env__.cwf_section('1239bc'),
		text=gen.common.quote_text(data),
	),
	gen.Result(
		location=__env__.cwf_section('def923'),
		text=gen.common.memorize_indexed_seq(data,
			indices=1,
			states=read.read_flashcard_states(__env__.cwf_section('def923')),
		),
	),
	gen.Result(
		location=__env__.cwf_section('ba12d3'),
		text=gen.common.memorize_linked_seq(data,
			tag='mem lnk',
			hinted=False,
			states=read.read_flashcard_states(__env__.cwf_section('ba12d3')),
		),
	),
)
```
%%

<!--08e5b0a3-f78a-46af-bf50-eb9b12f7fa1e generate section="1239bc"--><!-- The following content is generated at 2023-02-28T01:10:10.078654+08:00. Any edits will be overridden! -->

> 1. 學而
> 2. 為政
> 3. 八佾
> 4. 里仁
> 5. 公冶長
> 6. 雍也
> 7. 述而
> 8. 泰伯
> 9. 子罕
> 10. 鄉黨
> 11. 先進
> 12. 顏淵
> 13. 子路
> 14. 憲問
> 15. 衛靈公
> 16. 季氏
> 17. 陽貨
> 18. 微子
> 19. 子張
> 20. 堯曰

<!--/08e5b0a3-f78a-46af-bf50-eb9b12f7fa1e-->

<!--08e5b0a3-f78a-46af-bf50-eb9b12f7fa1e generate section="def923"--><!-- The following content is generated at 2023-02-28T01:10:10.093614+08:00. Any edits will be overridden! -->

1. 1:::學而
2. 2:::為政
3. 3:::八佾
4. 4:::里仁
5. 5:::公冶長
6. 6:::雍也
7. 7:::述而
8. 8:::泰伯
9. 9:::子罕
10. 10:::鄉黨
11. 11:::先進
12. 12:::顏淵
13. 13:::子路
14. 14:::憲問
15. 15:::衛靈公
16. 16:::季氏
17. 17:::陽貨
18. 18:::微子
19. 19:::子張
20. 20:::堯曰

<!--/08e5b0a3-f78a-46af-bf50-eb9b12f7fa1e-->

<!--08e5b0a3-f78a-46af-bf50-eb9b12f7fa1e generate section="ba12d3"--><!-- The following content is generated at 2023-02-28T01:10:10.102592+08:00. Any edits will be overridden! -->

1. _（開始）_→:::←學而
2. 學而→:::←為政
3. 為政→:::←八佾
4. 八佾→:::←里仁
5. 里仁→:::←公冶長
6. 公冶長→:::←雍也
7. 雍也→:::←述而
8. 述而→:::←泰伯
9. 泰伯→:::←子罕
10. 子罕→:::←鄉黨
11. 鄉黨→:::←先進
12. 先進→:::←顏淵
13. 顏淵→:::←子路
14. 子路→:::←憲問
15. 憲問→:::←衛靈公
16. 衛靈公→:::←季氏
17. 季氏→:::←陽貨
18. 陽貨→:::←微子
19. 微子→:::←子張
20. 子張→:::←堯曰
21. 堯曰→:::←_（完成）_

<!--/08e5b0a3-f78a-46af-bf50-eb9b12f7fa1e-->
