...
> ![The result of a [constant-Q transform](https://en.wikipedia.org/wiki/constant-Q_transform) applied to the waveform of a C major chord played on a piano. The x axis corresponds to frequency mapped to the standard pitch names \(increasing from lower pitches on the left to higher frequencies on the right\). The y axis is time, with the start of the piano chord happening at the bottom and the release happening at the top \(approximately 8 seconds later\) <br/> The darker areas correspond to higher values of the constant Q transform. The peak values are vibrating frequencies of the strings. The leftmost 3 strongest peaks are at the fundamental frequencies of the notes of the C major chord \(C, E, G\). The the remaining peaks to the right of the fundamental frequencies are [overtones](https://en.wikipedia.org/wiki/overtone). <p> Note that Brown-Puckette algorithm on the showcqt uses kernels derived directly from frequency domain, the bandwidth calculation is actually [variable-Q](https://en.wikipedia.org/wiki/Variable-Q_transform), and showcqt uses linear/nth root amplitude scaling rather than logarithmic.](../../archives/Wikimedia%20Commons/CQT-piano-chord.png)
>
...
> ![A visualization of the relationship between the time domain and the frequency domain of a function, based on its Fourier transform. <br/> The Fourier transform takes an input function f \(in red\) in the "time domain" and converts it into a new function f-hat \(in blue\) in the "frequency domain". <p> <br/> In other words, the original function can be thought of as being "amplitude given time", and the Fourier transform of the function is "amplitude given frequency". <p> Shown here, a simple 6-component approximation of the square wave is decomposed \(exactly, for simplicity\) into 6 sine waves. These component frequencies show as very sharp peaks in the frequency domain of the function, shown as the blue graph.](../../archives/Wikimedia%20Commons/Fourier%20transform%20time%20and%20frequency%20domains%20%28small%29.gif)
>
...
>
> The transform of an even-symmetric real-valued function ⁠$f(t)=f_{_{\text{RE} } }$⁠ is also an even-symmetric real-valued function \(⁠${\widehat {f} }\mkern-3mu_{_{\text{RE} } }$⁠\). The time-shift, ⁠$g(t)=g_{_{\text{RE} } }+g_{_{\text{RO} } }$⁠, creates an imaginary component, ⁠$i\ {\widehat {g\ \!} }_{_{\text{IO} } }$⁠. \(See _[§ Symmetry](#symmetry)_.\)
...
- The transform of an imaginary-valued function \(⁠$i\ f_{_{\text{IE} } }+i\ f_{_{\text{IO} } }$⁠\) is the _[conjugate antisymmetric](even%20and%20odd%20functions.md#complex-valued%20functions)_ function ⁠${\widehat {f} }\mkern-3mu_{_{\text{RO} } }+i\ {\widehat {f} }\mkern-3mu_{_{\text{IE} } }$⁠, and the converse is true.
- The transform of a _[conjugate symmetric](even%20and%20odd%20functions.md#complex-valued%20functions)_ function $(f_{_{\text{RE} } }+i\ f_{_{\text{IO} } })$ is the real-valued function ⁠${\widehat {f} }\mkern-3mu_{_{\text{RE} } }+{\widehat {f} }\mkern-3mu_{_{\text{RO} } }$⁠, and the converse is true.
- The transform of a _[conjugate antisymmetric](even%20and%20odd%20functions.md#complex-valued%20functions)_ function $(f_{_{\text{RO} } }+i\ f_{_{\text{IE} } })$ is the imaginary-valued function ⁠$i\ {\widehat {f} }\mkern-3mu_{_{\text{IE} } }+i\ {\widehat {f} }\mkern-3mu_{_{\text{IO} } }$⁠, and the converse is true.
...
> ![[Graph](https://en.wikipedia.org/wiki/Graph_of_a_function) of the normalized [sinc function](https://en.wikipedia.org/wiki/sinc_function) sin\(πx\)/\(πx\). <br/> [Mathworld's version](http://mathworld.wolfram.com/SincFunction.html). The original was generated by gnuplot, while the more recent one was generated with matplotlib. File size was reduced by efficient coding of the axis drawing and labeling and by a reworking of the graph's path. The resulting path is smoother even though fewer points are used in its determination. <p> ## omegatron's instructions <p> ![File:System-search.svg](../../archives/Wikimedia%20Commons/System-search.svg) <p> See also: [Wikipedia graph-making tips](https://en.wikipedia.org/wiki/Wikipedia:How_to_create_graphs_for_Wikipedia_articles). Then I opened the resulting SVG file in [Inkscape](https://en.wikipedia.org/wiki/Inkscape), changed the line colors, brought the plot lines to the top, made all the lines 3 px wide, made the grid lines 2px wide and 50% transparency.](../../archives/Wikimedia%20Commons/Sinc%20function%20%28normalized%29.svg)
>
...
> ![This image shows the real and imaginary parts of an oscillating function times the complex exponential with the same oscillation.](../../archives/Wikimedia%20Commons/Onfreq.png)
>
...
|   208    |                                                                    $e^{-a|x|}$                                                                    |                                                                     ${\frac {2a}{a^{2}+4\pi ^{2}\xi ^{2} } }$                                                                     |                                            ${\sqrt {\frac {2}{\pi } } }\,{\frac {a}{a^{2}+\omega ^{2} } }$                                            |                                           ${\frac {2a}{a^{2}+\omega ^{2} } }$                                            |                                                                                                                                  For Re\(_a_\) \> 0. That is, the Fourier transform of a [two-sided decaying exponential function](Laplace%20distribution.md) is a [Lorentzian function](Lorentzian%20function.md#Probability%20density%20function%20(PDF)).                                                                                                                                  |
...
64. [Katznelson 2004](#CITEREFKatznelson2004), p. 173 The typical conventions in probability theory take _e_<sup>_iξx_</sup> instead of _e_<sup>−<!-- markdown separator -->_i_<!-- markdown separator -->2π<!-- markdown separator -->_ξx_</sup>. <a id="^ref-64"></a>^ref-64
...
- <a id="CITEREFFourier1822"></a> [Fourier, J.B. Joseph](Joseph%20Fourier.md) \(1822\), [_Théorie analytique de la chaleur_](https://books.google.com/books?id=TDQJAAAAIAAJ&q=%22c%27est-%C3%A0-dire+qu%27on+a+l%27%C3%A9quation%22&pg=PA525) \(in French\), Paris: Firmin Didot, père et fils, [OCLC](OCLC%20(identifier).md#identifiers%20and%20linked%20data) [2688081](https://search.worldcat.org/oclc/2688081)
...
- <a id="CITEREFHewittRoss1970"></a> Hewitt, Edwin; Ross, Kenneth A. \(1970\), _Abstract harmonic analysis: Structure and analysis for compact groups. Analysis on locally compact Abelian groups_, Die Grundlehren der mathematischen Wissenschaften, Band 152, vol. II, [Springer](Springer-Verlag.md#history), [MR](MR%20(identifier).md) [0262773](https://mathscinet.ams.org/mathscinet-getitem?mr=0262773)
