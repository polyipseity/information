...
5. Capture **content‑first details** and err on the side of completeness:
   ...
   - Experience with recent edits shows that new conceptual subsections often bring extended prose, analogies, or physical formulas (see diff on `electronic component.md`; energy/power sections compare a human body and a robot and derive $E=qV$, $P=IV$).  In such cases rewrite fragments into complete sentences, keep the exposition before the flashcard list (separated by `---`), and when numeric examples appear make the corresponding cards self‑contained (full problem statement on the left, stepwise solution on the right).  These patterns are now canonical styling and are useful reference points during review.
   ...
   - **One section, one flashcard block:** never merge cards belonging to different markdown sections.  Each heading or logical division in the document must have its own `Flashcards for this section` list or inline cloze group; the validator will warn if content appears without a corresponding block.
   - **Units inside math:** keep physical units inside the dollar‑delimited LaTeX expression (e.g. `$5\text{ V}$`, `$2.6\text{ mW}$`).  Placing units outside the math environment breaks rendering and confuses the generator.
   - **No next-lecture remarks:** avoid adding notes about what will be covered next time unless they refer to a major grading item (exam, project milestone, etc.).  Such comments belong in the syllabus or index, not the session content.
