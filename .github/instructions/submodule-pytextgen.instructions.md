...
- When updating the submodule pointer in the parent repo, include a short commit message `chore(submodules): update pytextgen to <ref>` and run the parent repo `pnpm run check` and `pnpm run test` to validate integration.
