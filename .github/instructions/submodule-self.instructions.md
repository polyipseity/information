...
- Apply the same repo-level developer and testing conventions inside submodules: use `pyproject.toml`, add unit and integration tests, run `pnpm run check` and `pnpm run test` locally (when possible supply explicit file arguments to limit scope and speed up the command), and document any submodule-specific deviations in the submodule's `AGENTS.md`.
