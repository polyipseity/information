...
- Full-repo `bun run check` and `bun run test` have pre-existing failures: MD058 in `tests/scripts/convert_wiki/snapshots/heavy_tables.expected.md`, 3 prettier SyntaxErrors in the gitignored `special/academia/HKUST/COMP 4211/.comp4211sources/` HTML files, and 2 symlink tests in `tests/scripts/test_convert_wiki.py`. Prefer `bun run check:py` and targeted `uv run --locked pytest <dir>`; before blaming a change, verify the failure exists on the baseline (e.g. `git stash` comparison).
