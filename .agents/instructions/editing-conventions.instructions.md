...
- Dependency management: `pyproject.toml` is authoritative; add runtime deps to `[project].dependencies` and developer/test tools to `[dependency-groups].dev`. Run `bun install` (which triggers `prepare`, running `uv sync`) to register Husky hooks and install dev extras locally.
