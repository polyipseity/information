...
- Ensure every Markdown section in a topic note contains flashcard entries (inline clozes or a rubric introduced by “Flashcards for this section are as follows:” preceded by `---`). The validator flags missing cards. Index and questions pages (e.g. `questions.md`) are not topic notes and do not need that rubric.
