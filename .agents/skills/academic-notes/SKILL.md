...
- Add a gloss by appending `::@::` after a linked term. The text after `::@::` becomes the card content; keep it concise, normally one or two sentences. Long derivations do not make good cards. When you lift sentences from prose or examples, also copy any adjacent diagrams or images into the question — cards with visual context are easier to learn and the validator will warn if an image appears in the source but not in the flashcard. It is fine to use LaTeX on **either side** of a card (left or right) as long as the usual math rules above are respected (no standalone math lines, dollar delimiters only, no newlines inside).
...
- Prefix every gloss with the complete hierarchical path (typically `topic / item`). In a course folder’s **topic-specific notes**, the course context is implied, so omit the course code prefix (write `probability measure / ...`, not `MATH 2431 / probability measure / ...`). Use an explicit course-code prefix (`COURSE / topic / item`) only when you are writing **index/session-level cards** or other mixed-context pages where multiple courses or topics might otherwise collide. Do not rely on indentation for context; only the literal text is used by the flashcard viewer.
...
- Keep gloss text single‑lined; use `<br/>` for right‑hand breaks, never sublists. For glosses that contain bullet‑like numbered items, write them inline as `(1) … <br/> (2) … <br/> (3) …` rather than using an actual list. Bibliographic references on the right‑hand side should be dash‑separated with a space before each `<br/>`.
