...
Generate the whole set from a single script kept in the same `attachments/` directory (`generate_circuit_diagrams.py`), the way `ELEC 1100` and `ELEC 2400` do. It writes its output beside itself, the SVGs and the script are both committed, and the script is the only thing edited when a drawing changes. Name each file for what it draws (`symbol_<thing>.svg`, `<thing>_<convention>.svg`), never for the slide, page, or lecture it came from, and keep one form per file (`symbol_source_battery.svg`, `symbol_source_circle.svg`) so the Markdown can place the set side by side: a drawing library cannot put a second figure into one image without coordinates.
...
Then wire it into the note, and check it:
