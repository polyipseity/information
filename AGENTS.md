...
__Skills metadata__: Each skill is self-described in its `SKILL.md` frontmatter with `name` and `description`; `argument-hint`, `compatibility`, `disable-model-invocation`, `license`, `metadata`, and `user-invocable` are the only other supported keys (see [`.agents/skills/README.md`](.agents/skills/README.md)). Agents may inspect individual skill documents directly.
...
- __[academic-crud-transcludes](.agents/skills/academic-crud-transcludes/SKILL.md)__ — Wikipedia articles included by reference
...
__Skill flow__: Start with `academic-ingest` for all ingestion. It dispatches to the appropriate CRUD skill, `academic-vision` covers every step that depends on what an image shows, and `academic-video` covers a video link the material carries. Run `academic-lint` after every edit. Most workflows use multiple skills in sequence; see individual skill files for cross-references and integration guidance.
